...
import { effectiveDelivery, fallbackVoiceIds, getVoiceProfile } from "./voiceProfile.js";

const ttsCache = new Map<string, { pcm: Buffer; sampleRate: number }>();
const TTS_CACHE_MAX = 50;

/**
 * WHY THERE IS HEALTH STATE HERE AT ALL.
 *
 * On 2026-08-17 a voice id that does not exist on the account was saved. Every
 * text-to-speech call 404'd, `generateTTS` returned null, and Harvey went
 * completely silent — while /health cheerfully reported the key configured and
 * the voice named. The failure was real, total, and invisible to everything
 * except the Fly logs. So the outcome of the last attempt is now remembered and
 * published, and a rejected voice falls back instead of ending in silence.
 */
interface TtsHealth {
  /** null until Harvey has actually tried to say something this boot. */
  lastOk: boolean | null;
  lastError: string | null;
  lastAt: string | null;
  /** Set when the configured voice was rejected and another one is speaking. */
  substitutedFrom: string | null;
  substitutedTo: string | null;
}
const health: TtsHealth = {
...
export function ttsHealthReport(): TtsHealth {
  return { ...health };
}
...
export async function generateTTS(rawText: string): Promise<{ pcm: Buffer; sampleRate: number } | null> {
   ...
   */
  const attempts = [profile.voiceId, ...fallbackVoiceIds(profile.voiceId)]
  ...
  for (const voiceId of attempts) {
    ...
    if (rejectedVoices.has(voiceId)) continue;
    ...
    if (res.ok) {
      ...
      if (!pcm.length) { lastDetail = "empty audio"; continue; }
    ...
    if (res.status === 404 || /voice.*not.*found/i.test(lastDetail)) {
      ...
      console.error(
        `[hull/tts] voice "${voiceId === profile.voiceId ? profile.voiceName : voiceId}" (${voiceId}) ` +
