...
exports.analyzeReelViaOpenShorts = analyzeReelViaOpenShorts;
/**
 ...
 */
async function analyzeReelViaOpenShorts(url, note) {
    const health = await checkOpenShortsHealth();
    if (!health.running) {
    ...
    let jobId;
    try {
        const submit = await axios_1.default.post(`${OPENSHORTS_BASE_URL}/api/reel/analyze`, formData, {
            headers: formData.getHeaders(),
            timeout: 30000,
        });
        ...
        const detail = (axios_1.default.isAxiosError(err) && err.response?.data?.detail) ||
            (err instanceof Error ? err.message : String(err));
        return { status: "failed", error: `Couldn't start the reel analysis: ${detail}` };
    }
    if (!jobId)
        return { status: "failed", error: "Reel analysis did not start (no job id returned)." };
    for (let attempt = 0; attempt < REEL_MAX_POLL_ATTEMPTS; attempt++) {
        try {
            const response = await axios_1.default.get(`${OPENSHORTS_BASE_URL}/api/reel/jobs/${jobId}`, {
                timeout: STATUS_CHECK_TIMEOUT_MS,
            });
            const data = response.data;
            const status = String(data.status || "queued");
            ...
            if (status === "failed") {
                return { status: "failed", error: String(data.error || "Reel analysis failed"), metadata: data.metadata || {} };
            }
        }
        catch {
