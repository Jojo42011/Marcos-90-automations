 ...
 */
async function bridgeGet(url, token) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            const fix = res.status === 401 || res.status === 403
...
async function fetchProperties(opts = {}) {
    ...
    const filters = [];
    if (opts.since)
        filters.push(`ModificationTimestamp gt ${opts.since}`);
    if (opts.filter?.trim())
        filters.push(`(${opts.filter.trim()})`);
    ...
    while (url && records.length < max) {
        ...
        if ("error" in page) {
            ...
            if (records.length)
                return { ok: true, records: records.slice(0, max), truncated: true };
            return page;
        }
        records.push(...page.value);
        if (!page.nextLink || page.value.length === 0)
            break;
        url = page.nextLink;
    }
