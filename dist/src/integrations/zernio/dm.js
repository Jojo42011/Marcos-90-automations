...
exports.ZERNIO_ACK_BUDGET_MS = void 0;
exports.zernioTikTokAccountId = zernioTikTokAccountId;
exports.isZernioDmConfigured = isZernioDmConfigured;
exports.zernioWebhookSecretConfigured = zernioWebhookSecretConfigured;
exports.verifyZernioSignature = verifyZernioSignature;
exports.parseZernioInboundMessage = parseZernioInboundMessage;
exports.fetchVaOpener = fetchVaOpener;
...
function zernioTikTokAccountId() {
    return process.env.ZERNIO_TIKTOK_ACCOUNT_ID?.trim() ?? "";
}
function isZernioDmConfigured() {
    return Boolean(apiKey());
}
function zernioWebhookSecretConfigured() {
 ...
 */
function verifyZernioSignature(rawBody, signature) {
 ...
 */
function parseZernioInboundMessage(body) {
    if (!body || typeof body !== "object")
        return null;
    const b = body;
    if (b.event !== "message.received")
        return null;
    const message = (b.message ?? {});
    const conversation = (b.conversation ?? {});
    const account = (b.account ?? {});
    const sender = (message.sender ?? {});
    if (str(message.direction) === "outgoing")
        return null;
    ...
    if (!senderId || !conversationId || !accountId)
        return null;
    return {
        eventId: str(b.id) ?? `${conversationId}:${str(message.id) ?? (0, crypto_1.randomUUID)()}`,
        conversationId,
        accountId,
 ...
 * throws.
 */
async function fetchVaOpener(conversationId, accountId) {
    if (!isZernioDmConfigured())
        return null;
    try {
        const qs = new URLSearchParams({ accountId, sortOrder: "asc", limit: "10" });
        const r = await zernioFetch(`/inbox/conversations/${encodeURIComponent(conversationId)}/messages?${qs}`, { method: "GET" });
        if (!r.ok)
            return null;
...
function toIncomingWebhookPayload(evt, vaOpener) {
    return {
 ...
 */
async function sendZernioReply(input) {
    if (!isZernioDmConfigured()) {
        return { success: false, status: 0, error: "ZERNIO_DM_API_KEY is not set" };
    }
    const text = input.text?.trim();
    if (!text)
        return { success: false, status: 0, error: "Empty reply, nothing sent" };
    try {
        const r = await zernioFetch(`/inbox/conversations/${encodeURIComponent(input.conversationId)}/messages`, {
            method: "POST",
