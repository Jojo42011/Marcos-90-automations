 ...
 */
import { getCaps, spentThisMonth, spentToday } from "../../core/aiUsageStore.js";
import { estimateCostUsd } from "./catalog.js";
import type { BudgetVerdict, ModelJob } from "./types.js";
 ...
 */
const DEGRADABLE: ModelJob[] = ["chat_fast", "chat_deep", "classify", "summarize", "extract", "schedule", "vision"];

export interface CheckBudgetInput {
  job: ModelJob;
  model: string;
  /** From the context plan — what we are about to send. */
  estimatedInputTokens: number;
  /** Output reservation. The whole reservation is priced, since it may be used. */
  maxTokens?: number;
  /** Caller's own ceiling for this one call, tighter than the global one. */
  maxCostUsd?: number;
}

const usd = (n: number) => `$${n.toFixed(n < 0.01 ? 4 : 2)}`;

export function checkBudget(input: CheckBudgetInput): BudgetVerdict {
  const caps = safe(() => getCaps(), {
    dailyCapUsd: 10,
    monthlyCapUsd: 150,
    maxCostPerCallUsd: 0.5,
  });
  const spentTodayUsd = safe(() => spentToday(), 0);
  const spentMonthUsd = safe(() => spentThisMonth(), 0);
  ...
  const perCallCeiling = Math.min(
    ...
    input.maxCostUsd && input.maxCostUsd > 0 ? input.maxCostUsd : Number.POSITIVE_INFINITY,
  );
...
export function estimateCallCostUsd(model: string, inputTokens: number, maxTokens: number): number {
  return estimateCostUsd(model, inputTokens, maxTokens);
}
