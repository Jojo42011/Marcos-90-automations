...
exports.isZernioConfigured = isZernioConfigured;
exports.zernioBaseUrl = zernioBaseUrl;
exports.mapZernioPayload = mapZernioPayload;
exports.fetchZernioPlatform = fetchZernioPlatform;
 ...
 */
function mapZernioPayload(payload) {
    const root = (payload && typeof payload === "object" ? payload : {});
    // Providers commonly nest under data/result/metrics/stats.
    const nestedKey = ["data", "result", "metrics", "stats", "summary"].find((k) => root[k] && typeof root[k] === "object" && !Array.isArray(root[k]));
    const src = (nestedKey ? root[nestedKey] : root);
    const stats = {
 ...
 */
async function fetchZernioPlatform(platform) {
    const key = process.env.ZERNIO_API_KEY?.trim();
    if (!key)
        return { ok: false, configured: false, error: "ZERNIO_API_KEY is not set" };
