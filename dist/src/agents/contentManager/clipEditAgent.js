...
const TOOLS = [
    ...
    },
    {
        ...
        description: "PROPOSE (do not execute) a video edit. Returns a proposal the human must confirm before any render. " +
        ...
        onToolCall: async (name, args) => {
            ...
            if (name === "propose_video_edit") {
                ...
                if (op === "trim") {
                    ...
                    if (!["auto", "warm", "cool", "punch", "flat"].includes(style))
                        return { error: "color_style must be auto, warm, cool, punch, or flat" };
                    proposal = {
                    ...
                    if (!["zoom_in", "zoom_out", "punch_in"].includes(zt))
                        return { error: "zoom_type must be zoom_in, zoom_out, or punch_in" };
                    const zs = Number(args.zoom_start);
                    const ze = Number(args.zoom_end);
                    if (!Number.isFinite(zs) || !Number.isFinite(ze) || ze <= zs)
                        return { error: "zoom needs numeric zoom_start < zoom_end" };
                    ...
                    proposal = {
                        ...
                        spec: { effects: [{ type: zt, start: zs, end: ze, amount }] },
                    };
                }
                else {
