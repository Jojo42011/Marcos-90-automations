...
exports.duplicateAutoPlan = duplicateAutoPlan;
...
function normalizeAutoPlanSteps(raw) {
    ...
    for (const item of raw) {
        ...
        const anchor = s.anchor === "prev_step" ||
        ...
        if (s.offsetUnit === "minutes" || s.offsetUnit === "hours")
            next.offsetUnit = s.offsetUnit;
        if (anchor !== "enrollment")
            next.anchor = anchor;
        ...
        if (s.contingent === true && next.anchor === "prev_step")
            next.contingent = true;
        if (type === "email") {
            if (typeof s.subject === "string")
                next.subject = s.subject;
            const addrs = (v) => Array.isArray(v)
            ...
            if (bcc.length)
                next.bcc = bcc;
            if (typeof s.templateId === "string" && s.templateId.trim())
                next.templateId = s.templateId.trim();
        }
        ...
        if (type === "email" || type === "text") {
            if (typeof s.sendFrom === "string" && s.sendFrom.trim())
                next.sendFrom = s.sendFrom.trim().slice(0, 60);
        }
        if (type === "task") {
            next.assignedTo = typeof s.assignedTo === "string" && s.assignedTo ? s.assignedTo : "Marco Puga";
            const pr = Number(s.taskPriority);
            if (Number.isInteger(pr) && pr >= 1 && pr <= 9)
                next.taskPriority = pr;
            if (typeof s.instructions === "string" && s.instructions.trim())
                next.instructions = s.instructions.trim();
            if (typeof s.notes === "string" && s.notes.trim())
                next.notes = s.notes.trim().slice(0, 2000);
            if (s.recurrence === "daily" || s.recurrence === "weekly" ||
...
function duplicateAutoPlan(id) {
    ...
    for (const st of steps) {
        if (st.afterStepId)
            st.afterStepId = idMap.get(st.afterStepId) ?? st.afterStepId;
    }
