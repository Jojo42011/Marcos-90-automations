...
exports.planTransactionImport = planTransactionImport;
exports.applyTransactionImport = applyTransactionImport;
...
function mapStatus(raw) {
    ...
    if (!s)
        return "active";
    if (s.includes("clos") || s.includes("sold") || s.includes("settl"))
        return "closed";
    if (s.includes("under contract") || s.includes("mutual") || s.includes("contract"))
        return "under_contract";
    if (s.includes("pend"))
        return "pending";
    if (s.includes("cancel") || s.includes("withdraw") || s.includes("expir") || s.includes("terminat"))
        return "cancelled";
    if (s.includes("fell") || s.includes("fall"))
        return "fell_through";
    return "active";
}
function mapSide(raw) {
    const s = raw.trim().toLowerCase();
    if (s.includes("dual") || s.includes("both"))
        return "dual";
    if (s.startsWith("s") || s.includes("sell") || s.includes("list"))
        return "seller";
    return "buyer";
}
function money(raw) {
...
function externalKey(address, mls) {
    const m = mls.trim();
    if (m)
        return `mls:${m.toLowerCase()}`;
 ...
 */
function planTransactionImport(csvText, opts) {
    ...
    if (ix.address == null) {
        ...
        plan.errors.push(`No Address/Property column found. Headers seen: ${rows[0].map((h) => h.trim()).filter(Boolean).join(", ")}`);
        return plan;
    }
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (!address) {
            plan.skip++;
            plan.rows.push({ line: i + 1, action: "skip", reason: "no address", externalKey: "", transaction: null });
            continue;
        }
        const mls = get(r, "mls");
        ...
        if (seenInFile.has(key)) {
            plan.skip++;
            plan.rows.push({ line: i + 1, action: "skip", reason: "duplicate row in this file", externalKey: key, transaction: null });
            continue;
        }
        seenInFile.add(key);
        ...
        const existing = (0, transactionsStore_js_1.getTransactionByExternalKey)(key);
        if (existing) {
            plan.update++;
 ...
 */
function applyTransactionImport(plan, meta) {
    const importedAt = new Date().toISOString();
    let created = 0;
    let updated = 0;
    const errors = [...plan.errors];
    for (const row of plan.rows) {
        if (row.action === "skip" || !row.transaction)
            continue;
        try {
            if (row.action === "update" && row.existingId) {
                (0, transactionsStore_js_1.updateTransaction)(row.existingId, { ...row.transaction, importedAt });
                updated++;
            }
            else {
                (0, transactionsStore_js_1.createTransaction)({ ...row.transaction, importedAt });
                created++;
            }
        }
        catch (err) {
    ...
    (0, transactionsStore_js_1.recordTransactionImport)({
    ...
    return {
        ...
        total: (0, transactionsStore_js_1.getAllTransactions)().length,
    };
}
