...
const outreachPublicPages_js_1 = require("./core/outreachPublicPages.js");
const autoPlanScheduling_js_1 = require("./core/autoPlanScheduling.js");
const mergeFields_js_1 = require("./core/mergeFields.js");
...
const sendingIdentity_js_1 = require("./core/sendingIdentity.js");
...
/* ── THE SITE LOCK ──────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────── */
app.use((0, lockdown_js_1.makeLockdown)({
...
function machineTokenOk(req) {
    ...
    const eq = (a) => a.length === expected.length && timingSafeEqualStr(a, expected);
...
app.get("/shell", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "shell.html"));
});
...
app.get("/who", (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "who.html"));
});
app.get("/", (_req, res) => {
    res.redirect("/shell");
});
...
app.get("/dashboard", requireAuthPage, (req, res) => {
    const embedded = req.query.embed === "1" || req.get("sec-fetch-dest") === "iframe";
    if (!embedded) {
...
app.get("/cma", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "cma.html"));
});
/** One listing, in full — where a click in the MLS tab lands. */
app.get("/listing", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "listing.html"));
});
...
app.get("/tracker", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "tracker.html"));
});
...
app.get("/harvey", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "harvey.html"));
});
...
async function currentSessionUser(req) {
    ...
    if (!token)
        return null;
    const { getSession } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
...
function requireAuthAdminPage(req, res, next) {
    if (!siteLoginEnabled()) {
        next();
        return;
    }
...
function requireAuthAdminApi(req, res, next) {
    if (!siteLoginEnabled()) {
        next();
        return;
    }
...
app.get("/change-password", (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "change-password.html"));
});
...
app.post("/api/auth/login", express_1.default.json(), async (req, res) => {
    ...
    const { getUserByEmail } = await Promise.resolve().then(() => __importStar(require("./core/users.js")));
    const { verifyPassword, createSession, recordLogin, pruneExpiredSessions } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
...
app.post("/api/auth/logout", async (req, res) => {
    const token = getCookieValue(req, SESSION_COOKIE);
    if (token) {
        const { destroySession } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
...
app.get("/api/auth/login-history", requireAuthAdminApi, async (_req, res) => {
    const { getLoginHistory } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
    res.json({ rows: getLoginHistory(150) });
});
app.get("/api/auth/audit-log", requireAuthAdminApi, async (_req, res) => {
    const { getAuditLog } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
    res.json({ rows: getAuditLog(250) });
});
app.post("/api/auth/team", express_1.default.json(), requireAuthAdminApi, async (req, res) => {
    ...
    const { getUserByEmail, createUser } = await Promise.resolve().then(() => __importStar(require("./core/users.js")));
    const { ROLE_PERMISSIONS } = await Promise.resolve().then(() => __importStar(require("./core/types.js")));
    if (getUserByEmail(email)) {
        res.status(409).json({ error: "A team member with that email already exists" });
        return;
    }
    const { hashPassword, genTempPassword, recordAudit } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
...
app.post("/api/auth/team/:id/reset-password", requireAuthAdminApi, async (req, res) => {
    ...
    const { getUserById, updateUser } = await Promise.resolve().then(() => __importStar(require("./core/users.js")));
    const target = getUserById(id);
    if (!target) {
        res.status(404).json({ error: "User not found" });
        return;
    }
    const { hashPassword, genTempPassword, destroyAllSessionsForUser, recordAudit } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
    const tempPassword = genTempPassword();
...
app.post("/api/auth/change-password", express_1.default.json(), async (req, res) => {
    const user = await currentSessionUser(req);
    if (!user) {
        res.status(401).json({ error: "Sign in required" });
        return;
    ...
    const { verifyPassword, hashPassword, recordAudit } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
...
app.get("/team", requireAuthAdminPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "team.html"));
});
...
app.get("/brivity-import", requireAuthAdminPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "brivity-import.html"));
});
...
app.post("/api/auth/bootstrap", express_1.default.json(), async (req, res) => {
    const { getUsers, getUserByEmail, updateUser } = await Promise.resolve().then(() => __importStar(require("./core/users.js")));
    ...
    const { hashPassword } = await Promise.resolve().then(() => __importStar(require("./core/authStore.js")));
 ...
 */
async function quoWebhookUrl(override) {
    const { quoWebhookSecret } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    ...
    const base = (override || process.env.PUBLIC_BASE_URL || "https://marco-90-automation.fly.dev")
        .trim().replace(/\/+$/, "");
    if (!/^https:\/\//.test(base))
        return null; // Quo will not call plain http
...
app.get("/api/quo/status", async (req, res) => {
    ...
    const { isQuoConfigured, checkQuo, getQuoPhoneNumber } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    const { quoMessageCount, quoThreadCount } = await Promise.resolve().then(() => __importStar(require("./core/quoStore.js")));
    const { lastQuoSyncAt } = await Promise.resolve().then(() => __importStar(require("./core/quoSync.js")));
    if (!isQuoConfigured()) {
        res.json({
    ...
    /* Whether Quo is calling us matters operationally — without the webhook the
       ...
       rather than assumed. */
    let webhook = { registered: false, url: null };
    try {
        const { listWebhooks } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
        const url = await quoWebhookUrl();
        const hooks = await listWebhooks();
        webhook = { registered: hooks.some((w) => w.url === url), url };
    }
    catch (err) {
        webhook = { registered: false, url: null, error: err.message };
    }
    res.json({
        configured: true,
        verified: check.ok,
...
app.get("/api/quo/threads", async (req, res) => {
    ...
    const { listQuoThreads } = await Promise.resolve().then(() => __importStar(require("./core/quoStore.js")));
    const { phoneKey } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    ...
    for (const l of snap.leads) {
        ...
        if (k && !byPhone.has(k))
            byPhone.set(k, { id: l.id, name: l.name });
    }
    res.json({
        ok: true,
...
app.get("/api/quo/threads/:peer", async (req, res) => {
    ...
    const { getQuoThread } = await Promise.resolve().then(() => __importStar(require("./core/quoStore.js")));
    const { phoneKey } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
...
app.get("/api/quo/numbers", async (req, res) => {
    ...
    const { listPhoneNumbers, isQuoConfigured, getQuoPhoneNumberId, getQuoPhoneNumber } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    const users = (0, users_js_1.getUsers)().filter((u) => u.active !== false);
    const assignedBy = new Map();
    for (const u of users)
        if (u.quoPhoneNumberId)
            assignedBy.set(u.quoPhoneNumberId, u.name);
    if (!isQuoConfigured()) {
        res.json({
            ok: false,
            configured: false,
    ...
    try {
        ...
        res.status(502).json({ ok: false, configured: true, error: err.message, numbers: [] });
    }
});
/** Assign (or clear) the Quo line a CRM user texts from. */
app.post("/api/quo/numbers/assign", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const target = (0, users_js_1.getUsers)().find((u) => u.id === userId);
    if (!target) {
    ...
    if (!phoneNumberId) {
        const cleared = (0, users_js_1.updateUser)(userId, { quoPhoneNumberId: undefined, quoPhoneNumber: undefined });
        res.json({ ok: true, cleared: true, user: cleared });
        return;
    }
    const { listPhoneNumbers, isQuoConfigured } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    if (!isQuoConfigured()) {
        res.status(503).json({ error: "Quo is not configured on this server." });
        return;
    }
    let numbers;
    try {
        numbers = await listPhoneNumbers();
    }
    catch (err) {
        res.status(502).json({ error: `Could not read Quo's numbers: ${err.message}` });
        return;
    }
    ...
    const clash = (0, users_js_1.getUsers)().find((u) => u.id !== userId && u.quoPhoneNumberId === phoneNumberId && u.active !== false);
    if (clash) {
        res.status(409).json({ error: `${match.number} is already assigned to ${clash.name}. Clear theirs first.` });
        return;
    }
    const saved = (0, users_js_1.updateUser)(userId, { quoPhoneNumberId: match.id, quoPhoneNumber: match.number });
...
app.get("/api/quo/my-line", async (req, res) => {
    ...
    const { getQuoPhoneNumberId, getQuoPhoneNumber } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    res.json({
        ok: true,
        line: (0, sendingIdentity_js_1.resolveSendingLine)(sessionUserSync(req), {
            defaultId: getQuoPhoneNumberId(),
            defaultNumber: getQuoPhoneNumber(),
        }),
    });
...
app.post("/api/quo/send", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    const { sendText, toE164, phoneKey, isQuoConfigured } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    if (!isQuoConfigured()) {
        res.status(503).json({ error: "Quo is not configured" });
        return;
    }
    ...
    const { getQuoPhoneNumberId, getQuoPhoneNumber } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    const line = (0, sendingIdentity_js_1.resolveSendingLine)(sessionUserSync(req), {
        defaultId: getQuoPhoneNumberId(),
        defaultNumber: getQuoPhoneNumber(),
    });
    if (!line.id) {
        res.status(503).json({ error: line.explain });
        return;
    }
    try {
        ...
        const { upsertQuoMessage } = await Promise.resolve().then(() => __importStar(require("./core/quoStore.js")));
        upsertQuoMessage({
            id: sent.id,
        ...
        try {
            ...
            if (lead) {
                const { appendLeadActivity } = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
 ...
 */
app.post("/api/quo/register-webhook", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { ensureMessageWebhook } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
    try {
        const url = await quoWebhookUrl(typeof req.body?.baseUrl === "string" ? req.body.baseUrl : undefined);
        if (!url) {
            res.status(400).json({ error: "No public base URL — set PUBLIC_BASE_URL or pass baseUrl" });
            return;
        }
...
app.post("/api/quo/sync", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { syncQuoMessages } = await Promise.resolve().then(() => __importStar(require("./core/quoSync.js")));
 ...
 */
app.post("/api/quo/webhook", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    try {
        const { quoWebhookSecret } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
        const secret = quoWebhookSecret();
        ...
        const to = Array.isArray(data.to) ? String(data.to[0] || "") : "";
        const peer = direction === "outgoing" ? to : from;
        if (!id || !peer) {
            res.status(202).json({ ok: true, ignored: "not a message payload" });
            return;
        }
        const { phoneKey } = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
        const { upsertQuoMessage } = await Promise.resolve().then(() => __importStar(require("./core/quoStore.js")));
...
app.get("/api/llm/health", async (req, res) => {
    ...
    if (!live.ok) {
        ...
        const byReason = {};
        for (const r of gate.recent)
            if (r.outcome === "canned_redirect" && r.reason)
...
app.get("/api/mls/facets", async (req, res) => {
    ...
    const { getMlsFacets } = await Promise.resolve().then(() => __importStar(require("./core/mlsFacets.js")));
    try {
        res.json(getMlsFacets(req.query.refresh === "1"));
    }
    catch (err) {
        res.status(503).json({ error: err.message, hint: "The MLS mirror is not readable — check /api/mls/status." });
    }
});
/** Live match count + a sample, so criteria are sanity-checked before saving. */
app.post("/api/outreach/preview", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { countMatching, findMatching, describeCriteria } = await Promise.resolve().then(() => __importStar(require("./core/listingCriteria.js")));
    const criteria = (req.body?.criteria && typeof req.body.criteria === "object" ? req.body.criteria : {});
    try {
        res.json({
            ok: true,
...
app.get("/api/leads/:id/outreach", async (req, res) => {
    ...
    const { listAlerts, listReports, listSends, engagementForLead } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const { countMatching, describeCriteria } = await Promise.resolve().then(() => __importStar(require("./core/listingCriteria.js")));
    const { isSmtpConfigured } = await Promise.resolve().then(() => __importStar(require("./integrations/smtp/index.js")));
    let mlsReady = false;
    try {
        const { listingCounts } = await Promise.resolve().then(() => __importStar(require("./core/listingsStore.js")));
...
app.post("/api/leads/:id/listing-alerts", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (!name) {
        res.status(400).json({ error: "A name for the alert is required" });
        return;
    }
    ...
    const alert = insertAlert({
        ...
        frequency: freq,
        criteria: (b.criteria && typeof b.criteria === "object" ? b.criteria : {}),
        paused: false,
        createdAt: now,
        updatedAt: now,
        lastSentAt: null,
        nextSendAt: nextAlertSend(freq),
        lastMatchCount: null,
...
app.patch("/api/listing-alerts/:id", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { getAlert, updateAlert } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const { nextAlertSend } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
    const existing = getAlert(String(req.params.id));
    if (!existing) {
        res.status(404).json({ error: "No such listing alert" });
        return;
    }
    const b = (req.body || {});
    const patch = {};
    ...
    if ("cc" in b)
        patch.cc = typeof b.cc === "string" && b.cc.trim() ? b.cc.trim() : null;
    if (typeof b.sendEmail === "boolean")
        patch.sendEmail = b.sendEmail;
    if (typeof b.paused === "boolean")
        patch.paused = b.paused;
    if (b.criteria && typeof b.criteria === "object")
        patch.criteria = b.criteria;
    if (["daily", "weekly", "monthly"].includes(String(b.frequency))) {
        ...
        patch.nextSendAt = nextAlertSend(String(b.frequency));
    }
...
app.delete("/api/listing-alerts/:id", async (req, res) => {
    ...
    const { deleteAlert } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    res.json({ ok: deleteAlert(String(req.params.id)) });
});
app.post("/api/listing-alerts/:id/send", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { sendAlertNow } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
    const out = await sendAlertNow(String(req.params.id), { force: req.body?.force !== false });
    res.status(out.ok ? 200 : 502).json(out);
});
/* -------------------------------------------------------- market reports -- */
app.post("/api/leads/:id/market-reports", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (!address) {
        res.status(400).json({ error: "An address for the report is required" });
        return;
    }
    ...
    const report = insertReport({
        ...
        drip,
        criteria: (b.criteria && typeof b.criteria === "object" ? b.criteria : {}),
        subject: (b.subject && typeof b.subject === "object" ? b.subject : {}),
        ...
        nextSendAt: drip ? nextReportSend(freq) : null,
        lastViewedAt: null,
        viewCount: 0,
        createdBy: (await currentSessionUser(req))?.name ?? null,
    });
    let firstSend = null;
    if (b.sendNow === true)
        firstSend = await sendReportNow(report.id);
    res.status(201).json({ ok: true, report, firstSend });
});
app.patch("/api/market-reports/:id", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { getReport, updateReport } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const { nextReportSend } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
    const existing = getReport(String(req.params.id));
    if (!existing) {
        res.status(404).json({ error: "No such market report" });
        return;
    }
    const b = (req.body || {});
    const patch = {};
    ...
    if (typeof b.paused === "boolean")
        patch.paused = b.paused;
    if (typeof b.includeHomeValue === "boolean")
        patch.includeHomeValue = b.includeHomeValue;
    ...
    if ("emailMessage" in b)
        patch.emailMessage = typeof b.emailMessage === "string" && b.emailMessage.trim() ? b.emailMessage.trim() : null;
    if (b.criteria && typeof b.criteria === "object")
        patch.criteria = b.criteria;
...
app.delete("/api/market-reports/:id", async (req, res) => {
    ...
    const { deleteReport } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
...
app.get("/api/market-reports/:id/preview", async (req, res) => {
    ...
    const { getReport } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const { buildMarketReport } = await Promise.resolve().then(() => __importStar(require("./core/marketReport.js")));
    const { anchorFor } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
...
app.post("/api/market-reports/preview", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { buildMarketReport, postalFromAddress, cityFromAddress } = await Promise.resolve().then(() => __importStar(require("./core/marketReport.js")));
    const b = (req.body || {});
    ...
    try {
        res.json({
            ...
            built: buildMarketReport({
                criteria,
                anchor: {
                    postalCode: criteria.postalCodes?.[0] || postalFromAddress(address),
                    city: criteria.cities?.[0] || (address ? cityFromAddress(address) : null),
                },
                subject: (b.subject && typeof b.subject === "object" ? b.subject : {}),
...
app.post("/api/market-reports/:id/send", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { sendReportNow } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
...
app.get("/api/outreach/status", async (req, res) => {
    ...
    const { outreachCounts, hotEngagement } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const { listingCounts, lastSuccessfulSync } = await Promise.resolve().then(() => __importStar(require("./core/listingsStore.js")));
    const { isSmtpConfigured } = await Promise.resolve().then(() => __importStar(require("./integrations/smtp/index.js")));
    const since = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString();
    let mls = null;
    try {
        mls = { ...listingCounts(), lastSyncAt: lastSuccessfulSync()?.finishedAt ?? null };
    }
    catch {
        mls = null;
    }
...
app.post("/api/outreach/run", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { runOutreach } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
...
app.get("/api/outreach/coverage", async (req, res) => {
    ...
    const { listAlerts, listReports } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
    const snap = await (0, db_js_1.getDashboardSnapshot)();
    ...
    const buyers = snap.leads.filter((l) => l.crmIntent !== "seller" && !!l.email && !dead.has(String(l.crmStatus || "")));
    const sellers = snap.leads.filter((l) => l.crmIntent === "seller" && !!l.email && !dead.has(String(l.crmStatus || "")));
    const brief = (l) => ({ id: l.id, name: l.name, email: l.email ?? null, status: l.crmStatus ?? null });
    res.json({
        ok: true,
...
app.get("/r/open", async (req, res) => {
    try {
        ...
        if (sendId) {
            const { getOutreachDb, markSendOpened } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
            const row = getOutreachDb()
                .prepare(`SELECT kind, subscription_id, lead_id FROM outreach_sends WHERE id = ?`)
                .get(sendId);
            if (row) {
                markSendOpened(sendId);
                const { recordEngagement } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
                recordEngagement({
...
app.get("/r/click", async (req, res) => {
    ...
    try {
        const { getOutreachDb } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
        const { noteEngagement } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
        const row = getOutreachDb()
            .prepare(`SELECT kind, subscription_id, lead_id FROM outreach_sends WHERE id = ?`)
            .get(sendId);
        if (row && listingKey) {
...
app.get("/r/stop", async (req, res) => {
    ...
    let done = false;
    try {
        const { updateAlert, updateReport, getAlert, getReport } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
        if (kind === "alert" && getAlert(id)) {
            updateAlert(id, { paused: true, sendEmail: false });
            done = true;
        }
        if (kind === "report" && getReport(id)) {
            updateReport(id, { paused: true, drip: false });
            done = true;
        }
    }
...
app.get("/r/report", async (req, res) => {
    ...
    try {
        const { getReport, updateReport } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
        const { buildMarketReport } = await Promise.resolve().then(() => __importStar(require("./core/marketReport.js")));
        const { anchorFor, noteEngagement } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
        const report = getReport(id);
        if (!report) {
            res.status(404).send((0, outreachPublicPages_js_1.publicShell)("That report is no longer available.", ""));
            return;
        }
        ...
        void sendId;
        res.setHeader("Content-Type", "text/html; charset=utf-8");
        res.end((0, outreachPublicPages_js_1.renderPublicReport)(report, built));
    }
    catch (err) {
        res.status(500).send((0, outreachPublicPages_js_1.publicShell)("We couldn't build that report just now.", err.message));
    }
});
/** Published CMA — the client-facing page step 7 creates. */
app.get("/c/:id", async (req, res) => {
    try {
        const store = await Promise.resolve().then(() => __importStar(require("./core/cmaStore.js")));
        const { cmaResults } = await Promise.resolve().then(() => __importStar(require("./core/cmaComps.js")));
        const { renderPublicCma } = await Promise.resolve().then(() => __importStar(require("./core/outreachPublicPages.js")));
        ...
        if (!session || session.status !== "published") {
            res.status(404).send((0, outreachPublicPages_js_1.publicShell)("That report is not available.", "The link may have expired, or the report may not have been published yet. Reply to Marco and he'll re-send it."));
            return;
        }
        ...
        res.status(500).send((0, outreachPublicPages_js_1.publicShell)("We couldn't load that report.", err.message));
    }
});
...
app.get("/l/:key", async (req, res) => {
    try {
        ...
        if (!found) {
            res.status(404).send((0, outreachPublicPages_js_1.publicShell)("That home is no longer on the market.", "It may have gone under contract or been withdrawn. Reply to Marco's email and he'll find you something similar."));
            return;
        }
        res.setHeader("Content-Type", "text/html; charset=utf-8");
        res.end((0, outreachPublicPages_js_1.renderPublicListing)(found));
    }
    catch (err) {
        res.status(500).send((0, outreachPublicPages_js_1.publicShell)("We couldn't load that listing.", err.message));
    }
});
...
app.get("/api/harvey/exec-status", async (req, res) => {
    ...
    const { execStatus } = await Promise.resolve().then(() => __importStar(require("./core/codeExec.js")));
...
async function harveyBudgetBlock() {
    const { getCaps, spentToday, spentThisMonth } = await Promise.resolve().then(() => __importStar(require("./core/aiUsageStore.js")));
    const caps = getCaps();
    const spentTodayUsd = spentToday();
    const spentMonthUsd = spentThisMonth();
    const ratio = Math.max(caps.dailyCapUsd > 0 ? spentTodayUsd / caps.dailyCapUsd : 0, caps.monthlyCapUsd > 0 ? spentMonthUsd / caps.monthlyCapUsd : 0);
    return {
        spentTodayUsd,
        spentMonthUsd,
...
function harveyToolResultForUi(result) {
    const LIMIT = 4000;
...
app.post("/api/harvey/chat", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    ...
    const memory = await Promise.resolve().then(() => __importStar(require("./harvey/memory.js")));
    const { runAgentLoop } = await Promise.resolve().then(() => __importStar(require("./hull/agentLoop.js")));
    const { BudgetRefusedError } = await Promise.resolve().then(() => __importStar(require("./hull/providers/index.js")));
    const sessionId = memory.getOrCreateSessionId(typeof body.sessionId === "string" ? body.sessionId : undefined);
    ...
    const rememberTurn = (speech) => {
        memory.appendSessionTurn(sessionId, "user", message);
        memory.appendSessionTurn(sessionId, "assistant", speech);
        void Promise.resolve().then(() => __importStar(require("./hull/memory/extraction.js"))).then((m) => m.runPostConversationExtraction(sessionId, [
    ...
    if (wantsStream) {
        ...
        try {
            ...
            const messageOut = err instanceof BudgetRefusedError
...
app.get("/api/harvey/models", async (req, res) => {
    ...
    try {
        const { listModelsForUi } = await Promise.resolve().then(() => __importStar(require("./hull/providers/index.js")));
...
app.post("/api/harvey/models/route", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    try {
        const { MODEL_JOBS, describeModel, routingTable } = await Promise.resolve().then(() => __importStar(require("./hull/providers/index.js")));
        if (!MODEL_JOBS.includes(job)) {
        ...
        const { setModelOverride } = await Promise.resolve().then(() => __importStar(require("./core/aiUsageStore.js")));
...
app.delete("/api/harvey/models/route/:job", async (req, res) => {
    ...
    try {
        const { MODEL_JOBS, routingTable } = await Promise.resolve().then(() => __importStar(require("./hull/providers/index.js")));
        if (!MODEL_JOBS.includes(job)) {
            res.status(400).json({ error: `Unknown job "${job}". Jobs are: ${MODEL_JOBS.join(", ")}.` });
            return;
        }
        const { clearModelOverride } = await Promise.resolve().then(() => __importStar(require("./core/aiUsageStore.js")));
...
app.get("/api/harvey/usage", async (req, res) => {
    ...
    try {
        const { usageSummary, recentErrors, getCaps, breakerStates } = await Promise.resolve().then(() => __importStar(require("./core/aiUsageStore.js")));
...
app.post("/api/harvey/usage/caps", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    const patch = {};
    for (const field of ["dailyCapUsd", "monthlyCapUsd"]) {
    ...
    try {
        const { setCaps } = await Promise.resolve().then(() => __importStar(require("./core/aiUsageStore.js")));
...
app.get("/api/harvey/approvals", async (req, res) => {
    ...
    try {
        const { listPendingApprovals } = await Promise.resolve().then(() => __importStar(require("./hull/approval.js")));
 ...
 */
app.post("/api/harvey/approvals/:id/approve", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const id = String(req.params.id || "").trim();
    try {
        const { getApproval, decideApproval } = await Promise.resolve().then(() => __importStar(require("./hull/approval.js")));
        ...
        const { executeHullTool } = await Promise.resolve().then(() => __importStar(require("./hull/tools.js")));
        try {
...
app.post("/api/harvey/approvals/:id/deny", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
app.get("/api/harvey/tasks", async (req, res) => {
    ...
    try {
        const { listTasks } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
        const { cronEnabled } = await Promise.resolve().then(() => __importStar(require("./hull/taskScheduler.js")));
...
app.get("/api/harvey/tasks/preview", async (req, res) => {
    ...
    if (!when) {
        res.status(400).json({ ok: false, error: "Pass ?when= the schedule to read back." });
        return;
    }
    try {
        const { previewSchedule } = await Promise.resolve().then(() => __importStar(require("./hull/scheduleTools.js")));
        const { DEFAULT_TIMEZONE } = await Promise.resolve().then(() => __importStar(require("./hull/cron.js")));
 ...
 */
app.post("/api/harvey/tasks", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (!title || !prompt) {
        res.status(400).json({ error: "title and prompt are both required." });
        return;
    }
    if (!raw) {
        res.status(400).json({ error: "Say when it should run — a cron expression or plain English." });
        return;
    }
    try {
        const { previewSchedule } = await Promise.resolve().then(() => __importStar(require("./hull/scheduleTools.js")));
        ...
        const { createTask } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
        const actor = await currentSessionUser(req);
...
app.patch("/api/harvey/tasks/:id", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    try {
        const { getTask, updateTask } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
        const existing = getTask(id);
        if (!existing) {
            res.status(404).json({ error: "No such scheduled task." });
            return;
        }
        const patch = {};
        ...
        if (["chat", "sms", "email", "none"].includes(String(body.deliver))) {
            patch.deliver = String(body.deliver);
        }
...
app.delete("/api/harvey/tasks/:id", async (req, res) => {
    ...
    const id = String(req.params.id || "").trim();
    try {
        const { deleteTask } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
        if (!deleteTask(id)) {
            res.status(404).json({ error: "No such scheduled task." });
            return;
        }
...
app.post("/api/harvey/tasks/:id/run", async (req, res) => {
    ...
    const id = String(req.params.id || "").trim();
    try {
        const { getTask } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
        if (!getTask(id)) {
            res.status(404).json({ error: "No such scheduled task." });
            return;
        }
        const { runTaskNow } = await Promise.resolve().then(() => __importStar(require("./hull/taskScheduler.js")));
...
app.get("/api/harvey/tasks/:id/runs", async (req, res) => {
    ...
    const id = String(req.params.id || "").trim();
    try {
        const { getTask, listRuns } = await Promise.resolve().then(() => __importStar(require("./core/harveyTaskStore.js")));
 ...
 */
app.post("/api/crm/lead/:id/activity", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (typeof body.activityDate === "string" && body.activityDate.trim()) {
        ...
        if (Number.isNaN(when.getTime())) {
            res.status(400).json({ error: "activityDate is not a real date" });
            return;
        }
    ...
    try {
        const { appendLeadActivity } = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
        const entry = { type: type, description, timestamp };
        if (notes)
            entry.notes = notes;
        if (subType)
        ...
        let pin;
        if (backdated) {
            const { getLeadById, normalizeCrmActivity } = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
...
async function contactLeadOr404(req, res) {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized", hint: "Set DASHBOARD_TOKEN in .env or pass ?token=" });
        return null;
    }
    ...
    try {
        const { seedFromLead } = await Promise.resolve().then(() => __importStar(require("./core/contactRecordStore.js")));
...
async function syncContactPrimaries(leadId) {
...
/* ── emails ── */
app.post("/api/crm/lead/:id/emails", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const body = (req.body || {});
...
app.patch("/api/crm/contact-email/:eid", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
/* ── phones ── */
app.post("/api/crm/lead/:id/phones", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const body = (req.body || {});
...
app.patch("/api/crm/contact-phone/:pid", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    try {
        ...
        if (body.kind !== undefined)
            patch.kind = body.kind;
        if (body.dnc !== undefined)
            patch.dnc = body.dnc === true;
        if (body.isPrimary !== undefined)
            patch.isPrimary = body.isPrimary === true;
...
/* ── addresses ── */
app.post("/api/crm/lead/:id/addresses", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const body = (req.body || {});
...
app.patch("/api/crm/contact-address/:aid", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
/* ── social links ── */
app.put("/api/crm/lead/:id/social", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    const links = (body.links && typeof body.links === "object" ? body.links : body);
    try {
        const store = await Promise.resolve().then(() => __importStar(require("./core/contactRecordStore.js")));
...
/* ── notes ── */
app.post("/api/crm/lead/:id/notes", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const body = (req.body || {});
...
app.patch("/api/crm/contact-note/:nid", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
const contactDocUpload = (0, multer_1.default)({
    storage: multer_1.default.diskStorage({
        destination: (_req, _file, cb) => {
            void Promise.resolve().then(() => __importStar(require("./core/contactRecordStore.js"))).then((m) => cb(null, m.resolveContactDocsDir()))
                .catch((err) => cb(err, ""));
        },
        filename: (_req, file, cb) => {
            const ext = path_1.default.extname(file.originalname || "").slice(0, 12).replace(/[^A-Za-z0-9.]/g, "");
...
app.get("/api/crm/contact-document/:did/file", async (req, res) => {
    ...
    try {
        ...
        if (!doc || !fs_1.default.existsSync(doc.storedPath)) {
            res.status(404).json({ error: "Document not found" });
            return;
        }
        res.setHeader("Content-Type", doc.mime);
        res.setHeader("Content-Disposition", `inline; filename="${doc.fileName.replace(/[^\w. -]/g, "_")}"`);
        fs_1.default.createReadStream(doc.storedPath).pipe(res);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/collaborators", async (req, res) => {
    try {
        const store = await Promise.resolve().then(() => __importStar(require("./core/collaboratorStore.js")));
...
app.post("/api/collaborators", express_1.default.json({ limit: "16kb" }), async (req, res) => {
...
app.post("/api/collaborators/sync-brivity", async (req, res) => {
    ...
    try {
        const { brivityConfigured, getBrivityPeople } = await Promise.resolve().then(() => __importStar(require("./core/brivityPeople.js")));
...
app.put("/api/crm/lead/:id/collaborators", express_1.default.json({ limit: "32kb" }), async (req, res) => {
    const lead = await contactLeadOr404(req, res);
    if (!lead)
        return;
    const b = (req.body && typeof req.body === "object" ? req.body : {});
...
function cmaKpi() {
    try {
        const store = require("./core/cmaStore.js");
        const sessions = store.listSessions({ limit: 500 });
        let withSold = 0;
        for (const s of sessions) {
            if (store.listComparables(s.id, "SOLD").length > 0)
                withSold++;
        }
        return {
            available: true,
            ...
            soldFromFeed: false,
            note: "Sold comparables do not come from the MLS feed — it publishes Active and Pending only. They come from " +
...
function cmaPriceLadder() {
    const out = [];
    for (let v = 50000; v <= 700000; v += 25000)
        out.push(v);
    return out;
}
function cmaBathLadder() {
    const out = [];
    for (let v = 1; v <= 10; v += 0.5)
        out.push(v);
    return out;
}
function cmaSqftLadder() {
    const out = [];
    for (let v = 500; v <= 9500; v += 500)
        out.push(v);
    return out;
}
...
app.get("/api/cma/meta", async (req, res) => {
    ...
    try {
        const { getMlsFacets } = await Promise.resolve().then(() => __importStar(require("./core/mlsFacets.js")));
        const { CANDIDATE_SORTS, UNAVAILABLE_SORT } = await Promise.resolve().then(() => __importStar(require("./core/cmaComps.js")));
            ...
            unavailable: [
                {
                    ...
                    reason: "Listings on this feed carry no latitude or longitude, so a radius around the subject cannot be " +
                ...
                },
                {
                    ...
                    reason: "A map view, geocoded price pins and re-searching against map bounds all need a coordinate on every " +
                ...
                },
                {
                    ...
                    reason: "No Google Maps Platform key is configured for this system. When the subject address matches a " +
                ...
                },
                {
                    ...
                    reason: "No Google Maps Platform key, so there is nothing to autocomplete or geocode against. Type the address " +
 ...
 */
app.post("/api/cma/sessions", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    try {
        const store = await Promise.resolve().then(() => __importStar(require("./core/cmaStore.js")));
        const { cityFromAddress, postalFromAddress, resolveArea } = await Promise.resolve().then(() => __importStar(require("./core/marketReport.js")));
        const b = (req.body || {});
        const clientName = String(b.clientName || "").trim();
        const subjectAddress = String(b.subjectAddress || "").trim();
        if (!clientName) {
            res.status(400).json({ error: "A client name is required — the CMA is prepared for someone." });
            return;
        }
        if (!subjectAddress) {
            res.status(400).json({ error: "A subject property address is required." });
            return;
        }
        ...
        if (!city && !postalCode) {
            res.status(400).json({
                error: "That address does not contain a city or postal code this board covers. There is no geocoder wired " +
        ...
        const session = store.createSession({
            ...
            subjectAddress,
            subjectCity: city,
            subjectState: b.subjectState || "TX",
            subjectPostalCode: postalCode,
            subjectPropertyType: b.subjectPropertyType || "RES",
            subjectBeds: b.subjectBeds,
            subjectBaths: b.subjectBaths,
            subjectSqft: b.subjectSqft,
            subjectLotSize: b.subjectLotSize,
            subjectYearBuilt: b.subjectYearBuilt,
            criteria,
        });
        ...
        const area = resolveArea({ postalCode, city }, { ...criteria, statuses: ["Active"], includeWithoutPhotos: true });
...
app.get("/api/cma/sessions/:id", async (req, res) => {
    ...
    try {
        ...
        const session = store.getSession(String(req.params.id));
        if (!session) {
            res.status(404).json({ error: "That CMA no longer exists." });
            return;
        }
        const comps = store.listComparables(session.id);
        res.json({
...
app.patch("/api/cma/sessions/:id", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
app.get("/api/cma/sessions/:id/candidates", async (req, res) => {
    ...
    try {
        const store = await Promise.resolve().then(() => __importStar(require("./core/cmaStore.js")));
        const { candidateFeed } = await Promise.resolve().then(() => __importStar(require("./core/cmaComps.js")));
        const session = store.getSession(String(req.params.id));
        if (!session) {
            res.status(404).json({ error: "That CMA no longer exists." });
            return;
        ...
        const feed = candidateFeed(session, status, {
            sort: req.query.sort,
        ...
        const selected = new Set(store
            .listComparables(session.id, status)
            .map((c) => c.sourceKey)
            .filter(Boolean));
        res.json({
            ok: true,
...
app.post("/api/cma/sessions/:id/comparables", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    try {
        ...
        const address = String(b.address || "").trim();
        if (!address) {
            res.status(400).json({ error: "A comparable needs an address." });
            return;
        }
        const comp = store.addComparable({
            ...
            mlsNumber: b.mlsNumber || null,
            address,
            city: b.city || null,
            postalCode: b.postalCode || null,
            price: b.price,
            originalListPrice: b.originalListPrice,
            soldPrice: b.soldPrice,
            sellerConcessions: b.sellerConcessions,
            beds: b.beds,
            baths: b.baths,
            sqft: b.sqft,
            lotSize: b.lotSize,
            yearBuilt: b.yearBuilt,
            listDate: b.listDate || null,
            statusDate: b.statusDate || null,
            estimatedClosingDate: b.estimatedClosingDate || null,
            daysOnMarket: b.daysOnMarket,
            offMarketType: b.offMarketType || null,
...
app.patch("/api/cma/comparables/:compId", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    try {
        ...
        if (!updated) {
            res.status(404).json({ error: "That comparable is no longer in this CMA." });
            return;
        }
 ...
 */
app.post("/api/cma/sessions/:id/send", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    try {
        ...
        if (!firstName) {
            res.status(400).json({ error: "A first name is required." });
            return;
        }
        ...
        const { publicBaseUrl, esc } = await Promise.resolve().then(() => __importStar(require("./core/outreachEmail.js")));
        const viewUrl = `${publicBaseUrl()}/c/${encodeURIComponent(session.id)}`;
        const money = (n) => (n == null ? null : "$" + Math.round(n).toLocaleString());
        const range = session.suggestedMinListPrice != null && session.suggestedMaxListPrice != null
        ...
        const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1F2933;line-height:1.6">` +
        ...
        if (wantsDrip) {
            try {
                const db = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
                ...
                dripNote = `The CMA was sent, but the market drip could not be scheduled: ${err.message}`;
            }
        }
        let ok = true;
        let error = null;
        ...
        if (!ok) {
            res.status(502).json({ error: `The CMA could not be emailed: ${error}`, delivery });
            return;
        }
        res.json({
            ok: true,
...
app.post("/api/cma/sessions/:id/publish", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
...
app.get("/api/settings/ui-prefs", async (req, res) => {
    ...
    const { getUserUiPrefs } = await Promise.resolve().then(() => __importStar(require("./core/userPrefs.js")));
    res.json({ ok: true, prefs: getUserUiPrefs(user) });
});
app.put("/api/settings/ui-prefs", express_1.default.json({ limit: "8kb" }), async (req, res) => {
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    ...
    try {
        const { setUserUiPrefs } = await Promise.resolve().then(() => __importStar(require("./core/userPrefs.js")));
 ...
 */
function timelineTitleFor(type, actor, contact, subType) {
    ...
    const who = actor || "";
    const did = (verb, rest = "") => (actor ? `${who} ${verb}${rest}` : `${verb.replace(/^(\w+)/, (m) => m.charAt(0).toUpperCase() + m.slice(1))}${rest}`);
    switch (type) {
...
function timelineCategoryFor(type) {
    if (type === "note")
        return "note";
    if (type === "email_sent" || type === "email_logged" || type === "email_pending")
        return "email";
    if (type === "call" || type === "call_made")
        return "call";
    if (type === "text_sent" || type === "text_received" || type === "text_logged")
        return "text";
    if (type === "appointment")
        return "appointment";
    if (type === "task")
        return "task";
    if (type === "web_visit" || type === "home_hearted" || type === "home_clicked")
        return "web";
    return "other";
}
app.get("/api/crm/lead/:id/timeline", async (req, res) => {
    ...
    try {
        ...
        activity.forEach((a, i) => {
            ...
            if (a.subType) {
                for (const key of Object.keys(detail)) {
                    if (String(detail[key]) === a.subType)
                        delete detail[key];
                }
            }
            if (a.notes)
        ...
        try {
            const thread = (0, smsStore_js_1.getThreadForLead)(id);
            (thread || []).forEach((m, i) => {
        ...
        try {
            const outreach = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
            const subs = [
                ...outreach.listAlerts(id).map((a) => ({ kind: "alert", id: a.id, name: a.name, address: "", frequency: a.frequency })),
                ...outreach.listReports(id).map((r) => ({ kind: "report", id: r.id, name: r.name, address: r.address, frequency: r.frequency })),
            ];
            for (const sub of subs) {
                for (const send of outreach.listSends(sub.id, 40)) {
                    ...
                    if (sub.address)
                        detail.Location = sub.address;
                    if (sub.frequency)
                        detail["Scheduled Drip"] = String(sub.frequency);
                    if (send.listingCount)
                        detail.Listings = send.listingCount;
        ...
        try {
            const { filterTasks } = await Promise.resolve().then(() => __importStar(require("./core/tasks.js")));
            for (const t of filterTasks({ leadId: id })) {
                const detail = {};
                if (t.dueDate)
                    detail.Due = t.dueDate + (t.dueTime ? " " + t.dueTime : "");
                if (t.assignedUserName)
                    detail["Assigned To"] = t.assignedUserName;
                if (t.priority)
        ...
        const counts = { all: items.length, note: 0, email: 0, call: 0, text: 0, appointment: 0, task: 0, other: 0, web: 0 };
 ...
 */
app.post("/api/crm/lead/:id/send-email", express_1.default.json({ limit: "2mb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const cc = (Array.isArray(body.cc) ? body.cc : [])
        ...
        .slice(0, 10);
    try {
        const { getLeadById, appendLeadActivity } = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
        const lead = await getLeadById(id);
        if (!lead) {
            res.status(404).json({ error: "Lead not found" });
            return;
        ...
        if (body.copyMe === true) {
            ...
            if (self && /\S+@\S+\.\S+/.test(self) && !recipients.includes(self))
        ...
        const meta = { To: to, Subject: subject };
        if (cc.length)
            meta.CC = cc.join(", ");
        ...
        res.status(502).json({ error: err.message || "Gmail rejected the send" });
    }
});
...
app.get("/api/crm/messaging-status", async (req, res) => {
    ...
    try {
        const quo = await Promise.resolve().then(() => __importStar(require("./integrations/quo/index.js")));
 ...
 */
app.post("/api/website/lead", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    const { websiteSecretConfigured, websiteSecretOk, mapWebsitePayload, websitePayloadIsUsable, websiteLeadPatch, } = await Promise.resolve().then(() => __importStar(require("./core/websiteWebhook.js")));
        ...
        for (const raw of items) {
            ...
            try {
                if (hit) {
                    ...
                    if (patch.crmNotes && !hit.crmNotes)
                        next.crmNotes = patch.crmNotes;
                    upsertLeadQuiet(next);
                    merged++;
                }
                else {
...
app.post("/api/zernio/webhook", express_1.default.raw({ type: "application/json", limit: "1mb" }), async (req, res) => {
    ...
    if (!zernioWebhookSecretConfigured()) {
        ...
        res.status(503).json({
            ok: false,
            error: "ZERNIO_WEBHOOK_SECRET is not set on this server, so the Zernio webhook is closed. " +
    ...
    if (!verifyZernioSignature(raw, signature)) {
        (0, marcoLog_js_1.marcoLog)("inbound_rejected", { reason: "zernio_bad_signature" });
    ...
    if ((0, conversationUtils_js_1.isDuplicateHandle)(evt.eventId)) {
        (0, marcoLog_js_1.marcoLog)("inbound_rejected", { reason: "zernio_duplicate_event", message_handle: evt.eventId });
        res.status(200).json({ ok: true, duplicate: true });
        return;
    }
    const requestId = (0, marcoLog_js_1.newMarcoRequestId)();
    const correlationId = (0, marcoLog_js_1.marcoCorrelationId)(evt.platform, evt.senderId);
    (0, marcoLog_js_1.marcoLog)("inbound_accepted", {
        ...
        message_preview: (0, marcoLog_js_1.previewText)(evt.text),
    ...
    void (async () => {
        try {
            ...
            if (attributed > 0) {
                (0, marcoLog_js_1.marcoLog)("comment_to_dm_converted", {
                    requestId,
                    correlationId,
            ...
            const result = (0, messageDebounce_js_1.shouldSkipMessageDebounce)(payload)
                ? await (0, webhook_js_1.handleIncomingPayload)(payload, { requestId, correlationId })
                : await (0, messageDebounce_js_1.scheduleDebouncedInbound)(payload, { requestId, correlationId }, (p, log) => (0, webhook_js_1.handleIncomingPayload)(p, log));
            ...
            (0, marcoLog_js_1.marcoLog)("zernio_reply_send", {
                requestId,
                correlationId,
            ...
            if (!send.success) {
                /* Most likely cause by far: TikTok's 48-hour / 10-message reply
                   ...
                   the agent. */
                console.error(`[zernio] reply NOT delivered to conversation ${evt.conversationId} ` +
                    `(HTTP ${send.status}): ${send.error ?? "unknown"}`);
            }
        }
        catch (err) {
            console.error("[zernio] inbound processing failed:", err);
            (0, marcoLog_js_1.marcoLog)("zernio_inbound_failed", {
                requestId,
                correlationId,
...
app.get("/api/zernio/status", async (req, res) => {
    ...
    const { isZernioDmConfigured, zernioWebhookSecretConfigured, zernioAccounts, zernioTikTokAccountId } = await Promise.resolve().then(() => __importStar(require("./integrations/zernio/dm.js")));
...
app.get("/api/comment-agent/status", async (req, res) => {
    ...
    const { getCommentAgentStats, getRecentCommentActions } = await Promise.resolve().then(() => __importStar(require("./core/commentAgentStore.js")));
    const { isCommentAgentEnabled } = await Promise.resolve().then(() => __importStar(require("./agents/commentAgent/index.js")));
 ...
 */
app.post("/api/comment-agent/dry-run", express_1.default.json({ limit: "32kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const { classifyAndDraft, vetCommentReply } = await Promise.resolve().then(() => __importStar(require("./agents/commentAgent/index.js")));
    const results = [];
...
app.get("/api/comment-agent/follow-ups", async (req, res) => {
    ...
    const { getFollowUpQueue } = await Promise.resolve().then(() => __importStar(require("./core/commentAgentStore.js")));
    ...
    res.json({
        ...
        count: rows.length,
        note: "TikTok does not allow a business to open a DM thread, so these must be messaged by hand " +
...
app.get("/api/ads/campaigns", async (req, res) => {
    ...
    const { getAdCampaigns } = await Promise.resolve().then(() => __importStar(require("./core/adsStore.js")));
    res.json({ campaigns: getAdCampaigns() });
});
app.post("/api/ads/campaigns", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const objective = b.objective === "traffic" || b.objective === "awareness" || b.objective === "engagement" || b.objective === "messages"
        ? b.objective
        : "leads";
    const { createAdCampaign } = await Promise.resolve().then(() => __importStar(require("./core/adsStore.js")));
    const actor = await currentSessionUser(req);
...
app.patch("/api/ads/campaigns/:id", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (typeof b.name === "string")
        updates.name = b.name;
    if (b.platform === "facebook" || b.platform === "instagram" || b.platform === "both")
        updates.platform = b.platform;
    if (["leads", "traffic", "awareness", "engagement", "messages"].includes(String(b.objective)))
        updates.objective = b.objective;
    if (["draft", "pending_review", "active", "paused", "completed"].includes(String(b.status)))
        updates.status = b.status;
    if (b.dailyBudget !== undefined)
        updates.dailyBudget = Number(b.dailyBudget) || 0;
    if (b.totalBudget !== undefined)
        updates.totalBudget = Number(b.totalBudget) || 0;
    ...
    if (typeof b.audience === "string")
        updates.audience = b.audience;
    if (typeof b.creativeNotes === "string")
        updates.creativeNotes = b.creativeNotes;
    const { updateAdCampaign } = await Promise.resolve().then(() => __importStar(require("./core/adsStore.js")));
    const updated = updateAdCampaign(id, updates);
    if (!updated) {
...
app.delete("/api/ads/campaigns/:id", async (req, res) => {
    ...
    const id = String(req.params.id || "").trim();
    const { getAdCampaignById, deleteAdCampaign } = await Promise.resolve().then(() => __importStar(require("./core/adsStore.js")));
...
app.get("/api/ads/boost-candidates", async (req, res) => {
    ...
    const { getSocialVideos } = await Promise.resolve().then(() => __importStar(require("./core/socialStore.js")));
...
app.get("/ads", requireAuthPage, (_req, res) => {
    res.sendFile(path_1.default.join(publicDir, "ads.html"));
});
...
app.get("/api/email/connection-status", async (_req, res) => {
    ...
    const { checkGmailAuth, getEmailTransport } = await Promise.resolve().then(() => __importStar(require("./integrations/gmail/index.js")));
    const { getSmtpStatus, verifySmtpConnection } = await Promise.resolve().then(() => __importStar(require("./integrations/smtp/index.js")));
 ...
 */
app.post("/api/email/smtp-test", express_1.default.json(), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    const { getSmtpConfig, getSmtpStatus, sendViaSmtp, verifySmtpConnection } = await Promise.resolve().then(() => __importStar(require("./integrations/smtp/index.js")));
    ...
    try {
        const sent = await sendViaSmtp({
            ...
            body: "<p>This is a test from the Marco 90 automation system.</p>" +
        ...
        res.status(502).json({ ok: false, stage: "send", error: err.message });
    }
});
...
app.post("/api/jarvis/voice", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    ...
    try {
        ...
        if (!audio) {
            ...
            const why = (0, tts_js_1.ttsHealthReport)().lastError;
            res.status(502).json({
...
app.get("/api/harvey/voice", async (req, res) => {
    ...
    const leadName = (n) => n.split(/\s+[-–—]\s+/)[0].trim().toLowerCase();
...
app.put("/api/harvey/voice", express_1.default.json({ limit: "32kb" }), async (req, res) => {
    ...
    const { setVoiceProfile, effectiveDelivery, voiceExistsOnAccount } = await Promise.resolve().then(() => __importStar(require("./hull/voice/voiceProfile.js")));
...
app.get("/api/dm/inbound-report", async (req, res) => {
    ...
    try {
        ...
        const { listAllLeads: allLeads, getConversation: getConv } = await Promise.resolve().then(() => __importStar(require("./core/db.js")));
        ...
        for (const lead of leads) {
            ...
            for (const m of conv.messages || []) {
                ...
                if (!s.lastInboundAt || m.at > s.lastInboundAt)
                    s.lastInboundAt = m.at;
                if (now - t <= 86400000)
                    s.inbound24h++;
                if (now - t <= 7 * 86400000) {
                    s.inbound7d++;
                    s.contacts7d.add(lead.id);
                }
                ...
                if (t >= cutoff) {
                    ...
                    if (!s.firstInboundInWindow || m.at < s.firstInboundInWindow)
                        s.firstInboundInWindow = m.at;
                    sawInbound = true;
                }
            }
            if (sawInbound)
                touch(platform).contactsWithInbound++;
        }
        /*
        ...
        function gapsOf(s) {
            ...
            let longestGapDays = 0, gapStart = null, gapEnd = null;
...
app.get("/api/dm/stats", async (req, res) => {
    ...
    try {
        ...
        for (const lead of leads) {
            ...
            if (touchedToday)
                activeToday++;
            if (String(lead.createdAt || "").slice(0, 10) === today)
                newLeadsToday++;
            if (lead.phone?.trim())
                phonesCaptured++;
            if (String(lead.state) === "flag_for_call")
                flaggedForCall++;
...
function streamClipVideoFile(req, res, filePath) {
    ...
    const pipeWithGuard = (opts) => {
        const stream = opts ? fs_1.default.createReadStream(filePath, opts) : fs_1.default.createReadStream(filePath);
        stream.on("error", (err) => {
            console.error(`[clip-video] stream error for ${filePath}: ${err.message}`);
            res.destroy();
        });
        stream.pipe(res);
    };
...
const scriptTextUpload = (0, multer_1.default)({
    storage: multer_1.default.diskStorage({
        destination: (_req, _file, cb) => cb(null, resolveContentVideoUploadDir()),
        filename: (_req, file, cb) => {
            const ext = path_1.default.extname(file.originalname).toLowerCase() || ".txt";
            cb(null, `script-${Date.now()}-${(0, crypto_1.randomUUID)()}${ext}`);
        },
    }),
...
const clipVideoReplaceUpload = (0, multer_1.default)({
    storage: multer_1.default.diskStorage({
        destination: (_req, _file, cb) => cb(null, resolveContentVideoUploadDir()),
        filename: (_req, file, cb) => {
            const ext = path_1.default.extname(file.originalname).toLowerCase() || ".mp4";
            cb(null, `freecut-${Date.now()}-${(0, crypto_1.randomUUID)()}${ext}`);
        },
    }),
...
function parseAiEditInstruction(instruction, duration) {
    ...
    const toSeconds = (value, unit) => {
        ...
        if (base == null)
            return null;
        if (unit && /^m/.test(unit) && !value.includes(":"))
            return base * 60;
        return base;
    };
    ...
    if (m) {
        ...
        if (t != null)
            return { start: t, end: duration, description: `Removed the first ${Math.round(t)}s` };
    }
    m = s.match(new RegExp(String.raw `(?:remove|trim|cut)\s+(?:the\s+)?last\s+${T}`));
    if (m) {
        const t = toSeconds(m[1], m[2]);
        if (t != null)
            return { start: 0, end: duration - t, description: `Removed the last ${Math.round(t)}s` };
    }
    m = s.match(new RegExp(String.raw `keep\s+(?:only\s+)?(?:the\s+)?first\s+${T}`));
    if (m) {
        const t = toSeconds(m[1], m[2]);
        if (t != null)
            return { start: 0, end: t, description: `Kept only the first ${Math.round(t)}s` };
    }
    m = s.match(new RegExp(String.raw `(?:cut|remove)\s+everything\s+after\s+${T}`));
    if (m) {
        const t = toSeconds(m[1], m[2]);
        if (t != null)
            return { start: 0, end: t, description: `Cut everything after ${Math.round(t)}s` };
    }
    m = s.match(new RegExp(String.raw `(?:start(?:ing)?\s+(?:at|from)|from)\s+${T}`));
    if (m) {
        const t = toSeconds(m[1], m[2]);
        if (t != null)
            return { start: t, end: duration, description: `Started the clip at ${Math.round(t)}s` };
    }
    return null;
}
...
async function parseAiEditWithClaude(instruction, duration) {
...
- "none": the instruction cannot be resolved to concrete timestamps from the wording alone (e.g. it refers to spoken words or visual events you cannot locate). When in doubt, use "none" — never guess.`;
    try {
        ...
        if (parsed.action !== "trim" && parsed.action !== "cut_middle")
            return null;
...
app.get("/api/crm/vocabulary", (req, res) => {
    ...
    res.json({
        ...
        sources: (0, crmVocabulary_js_1.listVocabulary)("source"),
        tags: (0, crmVocabulary_js_1.listVocabulary)("tag"),
        vocabularyStats: (0, crmVocabulary_js_1.vocabularyStats)(),
    });
});
/** Add a custom source or tag to the managed list. */
app.post("/api/crm/vocabulary/:kind", express_1.default.json(), (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    if (!kind) {
        res.status(400).json({ error: "kind must be sources or tags" });
        return;
    }
    const name = String((req.body || {}).name || "").trim();
    if (!name) {
    ...
    if (name.length > 80) {
        res.status(400).json({ error: "Keep it under 80 characters" });
        return;
    }
    const added = (0, crmVocabulary_js_1.addVocabulary)(kind, name, sessionUserSync(req)?.email || undefined);
    if (!added) {
        res.status(409).json({ error: `"${name}" is already on the list` });
        return;
    }
    res.json({ ok: true, name: added, list: (0, crmVocabulary_js_1.listVocabulary)(kind) });
});
...
app.delete("/api/crm/vocabulary/:kind/:name", (req, res) => {
    ...
    if (!(0, crmVocabulary_js_1.removeVocabulary)(kind, name)) {
        res.status(400).json({
    ...
    res.json({ ok: true, list: (0, crmVocabulary_js_1.listVocabulary)(kind) });
});
app.get("/api/crm/lead-metrics", async (req, res) => {
    ...
    const metrics = {};
    const touch = (id) => (metrics[id] = metrics[id] || {});
    try {
        const { getLatestScoresForAllLeads } = await Promise.resolve().then(() => __importStar(require("./core/leadScoreStore.js")));
    ...
    try {
        const { favoriteSummaryByLead } = await Promise.resolve().then(() => __importStar(require("./core/favoritesStore.js")));
    ...
    try {
        const { outreachSummaryByLead } = await Promise.resolve().then(() => __importStar(require("./core/outreachStore.js")));
...
app.get("/api/users", (req, res) => {
    ...
    res.status(200).json({ users: (0, users_js_1.getUsers)().map(({ passwordHash, ...safe }) => { void passwordHash; return safe; }) });
});
...
app.patch("/api/users/:id", express_1.default.json(), (req, res) => {
    ...
    void currentSessionUser(req).then((actor) => {
        void Promise.resolve().then(() => __importStar(require("./core/authStore.js"))).then(({ recordAudit }) => {
...
app.delete("/api/users/:id", (req, res) => {
    ...
    const id = String(req.params.id || "").trim();
    const target = (0, users_js_1.getUserById)(id);
...
app.get("/api/auto-plans/meta", async (req, res) => {
    ...
    res.json({
        ok: true,
        mergeFields: mergeFields_js_1.MERGE_FIELDS,
        roles: autoPlanScheduling_js_1.AUTO_PLAN_ROLES,
        team: (0, teamRoster_js_1.listTeamMembers)().map((m) => ({ id: m.id, name: m.name, role: m.role })),
        templates,
        offsetUnits: ["minutes", "hours", "days"],
        recurrences: ["never", "daily", "weekly", "monthly", "yearly"],
        maxRecurringRuns: autoPlanScheduling_js_1.MAX_RECURRING_RUNS,
    });
});
/**
 ...
 */
app.post("/api/auto-plans/draft-step", express_1.default.json({ limit: "32kb" }), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    try {
        ...
        const { HARVEY_HOUSE_STYLE, stripAiTypography } = await Promise.resolve().then(() => __importStar(require("./core/houseStyle.js")));
        const limits = kind === "text"
        ...
        const placeholders = mergeFields_js_1.MERGE_FIELDS.map((f) => `{{${f.key}}}`).join(", ");
        const prompt = [
...
async function executeDueAutoPlanSteps() {
    /* Dynamic, matching how every other caller reaches emailStore — it opens a
       ...
       has needed it. */
    const { getEmailTemplate } = await Promise.resolve().then(() => __importStar(require("./core/emailStore.js")));
    const plans = (0, autoPlans_js_1.getAutoPlans)();
    ...
    for (const lead of leads) {
        ...
        for (const enr of enrollments) {
            ...
            const lastRunAt = { ...(enr.lastRunAt || {}) };
            const runCount = { ...(enr.runCount || {}) };
            for (const step of plan.steps) {
                const repeatMs = (0, autoPlanScheduling_js_1.recurrenceMs)(step.recurrence);
                ...
                if (step.anchor === "birthday" || step.anchor === "home_anniversary") {
                    ...
                    const specific = (0, autoPlanScheduling_js_1.specificDateDueMs)(step, lead, enrolledMs);
                    if (specific === null)
                        continue; // no such date on this contact
                    dueMs = specific;
                }
                else {
                    ...
                    dueMs = baseMs + (0, autoPlanScheduling_js_1.offsetMs)(step);
                }
                if (isRecurring) {
                    /* A repeating step is never "done": it re-arms off its own last run
                       until the run cap stops it. */
                    const runs = runCount[step.id] || 0;
                    if (runs >= autoPlanScheduling_js_1.MAX_RECURRING_RUNS) {
                        completed.add(step.id);
                        completedAt[step.id] = new Date().toISOString();
                        changed = true;
                        newActivity.push({
                            type: "auto_plan",
                            description: `Auto Plan step stopped repeating after ${autoPlanScheduling_js_1.MAX_RECURRING_RUNS} runs: ${(step.content || "").slice(0, 80)}`,
                            timestamp: new Date().toISOString(),
                        });
                        continue;
                    }
                    ...
                    if (last)
                        dueMs = new Date(last).getTime() + repeatMs;
                }
                if (dueMs > now)
                    continue;
                ...
                const merged = (0, mergeFields_js_1.applyMergeFields)(step.content || "", lead, { name: "Marco Puga", phone: process.env.HARVEY_OWNER_NUMBER });
                const content = merged.text;
                if (step.type === "text") {
                    if (!(0, mergeFields_js_1.isSendable)(merged)) {
                        ...
                        (0, tasks_js_1.createTask)({
                            title: `Auto Plan text held back — ${(0, mergeFields_js_1.describeMergeProblem)(merged)}`,
                            ...
                            assignedUserName: (0, autoPlanScheduling_js_1.resolveSender)(step.sendFrom, lead).name,
                            source: "auto_plan",
                        });
                        newActivity.push({ type: "auto_plan", description: `Auto Plan text HELD BACK — ${(0, mergeFields_js_1.describeMergeProblem)(merged)} (task created)`, timestamp: stamp });
                        completed.add(step.id);
                    ...
                    if (!lead.phone) {
                        ...
                        const sender = (0, autoPlanScheduling_js_1.resolveSender)(step.sendFrom, lead);
                    ...
                    const mergedBody = (0, mergeFields_js_1.applyMergeFields)(bodySource, lead, { name: "Marco Puga", phone: process.env.HARVEY_OWNER_NUMBER });
                    const mergedSubject = (0, mergeFields_js_1.applyMergeFields)(step.subject || tpl?.subject || "", lead, { name: "Marco Puga" });
                    const sender = (0, autoPlanScheduling_js_1.resolveSender)(step.sendFrom, lead);
                    const recipients = [
                        ...(step.cc || []).map((a) => `cc ${a}`),
                    ...
                    if (!(0, mergeFields_js_1.isSendable)(mergedBody) || !(0, mergeFields_js_1.isSendable)(mergedSubject)) {
                        const why = (0, mergeFields_js_1.describeMergeProblem)((0, mergeFields_js_1.isSendable)(mergedBody) ? mergedSubject : mergedBody);
                        (0, tasks_js_1.createTask)({
                        ...
                        const { sendEmail: sendRealEmail } = await Promise.resolve().then(() => __importStar(require("./integrations/gmail/index.js")));
                        try {
                    ...
                    const assignee = (0, autoPlanScheduling_js_1.resolveSender)(step.assignedTo, lead);
                    ...
                    const priority = pr === undefined ? "normal" : pr <= 2 ? "urgent" : pr <= 4 ? "high" : pr <= 6 ? "normal" : "low";
                    const body = [
                    ...
                    if (step.notes) {
                        newActivity.push({
                            type: "note",
                            description: `Auto Plan note (with task "${content.slice(0, 60)}"): ${(0, mergeFields_js_1.applyMergeFields)(step.notes, lead).text}`,
                            timestamp: stamp,
                        });
                    }
                }
    ...
    for (const tx of (0, transactionsStore_js_1.getAllTransactions)()) {
        ...
        for (const enr of enrollments) {
            ...
            for (const step of plan.steps) {
                ...
                const dueMs = new Date(baseIso).getTime() + (0, autoPlanScheduling_js_1.offsetMs)(step);
 ...
 */
app.post("/api/tracker/records/bulk-stage", express_1.default.json({ limit: "64kb" }), (req, res) => {
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    const ids = Array.isArray(body.ids)
        ? body.ids.filter((x) => typeof x === "string").slice(0, 500)
        : [];
    const side = body.side === "seller" ? "seller" : "buyer";
    const stage = body.stage === null ? null : String(body.stage || "");
    if (!ids.length) {
    ...
    const notFound = [];
    for (const id of ids) {
        const rec = (0, trackerStore_js_1.setTrackerStage)(id, side, stage);
        if (rec)
            moved.push(id);
        else
            notFound.push(id);
 ...
 */
app.post("/api/tracker/stage-from-engagement", express_1.default.json({ limit: "16kb" }), async (req, res) => {
    const body = (req.body && typeof req.body === "object" ? req.body : {});
    const apply = body.apply === true;
    try {
        const { stageTrackerFromEngagement } = await Promise.resolve().then(() => __importStar(require("./core/trackerEngagement.js")));
        const leads = await (0, db_js_1.listAllLeads)();
...
app.post("/api/leads/:id/suggest-reply", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    ...
    const b = (req.body || {});
    try {
        const out = await (0, replySuggest_js_1.suggestReply)(lead, {
...
app.post("/api/leads/:id/complete-reply", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    const lead = await (0, db_js_1.getLeadById)(String(req.params.id || "").trim());
    if (!lead) {
        res.status(404).json({ error: "Lead not found" });
        return;
    ...
    try {
        const completion = await (0, replySuggest_js_1.completeReply)(lead, draft, {
            channel: b.channel === "email" ? "email" : "sms",
        });
        res.json({ completion });
    }
...
app.post("/api/browser/poll", express_1.default.json({ limit: "64kb" }), async (req, res) => {
    ...
    const account = (0, browserControl_js_1.accountForToken)(String(b.token || ""));
    if (!account) {
...
app.post("/api/browser/result", express_1.default.json({ limit: "6mb" }), (req, res) => {
    const b = (req.body || {});
 ...
 */
app.post("/api/browser/command", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    const token = String(req.get("X-Browser-Token") || req.query.token || "");
    const cmdAccount = (0, browserControl_js_1.accountForToken)(token);
    if (!cmdAccount) {
        res.status(401).json({ error: "Bad or missing pairing token" });
        return;
    }
    ...
    try {
        const result = await (0, browserControl_js_1.run)(b.command, {
 ...
 */
app.post("/api/browser/chat", express_1.default.json({ limit: "256kb" }), async (req, res) => {
    const token = String(req.get("X-Browser-Token") || req.query.token || "");
    const account = (0, browserControl_js_1.accountForToken)(token);
    if (!account) {
...
app.get("/api/mls/listing/:key", async (req, res) => {
    ...
    const prop = (p.property && typeof p.property === "object" ? p.property : {});
    const geo = (p.geo && typeof p.geo === "object" ? p.geo : {});
    const school = (p.school && typeof p.school === "object" ? p.school : {});
    ...
    res.json({
        ...
        detail: {
            ...
            area: s(prop.area),
            acres: n(prop.acres),
            taxes: n(p.tax && typeof p.tax === "object" ? p.tax.taxAnnualAmount : null),
            taxYear: n(p.tax && typeof p.tax === "object" ? p.tax.taxYear : null),
            ...
            daysOnMarket: n(p.mls && typeof p.mls === "object" ? p.mls.daysOnMarket : null),
        },
    });
});
/**
...
app.get("/api/browser/status", (req, res) => {
    ...
    res.json({
        ...(0, browserControl_js_1.status)(account.id),
        account: { id: account.id, name: account.name },
        recent: (0, browserControl_js_1.recentActivity)(10),
    });
});
/**
 ...
 */
const extensionDir = path_1.default.join(publicDir, "extension");
function collectExtensionFiles(dir, prefix = "") {
    ...
    for (const entry of fs_1.default.readdirSync(dir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
        ...
        if (entry.isDirectory())
            out.push(...collectExtensionFiles(full, rel));
        else if (entry.isFile())
            out.push({ name: rel, data: fs_1.default.readFileSync(full) });
    }
    return out;
}
app.get("/api/browser/extension.zip", (_req, res) => {
    try {
        ...
        if (!files.length)
            throw new Error("extension folder is empty");
        ...
        const zip = (0, zipWriter_js_1.buildZip)(files);
        ...
        res.status(500).json({ error: "Could not build the extension download: " + e.message });
    }
});
/**
...
app.delete("/api/deals/:id", async (req, res) => {
    ...
    const ok = (0, deals_js_1.deleteDeal)(dealId);
    if (!ok) {
        res.status(404).json({ error: "Deal not found" });
        return;
    }
 ...
 */
app.post("/api/transactions/import-csv", express_1.default.text({ type: ["text/csv", "text/plain"], limit: "8mb" }), express_1.default.json({ limit: "8mb" }), (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized", hint: "Set DASHBOARD_TOKEN or pass ?token=" });
        return;
    }
    const body = req.body;
    const asObject = body && typeof body === "object" ? body : {};
    ...
    try {
        const plan = (0, transactionImport_js_1.planTransactionImport)(csv);
        if (plan.errors.length && !plan.rows.length) {
...
const referenceAudioUpload = (0, multer_1.default)({
    storage: multer_1.default.diskStorage({
        destination: (_req, _file, cb) => {
            const dir = path_1.default.join(voiceCloneDataRoot, "reference");
            fs_1.default.mkdirSync(dir, { recursive: true });
            cb(null, dir);
        },
        filename: (_req, file, cb) => {
...
app.post("/api/voice-clone/reference-clips/upload", referenceAudioUpload.single("audio"), async (req, res) => {
    if (!dashboardTokenOk(req)) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    }
    ...
    const makePrimary = (0, voiceCloneStore_js_1.getAllReferenceClips)().length === 0; // first upload becomes primary
    const clip = (0, voiceCloneStore_js_1.createReferenceClip)({
    ...
    if (makePrimary && clip.id)
        (0, voiceCloneStore_js_1.setPrimaryReferenceClip)(clip.id);
...
app.post("/api/voice-clone/reference-clips/:id/set-primary", async (req, res) => {
    ...
    let cloneError;
    const clip = (0, voiceCloneStore_js_1.getReferenceClipById)(req.params.id);
    if (clip?.elevenVoiceId) {
        ...
        if (clone.success && clone.voiceId) {
            (0, voiceCloneStore_js_1.setReferenceClipVoiceId)(req.params.id, clone.voiceId);
            voiceReady = true;
        }
        else {
            cloneError = clone.error;
   ...
   src/core/initialAdmin.ts for why that distinction matters. */
{
    ...
    try {
        ...
        if (r.ran) {
            console.log(`[security] Site lock armed. ${r.sessionsRevoked} session(s) revoked; ` +
...
httpServer.listen(PORT, "0.0.0.0", () => {
    ...
    void (async () => {
        try {
            const { startTaskScheduler } = await Promise.resolve().then(() => __importStar(require("./hull/taskScheduler.js")));
    ...
    try {
        ...
        void (async () => {
            ...
            const { scheduleOutreach } = await Promise.resolve().then(() => __importStar(require("./core/outreachRunner.js")));
            scheduleOutreach(60);
            ...
            if (isQuoConfigured()) {
                try {
                    ...
                    console.error("[Quo] webhook registration failed (poll still covers it):", err.message);
                }
            }
        })();
    }
