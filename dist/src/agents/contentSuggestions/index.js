...
exports.fetchCurrentContentTrends = fetchCurrentContentTrends;
exports.generateWeeklyContentSuggestions = generateWeeklyContentSuggestions;
exports.getLatestContentSuggestions = getLatestContentSuggestions;
exports.scheduleWeeklyContentSuggestionsMonday8am = scheduleWeeklyContentSuggestionsMonday8am;
...
function migrateContentSuggestionsTable(db) {
    try {
    ...
    try {
        const response = await anthropic.messages.create({
            ...
            messages: [
                {
                    role: "user",
                    content: "Search for current TikTok content trends for real estate agents — trending formats, sounds, hooks, or challenges that are working right now for real estate content creators. Focus on the last 1-2 weeks.\n\nReturn a brief 3-4 sentence summary of what's trending and how a real estate agent could apply it.",
                },
            ],
        });
...
${topVideos
        .map((v, i) => `${i + 1}. "${v.description.substring(0, 100)}" — Score ${v.score}/100, ${v.views.toLocaleString()} views`)
        .join("\n")}
...
function saveContentSuggestions(result) {
  ...
  `);
    ...
    db.prepare(`INSERT INTO content_suggestions (generated_at, week_of, suggestions, based_on, current_trends_summary)
     VALUES (?, ?, ?, ?, ?)`).run(result.generatedAt, result.weekOf, JSON.stringify(result.suggestions), result.basedOn, result.currentTrendsSummary);
}
function getLatestContentSuggestions() {
    const db = (0, socialStore_js_1.getSocialDb)();
    try {
        migrateContentSuggestionsTable(db);
        const row = db
        ...
        if (!row)
            return null;
        const rawSuggestions = JSON.parse(String(row.suggestions));
        return {
...
let lastScheduledContentSuggestionsDate = null;
function scheduleWeeklyContentSuggestionsMonday8am() {
    const checkAndRun = () => {
        const now = new Date();
        ...
        if (centralDay === "Monday" &&
            ...
            lastScheduledContentSuggestionsDate = dateStr;
            generateWeeklyContentSuggestions().catch((err) => console.error("[ContentSuggestions] scheduled run failed:", err));
        }
    };
