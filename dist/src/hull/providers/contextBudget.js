...
exports.estimateToolTokens = estimateToolTokens;
...
function blockTokens(block) {
    ...
    switch (block.type) {
        ...
        case "tool_use": {
            ...
            return estimateTokens(block.name) + estimateTokens(json) + 8;
        }
        case "tool_result": {
            const content = block.content;
            if (typeof content === "string")
                return estimateTokens(content) + 8;
            if (Array.isArray(content)) {
                return content.reduce((n, b) => n + blockTokens(b), 0) + 8;
            }
            return 8;
        }
        default: {
...
function isToolResultBlock(b) {
    return Boolean(b) && b.type === "tool_result";
}
...
function compactToolResult(block) {
    const tr = block;
...
function planContext(input) {
    const source = (Array.isArray(input.messages) ? input.messages : []).filter((m) => m && typeof m === "object");
    const budgetTokens = budgetFor(input.job, input.model);
    const fixed = estimateTokens(input.system) + estimateToolTokens(input.tools) + MESSAGE_OVERHEAD_TOKENS;
    const measure = (list) => list.reduce((n, m) => n + estimateMessageTokens(m), fixed);
    ...
    if (measure(working) > budgetTokens) {
        ...
        const keepRaw = Math.max(0, Math.round(envNumber("HARVEY_RAW_TOOL_RESULTS", DEFAULT_RAW_TOOL_RESULTS)));
        ...
        working = working.map((m) => {
            ...
            const content = msg.content.map((b) => {
                if (!isToolResultBlock(b))
                    return b;
                const index = seen++;
                if (index >= compactBefore)
                    return b;
                const size = blockTokens(b) * CHARS_PER_TOKEN;
                if (size < MIN_COMPACTABLE_CHARS)
                    return b;
                changed = true;
    ...
    if (droppedTurns > 0 && working.length) {
        ...
        if (first.role === "user") {
            ...
            working = [{ role: "user", content: note }, ...working];
        }
