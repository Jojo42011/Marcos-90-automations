...
export function buildSignedClipUrl(videoId: string, ttlSeconds = 7200): string {
  const base = publicBaseUrl();
  if (!base) {
    throw new Error("PUBLIC_BASE_URL is not set — Instagram/Facebook need a public video URL to fetch.");
  }
  const key = signingKey();
  if (!key) throw new Error("No clip-URL signing key — set DASHBOARD_TOKEN.");
  const exp = Math.floor(Date.now() / 1000) + ttlSeconds;
  const sig = crypto.createHmac("sha256", key).update(`${videoId}.${exp}`).digest("hex");
...
export function verifySignedClip(videoId: string, exp: string, sig: string): boolean {
  const key = signingKey();
  if (!key || !exp || !sig) return false;
  const expNum = Number(exp);
  if (!Number.isFinite(expNum) || expNum < Math.floor(Date.now() / 1000)) return false;
  const expected = crypto.createHmac("sha256", key).update(`${videoId}.${expNum}`).digest("hex");
  try {
...
async function graphRequest(
  ...
  if (!res.ok || err) {
    throw new Error(
      `Meta Graph ${method} ${path} failed (HTTP ${res.status}): ${err?.message || JSON.stringify(data).slice(0, 300)}`,
    );
  }
  return data;
}
...
export async function postReelToInstagram(videoUrl: string, caption: string): Promise<{ mediaId: string }> {
  ...
  if (!creationId) throw new Error(`Instagram container not created: ${JSON.stringify(container).slice(0, 200)}`);
  ...
  if (!mediaId) throw new Error(`Instagram media_publish returned no id: ${JSON.stringify(published).slice(0, 200)}`);
  return { mediaId };
}
...
export async function postVideoToFacebookPage(
  ...
  if (!videoId) throw new Error(`Facebook video post returned no id: ${JSON.stringify(resp).slice(0, 200)}`);
  return { videoId };
}

// Cache live validation so the capabilities endpoint isn't a Graph call on
// every dashboard load. { ok, error, at }
const _validationCache = new Map<string, { ok: boolean; error?: string; at: number }>();
 ...
 */
export async function validateMeta(platform: "instagram" | "facebook"): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    if (platform === "instagram") {
      if (!instagramConfigured()) throw new Error("not configured");
      const igId = process.env.INSTAGRAM_BUSINESS_ACCOUNT_ID!.trim();
      const token = process.env.INSTAGRAM_ACCESS_TOKEN!.trim();
