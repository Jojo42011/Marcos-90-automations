...
app.get("/api/geo/status", async (_req, res) => {
  const { isGeoConfigured } = await import("./integrations/geo/index.js");
...
app.get("/api/geo/autocomplete", async (req, res) => {
  try {
    const { autocompleteAddress } = await import("./integrations/geo/index.js");
    const q = typeof req.query.q === "string" ? req.query.q : "";
    res.json(await autocompleteAddress(q));
  } catch (err) {
    res.status(500).json({ ok: false, configured: true, suggestions: [], error: (err as Error).message });
  }
});
 ...
 */
app.post("/api/mojo/webhook", express.json({ limit: "256kb" }), async (req, res) => {
  const { mojoSecretConfigured, mojoSecretOk, mapMojoPayload, mojoPayloadIsUsable, mojoLeadPatch } =
    await import("./core/mojoWebhook.js");
  ...
  const provided =
    ...
    (typeof req.headers["x-mojo-secret"] === "string" ? (req.headers["x-mojo-secret"] as string) : "");
  ...
  const items: Record<string, unknown>[] = Array.isArray(body)
    ...
    : Array.isArray((body as { contacts?: unknown }).contacts)
      ? ((body as { contacts: Record<string, unknown>[] }).contacts)
      : [body];
  ...
  try {
    ...
    for (const raw of items) {
      ...
      if (!mojoPayloadIsUsable(m)) { skipped++; continue; }
...
app.post("/api/browser/poll", express.json({ limit: "64kb" }), async (req, res) => {
  ...
  const operatorTab = b.operatorTab && typeof b.operatorTab === "object"
    ? (b.operatorTab as { url?: string; title?: string })
    : null;
