...
exports.CronError = exports.DEFAULT_TIMEZONE = void 0;
exports.minIntervalMinutes = minIntervalMinutes;
exports.parseCron = parseCron;
exports.smallestIntervalMinutes = smallestIntervalMinutes;
exports.validateSchedule = validateSchedule;
exports.nextRun = nextRun;
exports.parseNaturalSchedule = parseNaturalSchedule;
 ...
 */
const zonedTime_js_1 = require("../core/zonedTime.js");
...
function expandField(raw, min, max, names, fieldName) {
    const field = raw.trim().toLowerCase();
    if (!field)
        throw new CronError(`Empty ${fieldName} field.`);
    const out = new Set();
    let restricted = true;
    for (const part of field.split(",")) {
        const chunk = part.trim();
        if (!chunk)
            throw new CronError(`Empty entry in the ${fieldName} field.`);
        let step = 1;
        let range = chunk;
        ...
        const resolve = (token) => {
            const t = token.trim();
            if (names && names[t] !== undefined)
                return names[t];
            const n = parseInt(t, 10);
            if (!Number.isFinite(n))
                throw new CronError(`"${token}" is not valid in the ${fieldName} field.`);
            return n;
        };
        let lo;
        let hi;
...
function parseCron(expr) {
    ...
    if (fields.length === 6) {
        ...
        throw new CronError("This looks like a 6-field cron with seconds. Use 5 fields: minute hour day-of-month month day-of-week.");
    }
    if (fields.length !== 5) {
...
function smallestIntervalMinutes(fields) {
 ...
 */
function validateSchedule(cron, timezone = exports.DEFAULT_TIMEZONE) {
    const tz = timezone && (0, zonedTime_js_1.isValidTimeZone)(timezone) ? timezone : exports.DEFAULT_TIMEZONE;
    let fields;
    try {
    ...
    if (smallest < floor) {
        return {
            ...
            cron,
            timezone: tz,
            error: `That schedule can fire every ${smallest} minute${smallest === 1 ? "" : "s"}, and the floor is ` +
...
function dayMatches(fields, month, dom, dow) {
 ...
 */
function nextRun(cron, timezone = exports.DEFAULT_TIMEZONE, fromMs = Date.now()) {
    let fields;
    try {
        fields = parseCron(cron);
    }
    ...
    const start = (0, zonedTime_js_1.utcToZonedParts)(fromMs + 60000, tz);
    ...
    for (let dayOffset = 0; dayOffset <= 366 * 4; dayOffset++) {
        ...
        if (!dayMatches(fields, month, dom, dow))
            continue;
        ...
        for (const hour of fields.hours) {
            for (const minute of fields.minutes) {
                ...
                const resolved = (0, zonedTime_js_1.zonedWallToUtc)({ year, month, day: dom, hour, minute, second: 0 }, tz);
...
function parseNaturalSchedule(input) {
    ...
    for (const [word, num] of Object.entries(DOW_WORDS)) {
        if (new RegExp(`\\b${word}s?\\b`).test(text) && !dows.includes(num))
            dows.push(num);
    }
    if (dows.length) {
        const t = at || { hour: 9, minute: 0 };
...
function describeCron(cron, timezone = exports.DEFAULT_TIMEZONE) {
    let f;
    try {
        f = parseCron(cron);
    }
    ...
    const zoneShort = timezone === exports.DEFAULT_TIMEZONE ? "Central" : timezone;
    ...
    if (weekdayOnly && !f.domRestricted)
        return `Weekdays at ${timePart}`;
    ...
    if (f.domRestricted) {
        ...
        const monthPart = f.months.length === 12
            ? "month"
