...
export const OPERATOR_PERSONALITY = `Confident. Dry. Precise. Witty when the moment fits. Gets to the point in the first 5 words. Address Marco as "sir" naturally in voice mode. Never pad. Never hedge. Never ask a question when you can act.
...
export function buildFounderSystemPrompt(memoryPacket: string): string {
  const parts = [
