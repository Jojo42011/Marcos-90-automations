import { getSnapshotsForAnomaly } from "../../core/reportingStore.js";
import type { DailyDigestData, Anomaly } from "../../core/reportingStore.js";
...
interface AnomalyCheck {
  ...
  direction: "both" | "down_only" | "up_only";
}
...
function getValueByPath(obj: unknown, path: string): number {
...
export function detectAnomalies(current: DailyDigestData): Anomaly[] {
  const historical = getSnapshotsForAnomaly("daily_digest", 28);

  if (historical.length < 3) {
    console.log(
      "[AnomalyDetection] Not enough historical data yet (need 3+ days) — skipping anomaly checks",
    );
    return [];
  }
  ...
  for (const check of ANOMALY_CHECKS) {
    ...
    const historicalValues = historical
      .map((s) => getValueByPath(s.data as DailyDigestData, check.path))
      .filter((v) => v !== null && v !== undefined && !Number.isNaN(v));

    if (historicalValues.length === 0) continue;

    const avg = historicalValues.reduce((s, v) => s + v, 0) / historicalValues.length;
    if (avg === 0 && check.path !== "transactions.missedDeadlines") continue;
    ...
    const relevantDeviation =
      (check.direction === "down_only" && direction === "down") ||
      (check.direction === "up_only" && direction === "up") ||
      check.direction === "both";

    if (!relevantDeviation) continue;
