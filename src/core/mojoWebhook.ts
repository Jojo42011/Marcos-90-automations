...
/** The shape Zapier sends. Every field is optional — Mojo's own fields vary by
 *  list, and a payload missing a name is still worth keeping if it has a phone. */
export interface MojoWebhookPayload {
  id?: unknown;
  contact_id?: unknown;
  first_name?: unknown;
  last_name?: unknown;
...
function pick(p: MojoWebhookPayload, keys: string[]): string {
  for (const k of keys) {
    const v = str(p[k]);
    if (v) return v;
  }
...
export function mojoPhone(raw: unknown): string | null {
...
export interface MappedMojoLead {
  ...
  source: string;
  notes: string | null;
  /** The Mojo list/group this came from, kept as a tag so segments survive. */
  tags: string[];
}
 ...
 */
export function mapMojoPayload(p: MojoWebhookPayload): MappedMojoLead {
...
export function mojoPayloadIsUsable(m: MappedMojoLead): boolean {
 ...
 */
export function mojoSecretConfigured(): boolean {
  return Boolean(process.env.MOJO_WEBHOOK_SECRET?.trim());
}

export function mojoSecretOk(provided: string | undefined): boolean {
...
export function mojoLeadPatch(m: MappedMojoLead): Partial<Lead> {
