...
import { searchListings } from "../../core/listingsStore.js";
import {
  upsertCandidates,
  unscoredCandidates,
  setScore,
  ...
  topCandidates,
  luxuryStats,
  type LuxuryCandidate,
} from "../../core/luxuryContentStore.js";
import { complete, isAnthropicApiKeyConfigured } from "../../integrations/llm/index.js";
...
export interface LuxurySweepResult {
  ran: boolean;
  candidatesSeen?: number;
  newlyIngested?: number;
  scored?: number;
  scoreSource?: "claude" | "heuristic" | "mixed" | "none";
  wentOffMarket?: number;
...
function candidateBrief(c: LuxuryCandidate): string {
  const specs = [
...
async function scoreWithClaude(batch: LuxuryCandidate[]): Promise<number> {
  const context = batch.map(candidateBrief).join("\n\n");
  const text = await complete(SCORE_PROMPT, context);
  const jsonMatch = text.match(/\[[\s\S]*\]/);
  if (!jsonMatch) throw new Error("scorer returned no JSON array");
  const parsed = JSON.parse(jsonMatch[0]) as Array<{ key?: string; score?: number; reasons?: string }>;
  let n = 0;
  for (const row of parsed) {
    const key = String(row.key || "").trim();
    const score = Number(row.score);
    if (!key || !Number.isFinite(score)) continue;
    if (!batch.some((c) => c.listingKey === key)) continue; // never trust an invented key
    setScore(key, Math.max(0, Math.min(10, score)), String(row.reasons || "").slice(0, 300), "claude");
    n++;
  }
  return n;
}
...
const FEATURE_WORDS: Array<[RegExp, number, string]> = [
...
function scoreHeuristically(c: LuxuryCandidate): void {
  const text = `${c.remarks || ""} ${c.subdivision || ""}`;
  let score = 3; // a $1M+ home starts as "maybe"
  const hits: string[] = [];
  for (const [re, pts, label] of FEATURE_WORDS) {
    if (re.test(text)) { score += pts; hits.push(label); }
  }
  if ((c.livingArea || 0) > 5000) { score += 1; hits.push("scale"); }
  setScore(c.listingKey, Math.min(10, score),
    hits.length ? `keyword match: ${hits.join(", ")}` : "no standout keywords found",
    "heuristic");
}
...
function refreshShortlist(): number {
  const current = currentShortlist();
  const seats = SHORTLIST_TARGET - current.length;
  if (seats > 0) {
    for (const c of topCandidates(seats)) setStatus(c.listingKey, "shortlist");
  }
  const size = currentShortlist().length;
...
export async function runLuxurySweep(): Promise<LuxurySweepResult> {
  if (running) return { ran: false, error: "already running" };
  running = true;
  try {
    const live = searchListings({ status: "Active", minPrice: MIN_PRICE, limit: 2000 });
    const rows = live.listings;
    const ingested = upsertCandidates(rows.map((l) => ({
    ...
    const wentOffMarket = markOffMarket(new Set(rows.map((l) => l.listingKey)));

    let scored = 0;
    let usedClaude = false;
    let usedHeuristic = false;
    const pending = unscoredCandidates(MAX_SCORED_PER_RUN);
    if (isAnthropicApiKeyConfigured()) {
      for (let i = 0; i < pending.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          console.warn("[luxury] claude scoring failed, falling back to heuristic for this batch:",
            err instanceof Error ? err.message : err);
          for (const c of batch) scoreHeuristically(c);
          scored += batch.length;
          usedHeuristic = true;
        }
      }
    } else {
      for (const c of pending) scoreHeuristically(c);
      scored = pending.length;
      usedHeuristic = pending.length > 0;
    }

    const shortlistSize = refreshShortlist();
    const scoreSource: LuxurySweepResult["scoreSource"] =
      usedClaude && usedHeuristic ? "mixed" : usedClaude ? "claude" : usedHeuristic ? "heuristic" : "none";
...
export function getLuxuryShortlist(): { shortlist: LuxuryCandidate[]; stats: ReturnType<typeof luxuryStats> } {
  return { shortlist: currentShortlist(), stats: luxuryStats() };
}

export function setLuxuryStatus(listingKey: string, status: "filmed" | "dismissed" | "shortlist" | "candidate"): boolean {
  return setStatus(listingKey, status);
}

export function scheduleLuxurySweep(): void {
