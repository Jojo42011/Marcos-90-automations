/**
 * The model catalog: what Harvey is allowed to pick from, and what each one costs.
 *
 * CURATED, NOT THE FULL LIST. OpenRouter serves 400+ models. A picker with 400
 * rows is not a feature, and most of those models cannot hold 120 tool schemas
 * without falling over. So the catalog is a hand-picked set spanning the three
 * families Harvey actually uses (Anthropic, OpenAI, Google) across three tiers,
 * every one of which supports tool calling.
 *
 * THE PRICES HERE ARE A BEST-EFFORT SNAPSHOT, NOT A SOURCE OF TRUTH. Vendor
 * pricing moves, and a stale table quietly under-reports spend. Two guards:
 * `usage.cost` from OpenRouter is what gets recorded (these numbers are only
 * used for the pre-flight estimate and for providers that return no cost), and
 * `refreshCatalog()` pulls live pricing and context windows from OpenRouter's
 * own `/models` endpoint when it is reachable.
 *
 * THE REFRESH IS NEVER ON THE CHAT PATH. It is called from the models UI
 * endpoint, in the background, and every failure mode — no key, network down,
 * malformed body, timeout — degrades silently to the static table. A pricing
 * lookup must never be the reason a chat turn fails.
 */
import type { ModelInfo } from "./types.js";

/** Blank `available` — it depends on which keys exist, so it is filled in later. */
type CatalogEntry = Omit<ModelInfo, "available">;
 ...
 */
const SLUG_TO_ANTHROPIC: Record<string, string> = {
...
export function toSlug(modelId: string): string {
  const id = String(modelId || "").trim();
  if (!id) return id;
  if (id.includes("/")) return id;
  if (LEGACY_TO_SLUG[id]) return LEGACY_TO_SLUG[id];
  /* An unknown bare `claude-*` id is still an Anthropic model; guessing the
     vendor prefix beats refusing to route it. */
  if (/^claude[-.]/i.test(id)) return `anthropic/${id}`;
  return id;
}

/** OpenRouter slug → the id Anthropic's own API accepts. */
export function toAnthropicId(modelId: string): string {
  const id = String(modelId || "").trim();
  if (SLUG_TO_ANTHROPIC[id]) return SLUG_TO_ANTHROPIC[id];
...
let liveOverlay: Map<string, Partial<CatalogEntry>> = new Map();
let lastRefreshAt = 0;
 ...
 */
export async function refreshCatalog(opts: { force?: boolean } = {}): Promise<boolean> {
  const fresh = Date.now() - lastRefreshAt < REFRESH_TTL_MS;
  if (!opts.force && fresh && liveOverlay.size > 0) return true;
  ...
  inFlight = (async () => {
    ...
    try {
      ...
      const next = new Map<string, Partial<CatalogEntry>>();
      for (const raw of list) {
        ...
        if (Number.isFinite(ctx) && ctx > 0) patch.contextTokens = Math.round(ctx);
        const inP = perMillion(row?.pricing?.prompt);
        const outP = perMillion(row?.pricing?.completion);
        if (inP !== null) patch.inputPerM = inP;
        if (outP !== null) patch.outputPerM = outP;
        const modality = row?.architecture?.input_modalities;
        if (Array.isArray(modality)) patch.supportsVision = modality.includes("image");
        const params = row?.supported_parameters;
        if (Array.isArray(params)) patch.supportsTools = params.includes("tools");
...
export function catalogIsLive(): boolean {
  return liveOverlay.size > 0;
}

/* ────────────────────────── reads ────────────────────────── */

function withOverlay(entry: CatalogEntry): CatalogEntry {
  const patch = liveOverlay.get(entry.id);
  return patch ? { ...entry, ...patch } : entry;
}

/** Which providers the configured keys can actually reach. */
export function configuredProviders(): { openrouter: boolean; anthropic: boolean } {
  return {
...
export function getCatalog(): ModelInfo[] {
...
export function availableModels(): ModelInfo[] {
  const tierOrder = { cheap: 0, mid: 1, premium: 2 } as const;
 ...
 */
export function blendedPrice(m: Pick<ModelInfo, "inputPerM" | "outputPerM">): number {
  return (m.inputPerM + 3 * m.outputPerM) / 4;
}

export interface CheapestOpts {
  needsTools?: boolean;
  needsVision?: boolean;
  /** Do not consider anything above this tier. */
  maxTier?: ModelInfo["tier"];
  /** Only models reachable with the current keys. Defaults to true. */
  reachableOnly?: boolean;
  /** Never return one of these (a model that just failed, say). */
  exclude?: string[];
}

/** The cheapest model meeting the constraints, or null when nothing qualifies. */
export function cheapestModel(opts: CheapestOpts = {}): ModelInfo | null {
  const tierOrder = { cheap: 0, mid: 1, premium: 2 } as const;
