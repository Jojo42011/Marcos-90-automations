...
export function maybeAppendCuriosityQuestion(
  speech: string,
  hadToolOnly: boolean,
): string {
  if (hadToolOnly) return speech;
  if (!speech.trim()) return speech;

  const hourMs = 60 * 60 * 1000;
  if (Date.now() - lastCuriosityAt < hourMs) return speech;
  ...
  const asked = db
    ...
    .all(dim) as { question: string }[];
  const askedSet = new Set(asked.map((a) => a.question));
  const available = pool.filter((q) => !askedSet.has(q));
  if (!available.length) return speech;

  const question = available[Math.floor(Math.random() * available.length)];
  const id = randomUUID();
  const now = new Date().toISOString();
  db.prepare(
    "INSERT INTO identity_questions (id, dimension, question, asked_at, answered) VALUES (?, ?, ?, ?, 0)",
  ).run(id, dim, question, now);

  lastCuriosityAt = Date.now();
  return `${speech}\n\n${question}`;
}

export async function storeIdentityAnswer(dimension: string, answer: string): Promise<void> {
  const db = getHullDb();
  const now = new Date().toISOString();
  const id = randomUUID();
  const vec = await embedText(answer);
  db.prepare(
    `INSERT INTO facts (id, content, category, keywords, strength, access_count, last_accessed, created_at, embedding)
     VALUES (?, ?, 'identity', ?, 1.5, 0, ?, ?, ?)`,
  ).run(id, answer.trim(), dimension.toLowerCase(), now, now, vec ? float32ToBlob(vec) : null);

  const row = db
    .prepare("SELECT confidence FROM identity_dimensions WHERE dimension = ?")
    .get(dimension) as { confidence: number } | undefined;
  const conf = Math.min(1, (row?.confidence ?? 0) + 0.2);
  db.prepare("UPDATE identity_dimensions SET confidence = ?, updated_at = ? WHERE dimension = ?").run(
    conf,
    now,
    dimension,
  );
  ...
  db.prepare(
    "UPDATE identity_questions SET answered = 1, answer_fact_id = ? WHERE dimension = ? AND answered = 0 ORDER BY asked_at DESC LIMIT 1",
  ).run(id, dimension);
}
