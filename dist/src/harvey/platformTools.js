 ...
 */
function groundBrowserResult(r) {
    if (r.ok)
        return r;
    return {
        ...r,
        say: "This browser action FAILED — nothing happened in Marco's browser. " +
 ...
 */
const BUYER_STAGE_KEYS = types_js_2.BUYER_STAGES.map((s) => s.key);
const SELLER_STAGE_KEYS = types_js_2.SELLER_STAGES.map((s) => s.key);
...
exports.PLATFORM_TOOL_DEFINITIONS = [
    ...
    {
        name: "browser_scroll",
        description: "Scroll Harvey's tab. Use before reading a search-results page: portals lazy-load listings as you scroll, so a plain read only sees the first few. " +
    ...
    {
        name: "browser_tabs",
        description: "List the tabs Harvey currently has open. Use before working across several sites, and to find the tabId of a tab you opened earlier or the operator dragged into Harvey's group.",
        input_schema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "browser_open_tab",
        description: "Open a URL in an ADDITIONAL tab, keeping the ones already open. Use this — not browser_navigate — whenever you need to compare pages or hold several open at once: navigate REPLACES the current tab and loses what was there. Harvey's tabs are grouped under a cyan 'Harvey' group so the operator can see and close them together.",
        input_schema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Close one of Harvey's tabs, or all of them when no tabId is given. Only ever closes tabs Harvey opened or adopted — never the operator's own. Tidy up when a comparison is finished.",
        input_schema: {
            type: "object",
    ...
    },
    {
        ...
        description: "Take the tab the operator is looking at RIGHT NOW as the one to work on — including a tab they dragged into Harvey's group. Use when they say 'this page' or 'the tab I'm on'. Refuses Harvey's own UI, so he never mistakes his shell for the page in question.",
        input_schema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "browser_console",
        description: "Page errors recorded since Harvey opened this tab — uncaught exceptions, unhandled promise rejections, console.error/warn. Use when a click or form submit did not do what it should have: the page usually says why here. Reports plainly when nothing was recorded rather than implying the page is clean.",
        input_schema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "browser_check_captcha",
        description: "Check whether the page is showing a CAPTCHA. Harvey NEVER solves or bypasses one — if this comes back true, call browser_show_tab and ask the operator to complete it, then carry on.",
        input_schema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "browser_read",
    ...
    {
        name: "crm_api_index",
        description: "List every CRM endpoint you can reach with crm_api, read from this server's real routing table. Call this FIRST whenever you need to do something in the CRM that no other tool covers — do not guess a path. Filter with `contains` to narrow it, e.g. 'listing-alerts', 'auto-plans', 'transactions', 'knowledge'.",
        input_schema: {
            type: "object",
            properties: {
    ...
    {
        name: "update_lead",
        description: "Change a contact's CRM fields: status, pipeline stage, intent, priority, name, phone, email, notes, tags, source, address, or who it is assigned to. Only pass the fields you are actually changing — anything omitted is left alone. Call search_leads first to get the id. Use log_lead_activity (not this) to record that a call or text happened.",
        input_schema: {
            type: "object",
            properties: {
                leadId: { type: "string", description: "Lead id from search_leads." },
                crmStatus: { type: "string", enum: [...types_js_2.CRM_STATUSES], description: "Lead temperature." },
                crmStage: { type: "string", enum: [...types_js_2.CRM_STAGES], description: "Pipeline stage." },
    ...
    },
    {
        ...
        input_schema: {
            ...
            properties: {
                ...
                status: { type: "string", description: `One of: ${types_js_2.TRACKER_STATUSES.join(", ")}` },
...
async function executePlatformTool(name, input) {
    switch (name) {
        ...
        case "browser_navigate": {
            ...
            const r = groundBrowserResult(await (0, browserControl_js_1.run)({
                action: "navigate",
                url,
        ...
        case "browser_wait_for":
            return groundBrowserResult(await (0, browserControl_js_1.run)({
                action: "waitFor",
        ...
        case "browser_structured_data":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "structured" }, { device: str(input.device) || undefined }));
        case "browser_scroll":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "scroll", to: str(input.to) || "bottom" }, { timeoutMs: 30000, device: str(input.device) || undefined }));
        case "browser_tabs":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "tabs" }, { device: str(input.device) || undefined }));
        case "browser_open_tab":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "open_tab", url: str(input.url), focus: input.focus !== false }, { timeoutMs: 40000, device: str(input.device) || undefined }));
        case "browser_close_tab":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "close_tab", tabId: typeof input.tabId === "number" ? input.tabId : undefined }, { device: str(input.device) || undefined }));
        case "browser_use_current_tab":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "use_current_tab" }, { device: str(input.device) || undefined }));
        case "browser_console":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "console" }, { device: str(input.device) || undefined }));
        case "browser_check_captcha":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "captcha" }, { device: str(input.device) || undefined }));
        case "browser_show_tab":
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "focus" }, { device: str(input.device) || undefined }));
        case "browser_screenshot": {
            const r = groundBrowserResult(await (0, browserControl_js_1.run)({ action: "screenshot", maxWidth: num(input.maxWidth, 1000) }, { timeoutMs: 40000, device: str(input.device) || undefined }));
            if (!r.ok || !r.image)
                return r;
        ...
        case "browser_read": {
            ...
            if (r.ok && meta.truncated) {
                const next = r.meta?.nextOffset;
                return { ...r, hint: `This is only part of the page. Call browser_read again with offset=${next} to continue — do NOT report what is below as missing until you have read to the end.` };
            }
            return r;
        }
        case "browser_click": {
            ...
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "click", text: text || undefined, selector: selector || undefined }));
        }
        case "browser_fill": {
            ...
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "fill", fields }));
        }
        case "browser_extract": {
            ...
            return groundBrowserResult(await (0, browserControl_js_1.run)({ action: "extract", schema }));
        }
        ...
        case "get_crm_vocabulary": {
            ...
            const trim = (rows) => needle ? rows.filter((r) => r.name.toLowerCase().includes(needle)) : rows;
            const out = {};
            if (want("stages"))
                out.stageGroups = types_js_1.CRM_STAGE_GROUPS;
            if (want("appointmentTypes"))
                out.appointmentTypeGroups = types_js_1.APPOINTMENT_TYPE_GROUPS;
            if (want("sources")) {
                const rows = trim((0, crmVocabulary_js_1.listVocabulary)("source"));
                ...
                if (rows.length > 60)
                    out.sourcesNote = `${rows.length} sources match; showing the first 60. Pass \`contains\` to narrow.`;
            }
            if (want("tags"))
                out.tags = trim((0, crmVocabulary_js_1.listVocabulary)("tag"));
            return out;
        }
        case "crm_api_index": {
            const routes = (0, crmApiSurface_js_1.getCrmApiCatalogue)();
            if (!routes.length) {
        ...
        case "crm_api": {
            ...
            try {
                ...
                return { error: `Could not reach ${pathname}: ${err.message}` };
            }
            ...
            catch { /* not JSON — hand back the text */ }
            if (!response.ok) {
        ...
        case "update_lead": {
            ...
            if (!setIf("crmStatus", str(input.crmStatus), types_js_2.CRM_STATUSES)) {
                return { error: `Invalid crmStatus. Use one of: ${types_js_2.CRM_STATUSES.join(", ")}` };
            }
            if (!setIf("crmStage", str(input.crmStage), types_js_2.CRM_STAGES)) {
                return { error: `Invalid crmStage. Use one of: ${types_js_2.CRM_STAGES.join(", ")}` };
            }
            ...
            if (str(input.assignedUserId)) {
                ...
                const member = (0, users_js_1.getUsers)().find((u) => u.id === uid);
                if (!member)
                    return { error: `No team member with id ${uid}. Call get_team_status for the roster.` };
        ...
        case "get_tracker_pipelines":
            return {
                statuses: types_js_2.TRACKER_STATUSES,
                buyerStages: types_js_2.BUYER_STAGES,
                sellerStages: types_js_2.SELLER_STAGES,
        ...
        case "update_tracker_record": {
            ...
            const patch = {};
            if (str(input.status)) {
                if (!types_js_2.TRACKER_STATUSES.includes(str(input.status))) {
                    return { error: `'${input.status}' is not a status.`, validStatuses: types_js_2.TRACKER_STATUSES };
                }
