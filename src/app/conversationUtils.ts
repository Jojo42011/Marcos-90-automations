...
const WAVE_EMOJI_RE =
  /(?:\u{1F44B}|\u{1F91A}|\u{1F590}|\u{270B})(?:\u{1F3FB}|\u{1F3FC}|\u{1F3FD}|\u{1F3FE}|\u{1F3FF})?/gu;
 ...
 */
export function isWaveOnlyMessage(text: string): boolean {
  const raw = text.trim();
  if (!raw) return false;

  const withoutWaves = raw.replace(WAVE_EMOJI_RE, "").trim();
  if (withoutWaves === "" && WAVE_EMOJI_RE.test(raw)) return true;
...
export function lastAssistantMessageMatches(conversation: Conversation, target: string): boolean {
  const last = getLastAssistantMessageText(conversation);
  if (!last?.trim() || !target.trim()) return false;
...
export const MARCO_CALL_ASK_ATTEMPT_LINES = [
  MARCO_CALL_ASK_INSTATE,
  MARCO_CALL_ASK_EMAIL_DEFLECT,
  MARCO_REBATE_REPLY,
] as const;
...
export const MAX_CALL_ASK_ATTEMPTS = 2;

/** Count Marco outbounds that substantially match any target in the list. */
export function countAssistantMessagesMatchingAny(
...
export function countCallAskAttempts(conversation: Conversation): number {
  return countAssistantMessagesMatchingAny(conversation, MARCO_CALL_ASK_ATTEMPT_LINES);
}

export function threadContainsGracefulCallAskExit(conversation: Conversation): boolean {
  return conversation.messages.some(
    (m) =>
      m.role === "assistant" &&
      m.text?.trim() &&
      messagesAreSubstantiallyDuplicate(m.text, MARCO_CALL_ASK_GRACEFUL_EXIT),
  );
}

export function threadHadCallAskPath(conversation: Conversation): boolean {
  return countCallAskAttempts(conversation) > 0;
}

export function lastAssistantWasCallAsk(conversation: Conversation): boolean {
  const last = getLastAssistantMessageText(conversation);
  if (!last?.trim()) return false;
  return MARCO_CALL_ASK_ATTEMPT_LINES.some((line) => messagesAreSubstantiallyDuplicate(last, line));
}

/** Phone-captured confirm: call-ask path uses call-specific wording (Option B). */
export function resolvePhoneCapturedReply(conversation: Conversation): string {
  if (threadHadCallAskPath(conversation)) {
    return MARCO_PHONE_CAPTURED_CALL_REPLY;
  }
  return MARCO_PHONE_CAPTURED_REPLY;
}

/** Marco already offered the breakdown packet by text in this thread. */
export function threadContainsBreakdownOffer(conversation: Conversation): boolean {
  for (const m of conversation.messages) {
    if (m.role !== "assistant" || !m.text?.trim()) continue;
    if (messagesAreSubstantiallyDuplicate(m.text, MARCO_PRICE_REPLY)) return true;
    const t = m.text.toLowerCase();
    if (/\bwould it help if i sent\b/.test(t) && /\bbreakdown\b/.test(t)) return true;
    if (/\bsent over the entire breakdown\b/.test(t)) return true;
    if (/\bfull breakdown\b/.test(t) && /\b(text|send|sent|by text)\b/.test(t)) return true;
  }
  return false;
}
 ...
 */
export function signalsAffirmativeBreakdownAgreement(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (!t) return false;
  const digits = t.replace(/\D/g, "");
  if (digits.length >= 10) return false;
  if (isInStateConfirmation(text)) return false;
  if (isEmailDeflection(text)) return false;
  if (asksAboutCommissionRebate(text)) return false;

  if (/^(yes|yeah|yep|yup|sure|ok|okay|please|absolutely|definitely)\b/.test(t)) return true;
  if (/^(yes|yeah|sure)[!.]*$/.test(t)) return true;
  if (
    /\b(yes please|please send|send it|send that|send the|sounds good|that would be (great|helpful|awesome)|go ahead|go for it)\b/.test(
      t,
    )
  ) {
    return true;
  }
  if (/\b(i'?d like|i would like).{0,24}\b(send|sent|breakdown)\b/.test(t)) return true;
  return false;
}

/** Lead agreed to Marco's call-ask (any of the three Bucket F lines). */
export function signalsCallAskAgreement(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (!t) return false;
  if (/^(no|nah|nope|not really)\b/.test(t) && !/\b(yes|yeah|sure|open)\b/.test(t)) return false;
  if (/\b(no thanks|not interested|don'?t want|dont want|can'?t|cant)\b/.test(t) && !/\b(open|yes|sure)\b/.test(t)) {
    return false;
  }
  return (
    /^(yes|yeah|yep|yup|sure|ok|okay|absolutely|definitely)\b/.test(t) ||
    /\b(i'?m open|open to (a |that|it)|that works|sounds good|works for me|five minutes|5 minutes|happy to|would be open)\b/.test(
      t,
    ) ||
 ...
 */
export function isInStateConfirmation(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (!t) return false;

  if (
    /^(yes,?\s*)?(i'?m|im|i am|we'?re|were)\s+(in|based in|located in|living in)\s+(san antonio|\bsa\b|texas)\b/.test(
      t,
    )
  ) {
    return true;
  }
  if (/^(yes|yeah|yep|yup)[,!.]?\s*(i'?m|im|i am)\s+(in\s+)?(san antonio|\bsa\b|texas)\b/.test(t)) return true;
...
export function isEmailDeflection(text: string): boolean {
  if (signalsEmailDeliveryRequest(text)) return true;
  const t = text.trim().toLowerCase();
  if (!t) return false;
  if (/\byou can email me\b/.test(t)) return true;
  if (/\bemail me\b/.test(t) && /\b(at|@|instead|rather|prefer)\b/.test(t)) return true;
  if (/\b(i prefer|i'd prefer|id prefer|rather use)\s+email\b/.test(t)) return true;
  if (/\b(send|reach).{0,20}\bemail\b/.test(t) && /\b(instead|rather|prefer)\b/.test(t)) return true;
  return false;
}
...
export type CallAskBucketFResult =
  | { kind: "number_ask"; reply: string }
  | { kind: "call_ask"; reply: string }
  | { kind: "graceful_exit"; reply: string };
 ...
 */
export function resolveCallAskBucketF(
  conversation: Conversation,
  latestText: string,
): CallAskBucketFResult | null {
  if (threadContainsGracefulCallAskExit(conversation)) return null;

  if (lastAssistantWasCallAsk(conversation) && signalsCallAskAgreement(latestText)) {
    return { kind: "number_ask", reply: MARCO_CALL_NUMBER_ASK_REPLY };
  }

  let triggerReply: string | null = null;
  if (isInStateConfirmation(latestText)) {
    triggerReply = MARCO_CALL_ASK_INSTATE;
  } else if (isEmailDeflection(latestText)) {
    triggerReply = MARCO_CALL_ASK_EMAIL_DEFLECT;
  } else if (asksAboutCommissionRebate(latestText)) {
    triggerReply = MARCO_REBATE_REPLY;
  }
  if (!triggerReply) return null;

  if (countCallAskAttempts(conversation) >= MAX_CALL_ASK_ATTEMPTS) {
    return { kind: "graceful_exit", reply: MARCO_CALL_ASK_GRACEFUL_EXIT };
  }
...
const ACK_ONLY_PATTERNS: RegExp[] = [
...
export function isAcknowledgmentOnly(text: string): boolean {
  ...
  if (
    /\b(what|when|where|how|why|who|does|is|can|could|would|will|are|have|did|was|were)\b/.test(
      lower,
    )
  ) {
    return false;
  ...
  if (ACK_ONLY_PATTERNS.some((p) => p.test(normalized))) return true;
...
export type AcknowledgmentCloseoutTurn = "silence" | "closeout";
 ...
 */
export function resolveAcknowledgmentCloseoutTurn(
  conversation: Conversation,
  latestText: string,
  closeoutLine: string,
  options: { skipIfPhoneCapturedThisTurn: boolean },
): AcknowledgmentCloseoutTurn | null {
  if (options.skipIfPhoneCapturedThisTurn) return null;
  if (!isAcknowledgmentOnly(latestText)) return null;

  const hasMarcoOutbound = conversation.messages.some(
    (m) => m.role === "assistant" && m.text?.trim(),
  );
  if (!hasMarcoOutbound) return null;

  if (lastAssistantMessageMatches(conversation, closeoutLine)) return "silence";
  return "closeout";
}
...
export function messageAsksPropertyPriceOrCost(text: string): boolean {
  ...
  if (/^\s*(price|pricing|cost)\s*[!.?]*\s*$/i.test(t)) return true;
  if (/\bashing\s+price\b/.test(t)) return true;
  return false;
}
