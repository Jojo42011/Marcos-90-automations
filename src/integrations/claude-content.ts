 ...
 */
export function validateAnthropicKey(): boolean {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) {
    console.error(
      "[content-manager] ANTHROPIC_API_KEY not set. AI analysis will not work.",
    );
    return false;
  }
  if (!key.startsWith("sk-ant-")) {
    console.error(
      "[content-manager] ANTHROPIC_API_KEY appears invalid (expected sk-ant-... prefix). " +
...
export function isRetryableClaudeError(err: unknown): boolean {
 ...
 */
export function logContentAiFailure(task: string, err: unknown): void {
  const status = anthropicHttpStatus(err);
  const msg = err instanceof Error ? err.message : String(err);
  const retryable = isRetryableClaudeError(err) ? "retryable" : "non-retryable";
  console.error(
    `[content-ai] ${task} failed (${retryable}${status ? `, HTTP ${status}` : ""}): ${msg}`,
  );
}

export { client as claudeContent };
