...
exports.bootstrapHullMemory = bootstrapHullMemory;
...
function bootstrapHullMemory() {
    const db = (0, store_js_1.getHullDb)();
    const factCount = db.prepare("SELECT COUNT(*) as c FROM facts").get().c;
    if (factCount > 0) {
    ...
    const insertFact = db.prepare(`INSERT INTO facts (id, content, category, keywords, strength, access_count, last_accessed, created_at)
     VALUES (?, ?, ?, ?, ?, 0, ?, ?)`);
    const semanticFacts = [
    ...
    const insertRule = db.prepare(`INSERT INTO rules (id, trigger_condition, action, category, confidence, use_count, created_at, last_reinforced)
     VALUES (?, ?, ?, ?, 0.85, 0, ?, ?)`);
    const tx = db.transaction(() => {
        for (const s of semanticFacts) {
            insertFact.run((0, crypto_1.randomUUID)(), s.content, s.category, s.keywords, s.strength, now, now);
        }
        for (const [aName, aType, rel, bName, bType] of relations) {
            const aId = (0, nodes_js_1.upsertNode)(aName, aType);
            const bId = (0, nodes_js_1.upsertNode)(bName, bType);
            db.prepare(`INSERT INTO edges (id, source_id, target_id, relationship, strength, created_at, last_reinforced)
         VALUES (?, ?, ?, ?, 1.0, ?, ?)`).run((0, crypto_1.randomUUID)(), aId, bId, rel, now, now);
        }
        for (const p of procedures) {
            insertRule.run((0, crypto_1.randomUUID)(), p.trigger, p.action, p.category, now, now);
        }
        for (const dim of DIMENSIONS) {
            db.prepare("INSERT OR IGNORE INTO identity_dimensions (dimension, confidence, updated_at) VALUES (?, 0.0, ?)").run(dim, now);
        }
    });
    tx();
