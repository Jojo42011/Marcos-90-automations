...
exports.matchKnownCity = matchKnownCity;
exports.extractArea = extractArea;
exports.normalizeArea = normalizeArea;
exports.isJunkArea = isJunkArea;
exports.extractPriceCap = extractPriceCap;
exports.isJunkPriceCap = isJunkPriceCap;
...
function isPlacePhrase(words) {
    if (!words.length || words.length > 3)
        return false;
    if (words.some((w) => NOT_A_PLACE.has(w.toLowerCase())))
        return false;
    if (words.length === 1 && BARE_DIRECTIONS.has(words[0].toLowerCase()))
        return false;
...
function matchKnownCity(text) {
    if (!text)
        return null;
    for (const { city } of (0, listingsStore_js_1.knownCities)()) {
...
function properNounPlace(text) {
    ...
    for (const re of patterns) {
        ...
        while (words.length && LEADING_TRIM.has(words[0].toLowerCase()))
            words = words.slice(1);
        /* Capitalised, or an all-caps abbreviation people actually use — "RGV". */
        if (!isPlacePhrase(words))
            continue;
        return words.join(" ");
    }
    return null;
...
function extractPriceCap(text) {
    ...
    if (suffixed) {
        ...
        if (Number.isFinite(n) && n >= PRICE_FLOOR && n <= PRICE_CEILING)
            return Math.round(n);
    }
