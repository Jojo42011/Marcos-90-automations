...
export interface VocabEntry {
  name: string;
  /** People carrying it in the Brivity export. Null for anything added here. */
  brivityPeople: number | null;
  /** True when it came from the seeded export rather than being added here. */
  seeded: boolean;
  /** How many contacts in THIS system carry it right now. Filled by the caller. */
  inUse?: number;
}

/** Sources exactly as exported from Brivity on 2026-08-25: `[name, people]`. */
const SEED_SOURCES: Array<[string, number]> = [
...
const SEED_TAGS: Array<[string, number]> = [
...
export function initVocabularySchema(target?: Database.Database): void {
...
export type VocabKind = "source" | "tag";

const SEEDS: Record<VocabKind, Array<[string, number]>> = {
  source: SEED_SOURCES,
  tag: SEED_TAGS,
};
 ...
 * ASCII puts them.
 */
export function listVocabulary(kind: VocabKind): VocabEntry[] {
  ...
  let custom: Array<{ name: string }> = [];
  try {
    custom = getDb().prepare(`SELECT name FROM crm_vocabulary WHERE kind = ? ORDER BY name`).all(kind) as Array<{ name: string }>;
  } catch (err) {
  ...
  for (const c of custom) {
    ...
    if (!seededKeys.has(c.name.toLowerCase())) byKey.set(c.name, { name: c.name, brivityPeople: null, seeded: false });
  }
...
export function addVocabulary(kind: VocabKind, rawName: string, createdBy?: string): string | null {
  const name = String(rawName || "").trim();
  if (!name) return null;
  if (name.length > 80) return null;
  const key = name.toLowerCase();
  if (SEEDS[kind].some(([n]) => n.toLowerCase() === key)) return null;
  const info = getDb()
 ...
 */
export function removeVocabulary(kind: VocabKind, rawName: string): boolean {
  const key = String(rawName || "").trim().toLowerCase();
  if (!key) return false;
  if (SEEDS[kind].some(([n]) => n.toLowerCase() === key)) return false;
...
export function vocabularyStats(): { sources: number; tags: number; sourcesInBrivity: number; tagsInBrivity: number } {
  return {
