...
function scoreFunnelDepth(lead) {
    const f = STAGE_INTENT[String(lead.state).toLowerCase()];
    return Math.round(exports.WEIGHTS.funnelDepth * (f ?? 0));
}
/**
 ...
 */
function scoreEngagement(leadId) {
    let sms = 0;
    try {
    ...
    const replies = Math.max(sms, (0, db_js_1.getInboundDmCount)(leadId));
    if (replies === 0)
        return 0;
    if (replies === 1)
        return Math.round(exports.WEIGHTS.engagement * 0.35);
    if (replies <= 3)
        return Math.round(exports.WEIGHTS.engagement * 0.6);
    if (replies <= 7)
        return Math.round(exports.WEIGHTS.engagement * 0.85);
    return exports.WEIGHTS.engagement;
}
/**
...
function scoreRecency(lead) {
    const iso = (0, db_js_1.getLastInboundDmAt)(lead.id) || lead.lastActivity || null;
    if (!iso)
        return 0;
    ...
    if (days <= 2)
        return exports.WEIGHTS.recency;
    if (days <= 7)
        return Math.round(exports.WEIGHTS.recency * 0.8);
    if (days <= 14)
        return Math.round(exports.WEIGHTS.recency * 0.6);
    if (days <= 30)
        return Math.round(exports.WEIGHTS.recency * 0.35);
    if (days <= 60)
        return Math.round(exports.WEIGHTS.recency * 0.15);
    return 0;
}
/** A lead nobody can phone is not a call list entry, however interested. */
function scoreContactable(lead) {
    return lead.phone?.trim() ? exports.WEIGHTS.contactable : 0;
}
/**
 ...
 */
function scoreIntentSignals(lead) {
    let pts = 0;
    const timeline = (lead.criteria?.timeline ?? "").toLowerCase().trim();
    if (timeline) {
        if (/asap|immediately|now|this month|urgent/.test(timeline))
            pts += 5;
        else if (/1-3 months|soon|next few months|1 to 3/.test(timeline))
            pts += 4;
        else if (/3-6 months|3 to 6/.test(timeline))
            pts += 2;
        else if (/6\+|6 plus|just looking|exploring|no rush/.test(timeline))
            pts += 1;
        else
            pts += 2;
    }
    const pre = lead.preApprovalStatus;
    if (pre === "approved" || pre === "cash")
        pts += 5;
    else if (pre === "in_progress")
        pts += 3;
    const views = typeof lead.propertyViewsCount === "number" && lead.propertyViewsCount > 0
    ...
    if (showing)
        pts += showing.confirmationStatus === "confirmed" ? 2 : 1;
    return Math.min(exports.WEIGHTS.intentSignals, pts);
}
 ...
 * is cheap.
 */
async function checkAndRunAutoRescoreIfDue(force = false) {
...
function scheduleAutoRescore() {
    const run = () => checkAndRunAutoRescoreIfDue()
        .then((r) => {
        if (r.ran && r.result) {
            console.log(`[AutoRescore] ${r.result.scored} leads — ${r.result.hot} hot, ${r.result.warm} warm, ${r.result.cold} cold`);
        }
    })
    ...
    console.log(`[AutoRescore] Scheduled — first pass in 2 min, then every ${RESCORE_INTERVAL_MS / 3600000}h`);
}
