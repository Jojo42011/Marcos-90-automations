...
import type { FavoriteEntry } from "./favoritesStore.js";
...
const BRAND = rgb(0.055, 0.36, 0.30);      // deep teal — matches the app's accent
...
async function fetchPhoto(url: string): Promise<{ bytes: Uint8Array; kind: "jpg" | "png" } | null> {
  try {
    ...
    if (/png/i.test(type) || (bytes[0] === 0x89 && bytes[1] === 0x50)) return { bytes, kind: "png" };
...
export interface PropertyPdfInput {
  clientName: string;
  entries: FavoriteEntry[];
  /** Origin for the clickable listing links, e.g. https://marco-90-automation.fly.dev */
  linkOrigin: string;
}

export async function buildPropertyPdf(input: PropertyPdfInput): Promise<Uint8Array> {
  ...
  for (const entry of live) {
    ...
    if (!drewPhoto) {
      page.drawRectangle({ x: MARGIN, y: top - PHOTO_H, width: PHOTO_W, height: PHOTO_H, color: rgb(0.92, 0.94, 0.93) });
    ...
    if (cityLine) { page.drawText(winAnsiSafe(cityLine), { x: tx, y: ty, size: 10, font: helv, color: MUTED }); ty -= 15; }
    const specs = [
    ...
    if (specs) { page.drawText(specs, { x: tx, y: ty, size: 10, font: helv, color: INK }); ty -= 15; }
    if (l.subdivision) { page.drawText(winAnsiSafe(l.subdivision), { x: tx, y: ty, size: 9.5, font: helv, color: MUTED }); ty -= 14; }
    if (l.publicRemarks) {
    ...
    else page.node.set(doc.context.obj("Annots") as never, doc.context.obj([linkAnnot]) as never);
    if (l.mlsNumber) {
  ...
  if (gone.length) {
    if (y < 90) { footer(page); page = doc.addPage([PAGE_W, PAGE_H]); y = PAGE_H - MARGIN; }
    page.drawText(
      `${gone.length} previously shortlisted home${gone.length === 1 ? " is" : "s are"} no longer on the market and ${gone.length === 1 ? "was" : "were"} left out.`,
