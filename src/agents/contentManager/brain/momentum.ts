import {
  ...
  getPerformanceModel,
  listRecentPerformancePulls,
  upsertPerformanceModel,
} from "../../../core/contentDb.js";

export function updateMomentumState(): {
  streakType: string;
  streakCount: number;
  changed: boolean;
} {
  const pulls = listRecentPerformancePulls(10);
  let hotStreak = 0;
  let coldStreak = 0;

  for (const p of pulls) {
    if (p.views >= CONTENT_BENCHMARK_VIEWS) {
...
export function getMomentumStrategyAdjustment(streakType: string, streakCount: number): string {
