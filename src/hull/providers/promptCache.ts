 ...
 */
export function withCachedTools<T>(tools: T[] | undefined, model: string): T[] | undefined {
  if (!tools?.length || !cachingEnabled() || !supportsExplicitCache(model)) return tools;
  if (JSON.stringify(tools).length < MIN_CACHEABLE_CHARS) return tools;

  const copy = tools.slice();
  const last = copy[copy.length - 1];
  copy[copy.length - 1] = { ...(last as object), cache_control: EPHEMERAL } as T;
  return copy;
}
 ...
 */
export function withCachedSystem(
  system: string | undefined,
  model: string,
): string | Array<{ type: "text"; text: string; cache_control?: CacheControl }> | undefined {
  if (!system) return system;
  if (!cachingEnabled() || !supportsExplicitCache(model)) return system;
 ...
 */
export function withCachedOpenAiTools<T>(tools: T[] | undefined, model: string): T[] | undefined {
  if (!tools?.length || !cachingEnabled() || !supportsExplicitCache(model)) return tools;
  if (JSON.stringify(tools).length < MIN_CACHEABLE_CHARS) return tools;
  const copy = tools.slice();
  copy[copy.length - 1] = { ...(copy[copy.length - 1] as object), cache_control: EPHEMERAL } as T;
  return copy;
}
 ...
 */
export function withCachedOpenAiSystem<T extends { role?: string; content?: unknown }>(
  messages: T[],
  model: string,
): T[] {
  if (!cachingEnabled() || !supportsExplicitCache(model)) return messages;
  const first = messages[0];
  if (!first || first.role !== "system" || typeof first.content !== "string") return messages;
  if (first.content.length < MIN_CACHEABLE_CHARS) return messages;
 ...
 */
export function withCachedHistory<T>(messages: T[], model: string): T[] {
  if (!cachingEnabled() || !supportsExplicitCache(model)) return messages;
  if (messages.length < 4) return messages;
  if (JSON.stringify(messages).length < MIN_CACHEABLE_CHARS) return messages;
  ...
  const target = messages[idx] as { role?: string; content?: unknown };
  if (!target || typeof target !== "object" || !Array.isArray(target.content)) return messages;

  const copy = messages.slice();
  const blocks = target.content.slice();
  const lastBlock = blocks[blocks.length - 1];
  if (!lastBlock || typeof lastBlock !== "object") return messages;
  blocks[blocks.length - 1] = { ...(lastBlock as object), cache_control: EPHEMERAL };
  copy[idx] = { ...(target as object), content: blocks } as T;
  return copy;
}
