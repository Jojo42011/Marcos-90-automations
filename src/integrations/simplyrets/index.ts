...
export interface FeedProblem {
  ok: false;
  error: string;
  /** What the operator must do, naming the exact variable. */
  fix?: string;
}

export function simplyRetsConfig(): SimplyRetsConfig | FeedProblem {
...
export function isMlsFeedConfigured(): boolean {
  return !("ok" in simplyRetsConfig());
}

function authHeader(cfg: SimplyRetsConfig): string {
  return "Basic " + Buffer.from(`${cfg.username}:${cfg.password}`).toString("base64");
}

/** SimplyRETS paginates with a Link header; pull out the rel="next" target. */
function nextLinkFrom(header: string | null): string | null {
...
async function getPage(url: string, cfg: SimplyRetsConfig): Promise<RawPage | FeedProblem> {
  ...
  try {
    ...
    return {
      ok: true,
      records: Array.isArray(body) ? (body as Record<string, unknown>[]) : [],
...
export interface FetchOptions {
  /**
   * Stop once a record this old (or older) is reached. Combined with
   * `sort=-modified` this is what makes a sync incremental: the walk ends at
   * the first listing already held rather than re-reading the whole board.
   */
 ...
 */
export async function mlsPing(): Promise<
  | { ok: true; vendor: string | null; sampleFields: string[]; sample: Record<string, unknown> | null }
  | FeedProblem
> {
  const cfg = simplyRetsConfig();
  if ("ok" in cfg) return cfg;
  ...
  return {
    ...
    sample: first
      ? {
          ...
          status: (first.mls as Record<string, unknown> | undefined)?.status,
          city: (first.address as Record<string, unknown> | undefined)?.city,
