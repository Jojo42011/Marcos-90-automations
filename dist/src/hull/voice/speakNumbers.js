...
exports.integerToWords = integerToWords;
exports.speakNumbers = speakNumbers;
...
function moneyToWords(numRaw, suffix) {
...
function speakNumbers(text) {
    ...
    out = out.replace(/(?:\+1[\s.-]?)?\(?(\d{3})\)?[\s.-](\d{3})[\s.-](\d{4})\b/g, (_m, a, b, c) => `${digitsSpoken(a)}, ${digitsSpoken(b)}, ${digitsSpoken(c)}`);
    // Money: $425,000 / $40k / $1.2M / $99.50.
    out = out.replace(/\$\s?([\d,]+(?:\.\d+)?)\s*([kKmMbB])?\b/g, (_m, num, suffix) => moneyToWords(num, suffix || ""));
    // Times: 3:30pm / 10:05 AM / 9:00.
    out = out.replace(/\b(\d{1,2}):(\d{2})\s*(am|pm|AM|PM|a\.m\.|p\.m\.)?\b/g, (_m, h, mnt, ap) => {
    ...
    // Percents: 12% / 3.5%.
    out = out.replace(/\b(\d+(?:\.\d+)?)\s?%/g, (_m, num) => `${plainNumberToWords(num)} percent`);
    // Ordinals: 1st, 2nd, 3rd, 4th…
    out = out.replace(/\b(\d+)(st|nd|rd|th)\b/g, (_m, num) => ordinalToWords(parseInt(num, 10)));
    // Ranges between plain integers: 6-8 → "six to eight".
    out = out.replace(/\b(\d{1,3})\s?-\s?(\d{1,3})\b/g, (_m, a, b) => `${integerToWords(parseInt(a, 10))} to ${integerToWords(parseInt(b, 10))}`);
    // Years: 1900–2099, standing alone.
    out = out.replace(/\b(19\d{2}|20\d{2})\b/g, (_m, y) => yearToWords(parseInt(y, 10)));
    // Decimals: 3.5 → "three point five".
    out = out.replace(/\b(\d+)\.(\d+)\b/g, (_m, whole, frac) => `${integerToWords(parseInt(whole, 10))} point ${digitsSpoken(frac)}`);
...
function plainNumberToWords(num) {
    if (num.includes(".")) {
...
function ordinalToWords(n) {
    if (n < 20)
        return ORDINAL_ONES[n] ?? String(n);
    if (n < 100) {
        const t = Math.floor(n / 10);
        const r = n % 10;
