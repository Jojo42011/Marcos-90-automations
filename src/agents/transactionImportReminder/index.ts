...
export const REMINDER_TITLE = "Import the Brivity transaction export";
...
export interface ReminderCheck {
  action: "created" | "kept" | "escalated" | "cleared" | "none";
...
function openReminder(): CommandTask | undefined {
  return getCommandTasks().find((t) => t.title === REMINDER_TITLE && t.status !== "done");
}
...
function body(last: ReturnType<typeof getLastTransactionImport>, age: number | null): string {
  const where =
    "In the CRM open Transactions → Import CSV. It shows what it will change before writing " +
    "anything, and re-importing the same rows updates them rather than creating duplicates.";
  if (last && age !== null) {
    return (
      `Transaction data was last imported ${age} day(s) ago (${last.filename || "a CSV"}), so the ` +
  ...
  const total = getAllTransactions().length;
  return (
 ...
 */
export function checkTransactionImportReminder(): ReminderCheck {
  ...
  if (existing && fresh) {
    updateCommandTask(existing.id, { status: "done" });
    return {
      action: "cleared",
      daysSinceImport: age,
  ...
  if (existing) {
    ...
    if (overdue >= URGENT_AFTER_DAYS && existing.column !== "urgent") {
      updateCommandTask(existing.id, { column: "urgent", color: "red" });
      return {
        action: "escalated",
        daysSinceImport: age,
 ...
 */
export function scheduleTransactionImportReminder(): void {
  const run = () => {
    try {
      const r = checkTransactionImportReminder();
      if (r.action !== "none") console.log(`[txImportReminder] ${r.action}: ${r.reason}`);
    } catch (err) {
      console.error("[txImportReminder] check failed:", (err as Error).message);
    }
  };
  run();
