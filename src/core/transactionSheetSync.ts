...
import { planTransactionImport, applyTransactionImport } from "./transactionImport.js";
...
export interface SheetSyncResult {
  ...
  /** True when nothing changed — apply was skipped, and no import was logged. */
  noChanges?: boolean;
}

/** Most recent run, whatever triggered it — for the status endpoint. */
let lastRun: SheetSyncResult | null = null;
let running = false;
...
export function sheetSyncStatus(): { configured: boolean; running: boolean; lastRun: SheetSyncResult | null } {
...
export function normalizeSheetUrl(raw: string): string {
  ...
  if (!m || url.includes("/d/e/") || /output=csv/.test(url)) return url;
 ...
 */
export async function runSheetSync(opts?: { apply?: boolean }): Promise<SheetSyncResult> {
  ...
  try {
    ...
    const plan = planTransactionImport(text);
    ...
    const result = applyTransactionImport(plan, { filename: "google-sheet", source: "google-sheet" });
  ...
  function finish(r: SheetSyncResult): SheetSyncResult {
    lastRun = r;
    if (!r.ok) console.warn(`[sheetSync] failed: ${r.error}`);
    return r;
  }
}
