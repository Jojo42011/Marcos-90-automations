...
function resolveReferralFlow(lead, conversation, latestText, ctx) {
    ...
    if ((0, conversationUtils_js_1.threadContainsBreakdownOffer)(conversation)) {
        (0, marcoLog_js_1.marcoLog)("out_of_state_mid_thread_no_referral", {
            requestId: ctx.requestId,
            correlationId: ctx.correlationId,
            lead_id: lead.id,
...
async function run(payload, log) {
    ...
    const listingRef = (0, inboundListing_js_1.resolveInboundListingRef)(payload.listingRef, lead.mlsListingKey);
    if (listingRef.outcome !== "none") {
        (0, marcoLog_js_1.marcoLog)("inbound_listing_ref", {
            requestId,
            correlationId,
            lead_id: lead.id,
    ...
    if (pausedForReply) {
        lead = pausedForReply;
        await db.updateLead(lead);
        (0, marcoLog_js_1.marcoLog)("auto_plan_paused_on_reply", { requestId, correlationId, lead_id: lead.id });
    }
