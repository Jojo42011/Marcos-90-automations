...
import type { ModelJob, ProviderId, UsageRecord } from "../hull/providers/types.js";
...
function resolveDbPath(): string {
  return process.env.AI_USAGE_DB_PATH?.trim() || path.join(resolveBase(), "ai-usage.db");
}
 ...
 */
const AI_USAGE_MIGRATABLE_COLUMNS: Record<string, Array<[string, string]>> = {
  ai_requests: [
    ["model_used", "TEXT"],
...
export function migrateAiUsageColumns(database: Database.Database): string[] {
  const added: string[] = [];
...
export function initAiUsageSchema(database: Database.Database): void {
  database.exec(`
...
export function getAiUsageDb(): Database.Database {
  if (db) return db;
 ...
 */
export function recordUsage(rec: UsageRecord & { at?: string; modelUsed?: string }): string | null {
  try {
    const database = getAiUsageDb();
    const id = `air_${randomUUID()}`;
    ...
    const write = database.transaction(() => {
      ...
      database
        .prepare(
          `INSERT INTO ai_daily (day, calls, errors, prompt_tokens, completion_tokens, cost_usd)
...
export function spentToday(at: Date = new Date()): number {
  const row = getAiUsageDb()
    .prepare(`SELECT cost_usd c FROM ai_daily WHERE day = ?`)
    .get(businessDay(at)) as { c?: number } | undefined;
  return Number(row?.c ?? 0);
}

export function spentThisMonth(at: Date = new Date()): number {
  const row = getAiUsageDb()
    .prepare(`SELECT COALESCE(SUM(cost_usd), 0) c FROM ai_daily WHERE day LIKE ?`)
    .get(`${businessMonth(at)}%`) as { c?: number } | undefined;
  return Number(row?.c ?? 0);
}
...
export function sessionCostUsd(sessionId: string): number {
  ...
  try {
    const row = getAiUsageDb()
      ...
      .get(String(sessionId)) as { c?: number } | undefined;
    return Number(row?.c ?? 0);
  } catch {
    return 0;
  }
...
export interface UsageSummary {
  days: number;
  since: string;
  today: { calls: number; errors: number; costUsd: number; promptTokens: number; completionTokens: number };
  month: { costUsd: number };
  totals: { calls: number; errors: number; costUsd: number; promptTokens: number; completionTokens: number };
  byModel: Array<{ model: string; calls: number; costUsd: number; tokens: number }>;
  byJob: Array<{ job: ModelJob | string; calls: number; costUsd: number }>;
...
export function usageSummary(days = 30): UsageSummary {
  ...
  const todayRow = database.prepare(`SELECT * FROM ai_daily WHERE day = ?`).get(today) as
    | Record<string, number>
    | undefined;

  const totals = database
    .prepare(
      `SELECT COALESCE(SUM(calls),0) calls, COALESCE(SUM(errors),0) errors,
  ...
  const byModel = (
    database
      .prepare(
        `SELECT COALESCE(model_used, model) m, COUNT(*) calls, COALESCE(SUM(cost_usd),0) cost,
  ...
  const byJob = (
    database
      .prepare(
        `SELECT job, COUNT(*) calls, COALESCE(SUM(cost_usd),0) cost
           FROM ai_requests WHERE day >= ? GROUP BY job ORDER BY cost DESC`,
      )
      .all(since) as Array<Record<string, unknown>>
  ).map((r) => ({ job: String(r.job), calls: Number(r.calls), costUsd: Number(r.cost) }));

  const daily = (
    database
      .prepare(`SELECT day, calls, cost_usd FROM ai_daily WHERE day >= ? ORDER BY day ASC`)
      .all(since) as Array<Record<string, unknown>>
  ).map((r) => ({ date: String(r.day), calls: Number(r.calls), costUsd: Number(r.cost_usd) }));
...
export function recentErrors(limit = 20): RecentError[] {
  const n = Math.min(Math.max(Math.round(limit) || 20, 1), 200);
  return (
    getAiUsageDb()
      .prepare(
        `SELECT ts, job, COALESCE(model_used, model) m, provider, error
...
function readSetting(key: string): string | null {
  try {
    const row = getAiUsageDb().prepare(`SELECT value FROM ai_settings WHERE key = ?`).get(key) as
      | { value?: string }
      | undefined;
...
export function setCaps(patch: Partial<SpendCaps>): SpendCaps {
  const keys: Array<[keyof SpendCaps, string]> = [
...
const overrideKey = (job: ModelJob | string) => `model_override:${String(job)}`;

/** The operator's stored pick for one job, or null to use routing's default. */
export function getModelOverride(job: ModelJob | string): string | null {
  const raw = readSetting(overrideKey(job));
  return raw && raw.trim() ? raw.trim() : null;
}

export function setModelOverride(job: ModelJob | string, model: string): void {
 ...
 */
export function noteFailure(model: string, error?: string): void {
  try {
    const database = getAiUsageDb();
    const now = new Date();
    const row = database
      .prepare(`SELECT consecutive_failures f FROM ai_breaker WHERE model = ?`)
      .get(String(model)) as { f?: number } | undefined;
    const failures = Number(row?.f ?? 0) + 1;
    const pausedUntil =
      failures >= BREAKER_THRESHOLD ? new Date(now.getTime() + BREAKER_PAUSE_MS).toISOString() : null;
    database
      .prepare(
        `INSERT INTO ai_breaker (model, consecutive_failures, paused_until, last_error, updated_at)
...
export function noteSuccess(model: string): void {
  try {
    getAiUsageDb()
      .prepare(
        `INSERT INTO ai_breaker (model, consecutive_failures, paused_until, last_error, updated_at)
...
export function isModelPaused(model: string, at: Date = new Date()): boolean {
  try {
    const row = getAiUsageDb()
      .prepare(`SELECT paused_until FROM ai_breaker WHERE model = ?`)
      .get(String(model)) as { paused_until?: string | null } | undefined;
...
export type { ModelJob, ProviderId, UsageRecord };
