...
exports.getSocialDb = getSocialDb;
exports.upsertSocialPull = upsertSocialPull;
exports.getLatestDailySnapshot = getLatestDailySnapshot;
exports.getDailySnapshotByDate = getDailySnapshotByDate;
exports.getSocialVideos = getSocialVideos;
exports.socialDataAvailable = socialDataAvailable;
exports.getSocialSummaryForHarvey = getSocialSummaryForHarvey;
exports.getLatestSocialDashboardData = getLatestSocialDashboardData;
exports.saveSocialPull = saveSocialPull;
exports.upsertVideoScore = upsertVideoScore;
exports.getVideoScore = getVideoScore;
exports.saveVideoImprovements = saveVideoImprovements;
exports.getVideoImprovements = getVideoImprovements;
exports.setSocialRefreshTime = setSocialRefreshTime;
exports.getSocialRefreshTime = getSocialRefreshTime;
exports.createCommentReply = createCommentReply;
exports.getPendingCommentReplies = getPendingCommentReplies;
...
let db = null;
function dropLegacySocialTables(database) {
    database.exec(`
...
function initSchema(database) {
    ...
    if (!meta) {
        ...
        const row = database.prepare("SELECT version FROM social_schema_meta").get();
...
function migrateVideoScoresTable(database) {
    const columns = [
        "views_score INTEGER NOT NULL DEFAULT 0",
        "retention_score INTEGER NOT NULL DEFAULT 0",
        "saves_score INTEGER NOT NULL DEFAULT 0",
...
function chicagoDateId(d = new Date()) {
    return new Intl.DateTimeFormat("en-CA", { timeZone: "America/Chicago" }).format(d);
}
function upsertSocialPull(profile, scoredVideos, contentPatterns, avgViewsAtTime, pulledAt) {
  ...
  `);
    ...
    database
        .prepare(`INSERT INTO social_daily_snapshots (
...
function rowToSnapshot(row) {
    ...
    if (typeof raw === "string" && raw.trim()) {
        try {
            contentPatterns = JSON.parse(raw);
        }
        catch {
...
function getLatestDailySnapshot() {
    const database = getSocialDb();
    const row = database
        .prepare(`SELECT * FROM social_daily_snapshots WHERE platform = ? ORDER BY pulled_at DESC LIMIT 1`)
        .get(PLATFORM);
    return row ? rowToSnapshot(row) : null;
}
function getDailySnapshotByDate(dateId) {
    const database = getSocialDb();
    const row = database
...
function videoViews(v) {
    return v.views ?? 0;
}
function videoLikes(v) {
    return v.likes ?? 0;
}
function videoComments(v) {
    return v.comments ?? 0;
}
function videoShares(v) {
    return v.shares ?? 0;
}
/** Harvey tool payload — same source as GET /api/social/data (getLatestSocialDashboardData). */
function getSocialSummaryForHarvey() {
    try {
        ...
        if (weekAgo && weekAgo.avgViews > 0) {
            ...
            weekOverWeekTrend = {
                ...
                direction: wowAvgViewsChangePct > 0 ? "up" : wowAvgViewsChangePct < 0 ? "down" : "flat",
            };
        }
...
function getLatestSocialDashboardData() {
...
function saveSocialPull(profile, scoredVideos, avgViewsAtTime, pulledAt) {
...
function upsertVideoScore(postId, breakdown, pulledAt) {
    const database = getSocialDb();
    const at = pulledAt ?? new Date().toISOString();
    database
        .prepare(`INSERT INTO social_video_scores
        (post_id, score, views_score, retention_score, saves_score, shares_score, tier, viral, avg_views_at_time, pulled_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?)
      ON CONFLICT(post_id) DO UPDATE SET
        score = excluded.score,
        ...
        END`)
        .run(postId, breakdown.score, breakdown.viewsScore, breakdown.retentionScore, breakdown.savesScore, breakdown.sharesScore, breakdown.tier, at);
}
function getVideoScore(postId) {
    const database = getSocialDb();
    const row = database
        .prepare(`SELECT score, views_score, retention_score, saves_score, shares_score, tier
...
function setSocialRefreshTime(time) {
    const database = getSocialDb();
    database
        .prepare(`INSERT INTO social_refresh_schedule (id, pull_time, updated_at) VALUES (1, ?, ?)
       ON CONFLICT(id) DO UPDATE SET pull_time = excluded.pull_time, updated_at = excluded.updated_at`)
        .run(time, new Date().toISOString());
}
function getSocialRefreshTime() {
    const database = getSocialDb();
    const row = database
...
function rowToCommentReply(row) {
    return {
        id: Number(row.id),
...
function createCommentReply(reply) {
    const database = getSocialDb();
    const createdAt = new Date().toISOString();
    const result = database
        .prepare(`INSERT INTO comment_replies
        (post_id, comment_text, author_username, sentiment, generated_reply, status, requires_approval, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(reply.postId ?? null, reply.commentText, reply.authorUsername, reply.sentiment, reply.generatedReply, reply.status, reply.requiresApproval ? 1 : 0, createdAt);
    return { ...reply, id: Number(result.lastInsertRowid), createdAt };
}
function getPendingCommentReplies() {
    const database = getSocialDb();
    const rows = database
...
function updateCommentReplyStatus(id, status, reviewedBy) {
    const database = getSocialDb();
    database
        .prepare(`UPDATE comment_replies SET status = ?, reviewed_at = ?, reviewed_by = ? WHERE id = ?`)
        .run(status, new Date().toISOString(), reviewedBy ?? "marco", id);
}
function saveMorningScanResult(result) {
    const database = getSocialDb();
    migrateMorningScansTable(database);
    database
        .prepare(`INSERT INTO morning_scans (scanned_at, new_comments, new_mentions, lead_intent_flags, fetch_error)
       VALUES (?, ?, ?, ?, ?)`)
        .run(result.scannedAt, result.newComments, result.newMentions, JSON.stringify(result.leadIntentFlags), result.fetchError ?? null);
}
function getLatestMorningScanFromDb() {
    const database = getSocialDb();
    const row = database
    ...
    let leadIntentFlags = [];
    try {
        leadIntentFlags = JSON.parse(String(row.lead_intent_flags));
    }
    catch {
        leadIntentFlags = [];
    }
