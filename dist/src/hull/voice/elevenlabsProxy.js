...
exports.elevenLabsListenUrl = elevenLabsListenUrl;
exports.handleElevenLabsUpgrade = handleElevenLabsUpgrade;
/**
...
function handleElevenLabsUpgrade(request, socket, head, tokenOk) {
    const url = request.url || "";
    if (!url.includes("/api/jarvis/elevenlabs/listen"))
        return false;
    if (!tokenOk(request)) {
    ...
    const elWs = new ws_1.WebSocket(elevenLabsListenUrl(), {
    ...
    wss.handleUpgrade(request, socket, head, (ws) => {
        ...
        ws.on("message", (data, isBinary) => {
            if (elWs.readyState !== ws_1.WebSocket.OPEN)
                return;
            if (isBinary) {
