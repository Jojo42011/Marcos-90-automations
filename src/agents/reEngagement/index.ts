...
import { createNotification } from "../../core/crmNotificationStore.js";
import { logSmsMessage } from "../../core/smsStore.js";
import { getLastInboundMessage } from "../../core/smsStore.js";
...
function marcoNotifyEmail(): string | undefined {
  return process.env.MARCO_EMAIL_ADDRESS?.trim() || process.env.MARCO_EMAIL?.trim() || undefined;
}

export async function isLeadGhosted(lead: Lead): Promise<boolean> {
  if (lead.crmStatus === "dead") return false;
  if (lead.automationPaused) return false;
 ...
 */
export async function handleWebsiteVisit(leadId: string): Promise<{ triggered: boolean; reason?: string }> {
  const lead = await getLeadById(leadId);
  if (!lead) return { triggered: false, reason: "lead_not_found" };

  const now = new Date().toISOString();
  await updateLeadCrmFields({ leadId, lastWebsiteVisitAt: now });

  const refreshed = await getLeadById(leadId);
  if (!refreshed) return { triggered: false, reason: "lead_not_found" };

  const ghosted = await isLeadGhosted(refreshed);
  if (!ghosted) {
    await appendLeadActivity(
      leadId,
      [{ type: "web_visit", description: "Website visit recorded", timestamp: now }],
      { lastActivity: now },
    );
  ...
  if (refreshed.lastReEngagementTriggeredAt) {
    const daysSinceLastTrigger =
      (Date.now() - new Date(refreshed.lastReEngagementTriggeredAt).getTime()) / (24 * 60 * 60 * 1000);
  ...
  await updateLeadCrmFields({ leadId, lastReEngagementTriggeredAt: now });

  return { triggered: true };
}

async function runReEngagementFlow(lead: Lead): Promise<void> {
  ...
  createNotification({ leadId: lead.id, notificationType: "re_engagement", message: notificationMessage });
  ...
  if (marcoNumber) {
    await sendTwilioMessage(normalizeToE164(marcoNumber), `🔔 ${notificationMessage}`);
  }

  const marcoEmail = marcoNotifyEmail();
  if (marcoEmail) {
    await sendEmail(marcoEmail, `Returning lead: ${leadDisplayName}`, notificationMessage);
  } else {
