...
exports.parseSendTime = parseSendTime;
exports.suggestNextGoodTime = suggestNextGoodTime;
/**
 ...
 */
const commandSettings_js_1 = require("./commandSettings.js");
const WEEKDAYS = {
    sunday: 0, sun: 0,
    monday: 1, mon: 1,
 ...
 */
function parseSendTime(phrase, now = new Date(), tz = (0, commandSettings_js_1.getCommandSettings)().timeZone) {
    ...
    if (/\b(last|yesterday|ago|previous)\b/.test(raw))
        return null;
    const finish = (d) => {
    ...
    if (clockMatch)
        minuteOfDay = parseClock(clockMatch[1]);
    if (minuteOfDay == null) {
    ...
    const at = (dayOffset, mins) => {
        const base = new Date(Date.UTC(nowParts.year, nowParts.month - 1, nowParts.day));
        base.setUTCDate(base.getUTCDate() + dayOffset);
        return zonedToUtc(base.getUTCFullYear(), base.getUTCMonth() + 1, base.getUTCDate(), Math.floor(mins / 60), mins % 60, tz);
    };
    if (/\btomorrow\b/.test(raw))
        return finish(at(1, minuteOfDay ?? 9 * 60));
    ...
    for (const [word, target] of Object.entries(WEEKDAYS)) {
        ...
        if (delta === 0 && (/(\bnext\b)/.test(raw) || mins <= nowMinutes))
            delta = 7;
        if (/\bnext\b/.test(raw) && delta !== 7 && delta < 7)
            delta += 0; // "next tue" from Sun = this coming Tue
 ...
 */
function suggestNextGoodTime(now = new Date(), tz = (0, commandSettings_js_1.getCommandSettings)().timeZone) {
