...
let cache: { at: number; facets: MlsFacets } | null = null;
...
function topValues(sql: string, limit: number): Facet[] {
  const rows = getListingsDb().prepare(sql).all({ lim: limit }) as { v: string | null; n: number }[];
  return rows
 ...
 */
function featureFacets(jsonPath: string, minCount: number): Facet[] {
...
export function getMlsFacets(force = false): MlsFacets {
  ...
  const total = Number((db.prepare(`SELECT COUNT(*) n FROM listings`).get() as { n: number })?.n || 0);

  const facets: MlsFacets = {
    total,
    ...
    counties: topValues(
      `SELECT json_extract(raw,'$.geo.county') v, COUNT(*) n FROM listings GROUP BY LOWER(v) ORDER BY n DESC LIMIT @lim`, 40),
    schoolDistricts: topValues(
      `SELECT json_extract(raw,'$.school.district') v, COUNT(*) n FROM listings GROUP BY LOWER(v) ORDER BY n DESC LIMIT @lim`, 80),
    propertyTypes: topValues(`SELECT property_type v, COUNT(*) n FROM listings GROUP BY property_type ORDER BY n DESC LIMIT @lim`, 30),
    subTypes: topValues(
      `SELECT json_extract(raw,'$.property.subType') v, COUNT(*) n FROM listings GROUP BY v ORDER BY n DESC LIMIT @lim`, 30),
  ...
  const emptyCheck = (label: string, expr: string, reason: string) => {
    const n = Number(
      (db.prepare(`SELECT COUNT(*) n FROM listings WHERE ${expr} IS NOT NULL AND ${expr} <> ''`).get() as { n: number })?.n || 0,
    );
    if (n === 0) facets.unavailable.push({ field: label, reason });
  };
  emptyCheck("Garage spaces", `json_extract(raw,'$.property.garageSpaces')`, "SABOR does not publish a garage-space count on this feed.");
  emptyCheck("Map area (draw a boundary)", `json_extract(raw,'$.geo.lat')`,
    "Listings carry no latitude/longitude, so a drawn map boundary cannot be evaluated. Area is set by city, postal code, county, school district or subdivision instead.");
