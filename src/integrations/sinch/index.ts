...
function warnMissingSinchSendEnv(): void {
...
function extractTextMessageText(obj: unknown, depth = 0): string | null {
  if (depth > 12 || !isRecord(obj)) return null;
...
function extractSmsIdentity(obj: unknown, depth = 0): string | null {
 ...
 */
export function receiveInbound(payload: unknown): {
  platform: string;
  userId: string;
  username: null;
  displayName: null;
