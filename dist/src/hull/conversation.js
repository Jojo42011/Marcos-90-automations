...
exports.SESSION_GAP_MS = void 0;
exports.buildConversationState = buildConversationState;
exports.buildRetrievalQuery = buildRetrievalQuery;
exports.getConversationSummary = getConversationSummary;
exports.clearConversationSummary = clearConversationSummary;
exports.noteEvictedTurns = noteEvictedTurns;
...
exports.SESSION_GAP_MS = 45 * 60 * 1000;
 ...
 */
function buildConversationState(turns, now = new Date()) {
    if (!turns.length)
        return "";
    const lines = ["CONVERSATION STATE (rebuilt this turn)"];
    const last = turns[turns.length - 1];
    const lastAt = last.at ? new Date(last.at).getTime() : NaN;
    const gapMs = Number.isFinite(lastAt) ? now.getTime() - lastAt : 0;
    if (gapMs > exports.SESSION_GAP_MS) {
        const hours = gapMs / (60 * 60 * 1000);
        const ago = hours >= 1.5 ? `about ${Math.round(hours)} hours ago` : `${Math.round(gapMs / 60000)} minutes ago`;
        lines.push(`- NEW sitting: the last exchange was ${ago}. A brief two-word re-greeting is fine HERE ONLY, then substance. Do not pretend no time passed.`);
    }
    else {
        lines.push("- Same sitting, live thread: pick it up mid-flow. His short reply (\"yeah do it\", \"what about him?\") points at what YOU just said, so resolve it from the thread. No greeting.");
    }
    const lastAssistant = [...turns].reverse().find((t) => t.role === "assistant");
    if (lastAssistant && /\?\s*$/.test(lastAssistant.content.trim())) {
        lines.push("- Your last turn ended in a QUESTION. His message is most likely the ANSWER to it: respond to it as the answer, react to what he chose, then move forward.");
    }
    lines.push("- NEVER re-ask what the thread already answers. Asking twice tells him you weren't listening.");
    return lines.join("\n");
}
/**
 ...
 */
const DEICTIC = /\b(it|that|this|him|her|them|those|the (first|second|last) one|again|instead|what about|do it|send it|same)\b/i;
function buildRetrievalQuery(message, turns) {
    const m = message.trim();
    if (m.length >= 48 && !DEICTIC.test(m))
        return m; // specific enough alone
    const prior = turns
...
function clearConversationSummary(sessionId) {
 ...
 */
function noteEvictedTurns(sessionId, turns) {
    if (!turns.length)
        return;
...
async function foldSummary(sessionId) {
    if (summarizing.has(sessionId))
        return;
    ...
    try {
        ...
        const res = await client.messages.create({
            ...
            messages: [
                {
                    role: "user",
                    content: `Fold these older conversation turns into the running summary. Keep decisions, names, numbers, and anything either party committed to. Plain prose, under ${SUMMARY_MAX_CHARS} characters, no preamble.\n\n` +
        ...
        const text = res.content
            ...
            .trim();
        if (text)
            summaries.set(sessionId, text.slice(0, SUMMARY_MAX_CHARS));
    }
    catch (err) {
        ...
        if ((pendingEvictions.get(sessionId) ?? []).length)
            void foldSummary(sessionId);
    }
}
