...
import { mkdir, readdir, stat, writeFile as fsWriteFile, readFile as fsReadFile } from "node:fs/promises";
import { join, relative } from "node:path";
import { randomUUID } from "node:crypto";

import { WORKSPACE_ROOT, safePath } from "./workspace.js";

export type ExecMode = "off" | "local" | "sandbox";
export type ExecLanguage = "node" | "python";
...
export function execMode(): ExecMode {
  ...
  if (raw === "local" || raw === "sandbox") return raw;
  return "off";
}

export interface ExecArtifact { path: string; bytes: number }
...
export interface ExecResult {
  ...
  stderr: string;
  truncated: boolean;
  /** Workspace-relative paths: the script, its output, and anything it wrote. */
  files: ExecArtifact[];
  runDir: string;
  error?: string;
  note?: string;
 ...
 */
function scrubbedEnv(runDir: string): Record<string, string> {
  return {
...
async function listRunArtifacts(runDir: string, relDir: string): Promise<ExecArtifact[]> {
  const out: ExecArtifact[] = [];
  const walk = async (dir: string): Promise<void> => {
    ...
    for (const e of entries) {
      ...
      out.push({ path: join(relDir, relative(runDir, full)).split("\\").join("/"), bytes: st.size });
    }
  };
  await walk(runDir);
...
export interface RunScriptOptions {
  language: ExecLanguage;
  code: string;
  /** Workspace files to copy into the run directory so the script can read them. */
  inputs?: string[];
  timeoutMs?: number;
}
 ...
 */
export async function runScript(opts: RunScriptOptions): Promise<ExecResult> {
  const mode = execMode();
  const language: ExecLanguage = opts.language === "python" ? "python" : "node";
  ...
  if (!code.trim()) return { ...base, error: "code is required." };
  ...
  const runDir = join(WORKSPACE_ROOT, "runs", runId);
  ...
  for (const rel of inputs) {
    try {
      ...
      if (!st.isFile() || st.size > MAX_ARTIFACT_BYTES) continue;
      const name = String(rel).split("/").pop() || "input";
      await fsWriteFile(join(runDir, name), await fsReadFile(src));
      copied.push(name);
    } catch {
  ...
  await fsWriteFile(join(runDir, scriptName), code, "utf8");

  let bin: string;
  let args: string[];
  if (language === "python") {
    await fsWriteFile(join(runDir, "sitecustomize.py"), PY_GUARD, "utf8");
    bin = "python3";
    args = ["-I", "-S", "-c",
      `import sys;sys.path.insert(0,'.');import sitecustomize;exec(open('${scriptName}').read(),{'__name__':'__main__','__file__':'${scriptName}'})`];
  } else {
    await fsWriteFile(join(runDir, "guard.cjs"), NODE_GUARD, "utf8");
    bin = process.execPath;
    args = [
      "--permission",
      `--allow-fs-read=${runDir}`, `--allow-fs-read=${runDir}/*`,
      `--allow-fs-write=${runDir}`, `--allow-fs-write=${runDir}/*`,
      "--require", join(runDir, "guard.cjs"),
      scriptName,
    ];
  }

  const started = Date.now();
  const res = await new Promise<{ code: number | null; out: string; err: string; timedOut: boolean }>((resolve) => {
    ...
    child.stdout?.on("data", (d) => { if (out.length < MAX_OUTPUT_CHARS * 2) out += String(d); });
    child.stderr?.on("data", (d) => { if (err.length < MAX_OUTPUT_CHARS * 2) err += String(d); });
  ...
  await fsWriteFile(
    join(runDir, "output.txt"),
    `# exit: ${res.timedOut ? "TIMED OUT" : res.code}\n# duration: ${durationMs}ms\n\n--- stdout ---\n${res.out}\n--- stderr ---\n${res.err}\n`,
    "utf8",
  ).catch(() => {});

  const files = await listRunArtifacts(runDir, relDir);
  const produced = files.filter(
    (f) => !/\/(script\.(js|py)|guard\.cjs|sitecustomize\.py|output\.txt)$/.test(f.path)
...
export function execStatus(): {
  mode: ExecMode; enabled: boolean; languages: ExecLanguage[];
  timeoutMs: number; protections: string[]; limits: string[]; note: string;
} {
  const mode = execMode();
  return {
    mode,
