...
import { CONTENT_MODELS, logContentAiFailure } from "../../../integrations/claude-content.js";
...
export type CmBrainChatMessage = { role: "user" | "assistant"; content: string };

export function getContentBrainModel(): string {
  return process.env.CONTENT_BRAIN_MODEL?.trim() || CONTENT_MODELS.QUALITY;
}
 ...
 */
export async function claudeChatWithTools(input: {
  system: string;
  userMessage: string;
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    try {
      ...
      logContentAiFailure("brain tool-chat round", err);
      throw err;
    }
    ...
    for (const call of toolUses) {
      const result = await input.onToolCall(call.name, (call.input ?? {}) as Record<string, unknown>);
      toolResults.push({
        type: "tool_result",
        tool_use_id: call.id,
...
export async function claudeSimpleChat(userPrompt: string, model?: string): Promise<string> {
  try {
    const response = await client.messages.create({
      model: model ?? CONTENT_MODELS.FAST,
      max_tokens: 600,
    ...
    logContentAiFailure("brain simple chat", err);
    throw err;
  }
}
