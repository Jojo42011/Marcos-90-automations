...
exports.strictApproval = strictApproval;
...
exports.heldToolResultText = heldToolResultText;
exports.__resetApprovals = __resetApprovals;
/**
...
function classifyToolCall(name, args = {}) {
    if (HIGH_RISK[name])
        return { level: "high", reason: HIGH_RISK[name] };
    ...
    if (MEDIUM_RISK[name])
        return { level: "medium", reason: MEDIUM_RISK[name] };
    if (LOW_RISK_WRITES.has(name))
        return { level: "low", reason: "Writes inside this system only." };
...
function needsApproval(name, args = {}) {
    if (!approvalRequired())
        return false;
...
function listPendingApprovals(sessionId) {
    sweep();
...
function decideApproval(id, decision, opts) {
    ...
    if (decision === "denied" && opts?.reason)
        approval.denyReason = opts.reason.slice(0, 500);
    return approval;
}
/** What the model is told when a call was held. It must not retry blindly. */
function heldToolResultText(approval) {
    return (`HELD FOR APPROVAL. ${approval.summary}. ` +
