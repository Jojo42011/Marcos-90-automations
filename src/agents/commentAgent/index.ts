...
import { GLOBAL_CONCISE_TEXTING } from "../../../config/prompts.js";
import {
...
} from "../../core/commentAgentStore.js";
import {
  postCommentReply,
  readBackComment,
  type ZernioInboundComment,
} from "../../integrations/zernio/comments.js";
import { marcoLog, previewText } from "../../app/marcoLog.js";
...
export async function handleInboundComment(
  ...
  if (!posted.success) {
    marcoLog("comment_agent_post_failed", {
  ...
  marcoLog("comment_agent_replied", {
    comment_id: evt.commentId,
    post_id: evt.platformPostId,
    author: username ?? evt.authorId.slice(0, 8),
    bucket: drafted.bucket,
    comment_preview: previewText(back.text ?? evt.text, 90),
    reply_preview: previewText(vetted.text, 120),
