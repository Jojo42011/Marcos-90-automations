...
const WHATSAPP_SEND_TOOL: Tool = {
  name: "whatsapp_send",
  description:
    "Send a WhatsApp message from Marco's linked line (+19804430453). Only when Marco explicitly asks to text/send someone. Allowed contacts: jahan (+17373461943).",
  input_schema: {
    type: "object",
    properties: {
...
const GMAIL_SEND_TOOL: Tool = {
  name: "gmail_send",
  description:
    "Send an email from Marco's Gmail account. Only when Marco explicitly asks to email or send something to a lead or contact. Requires recipient address, subject, and body.",
  input_schema: {
    type: "object",
    properties: {
...
export async function executeHullTool(name: string, input: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === "whatsapp_send") {
    ...
    if (!resolveWhatsAppTarget(to)) {
      return { error: `Not allowed to message ${to}. Allowed: jahan (+17373461943)` };
    }
    return sendWhatsAppViaGateway(to, message);
  }
  ...
  if (name === "gmail_send") {
    ...
    if (!toRaw || !subject || !body) return { error: "to, subject, and body required" };
    console.log(`[gmail] tool gmail_send to=${toRaw} subject="${subject.slice(0, 60)}"`);
    try {
      const result = await sendEmail({ to: toRaw, subject, body, html: input.html === true });
  ...
  return executeHarveyTool(name, input);
}
