...
exports.buildCriteriaSql = buildCriteriaSql;
exports.countMatching = countMatching;
exports.findMatching = findMatching;
exports.statsFor = statsFor;
exports.describeCriteria = describeCriteria;
/**
 ...
 * from a quiet market.
 */
function orEquals(expr, values, key, params) {
    if (!values.length)
        return null;
...
function orContains(expr, values, key, params, mode) {
    if (!values.length)
        return null;
    const parts = values.map((v, i) => {
        const p = `${key}${i}`;
...
function buildCriteriaSql(c) {
    const where = [];
    const params = {};
    const push = (clause) => { if (clause)
        where.push(clause); };
    ...
    const range = (col, min, max, key) => {
        if (isNum(min)) {
            params[key + "Min"] = min;
            where.push(`${col} >= @${key}Min`);
        }
        if (isNum(max)) {
            params[key + "Max"] = max;
            where.push(`${col} <= @${key}Max`);
        }
    };
    ...
    if (c.pool === true)
        where.push(`UPPER(COALESCE(json_extract(raw,'$.property.pool'),'')) = 'Y'`);
    if (c.pool === false)
        where.push(`UPPER(COALESCE(json_extract(raw,'$.property.pool'),'')) <> 'Y'`);
    ...
    if (!c.includeWithoutPhotos)
        where.push(`photo_url IS NOT NULL AND photo_url <> ''`);
...
function countMatching(c) {
    const { where, params } = buildCriteriaSql(c);
    const row = (0, listingsStore_js_1.getListingsDb)().prepare(`SELECT COUNT(*) AS n FROM listings ${where}`).get(params);
    return Number(row?.n || 0);
}
function findMatching(c, limit = 50) {
    const { where, params } = buildCriteriaSql(c);
    const rows = (0, listingsStore_js_1.getListingsDb)()
        .prepare(`SELECT * FROM listings ${where}
       ORDER BY COALESCE(listed_at, modification_ts, synced_at) DESC
       LIMIT @lim`)
        .all({ ...params, lim: Math.min(Math.max(1, limit), 500) });
...
function statsFor(c) {
    ...
    const agg = db
        .prepare(`SELECT COUNT(*) n, MIN(list_price) lo, MAX(list_price) hi, AVG(list_price) avg,
    ...
    if (n > 0) {
        ...
        const pn = Number(db.prepare(`SELECT COUNT(*) AS n FROM listings ${priced}`).get(params)?.n || 0);
        if (pn > 0) {
            ...
            const mid = db
                ...
                .all({ ...params, take, off });
            if (mid.length)
                median = mid.reduce((t, r) => t + Number(r.p), 0) / mid.length;
        }
    }
...
function describeCriteria(c) {
    ...
    if (isNum(c.minPrice) && isNum(c.maxPrice))
        bits.push(`${money(c.minPrice)}–${money(c.maxPrice)}`);
    else if (isNum(c.maxPrice))
        bits.push(`under ${money(c.maxPrice)}`);
    else if (isNum(c.minPrice))
        bits.push(`over ${money(c.minPrice)}`);
    if (isNum(c.minBeds))
        bits.push(`${c.minBeds}+ bed`);
    if (isNum(c.minBaths))
        bits.push(`${c.minBaths}+ bath`);
    if (isNum(c.minSqft))
        bits.push(`${c.minSqft.toLocaleString()}+ sqft`);
    if (c.pool === true)
        bits.push("pool");
    const places = [...clean(c.cities), ...clean(c.postalCodes), ...clean(c.subdivisions), ...clean(c.schoolDistricts)];
    if (places.length)
        bits.push(`in ${places.slice(0, 3).join(", ")}${places.length > 3 ? ` +${places.length - 3}` : ""}`);
