...
export function runJudgment(ctx: HarveyContext, operatorMessage: string): JudgmentResult {
  const focus: HarveyFocusArea[] = [];
  const directives: HarveyDirective[] = [];
  const msg = operatorMessage.toLowerCase();

  const hotSms = ctx.hotLeads.filter(
    (l) => l.hasPhone && (l.crmStatus === "not_contacted" || l.crmStatus === "contacted"),
  );
  ...
  if (stalled > 0) {
    ...
    const igStalled = ctx.stalledOpeningLeads.filter((l) =>
      l.platform.toLowerCase().includes("insta"),
    ).length;
    const ttStalled = ctx.stalledOpeningLeads.filter((l) =>
      l.platform.toLowerCase().includes("tik"),
    ).length;
    directives.push({
      severity: "warn",
