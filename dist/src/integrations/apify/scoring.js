...
exports.calculateVideoScore = calculateVideoScore;
exports.scoreVideos = scoreVideos;
exports.extractContentPatterns = extractContentPatterns;
...
function calculateVideoScore(video, allVideos) {
    ...
    const avgViews = positiveViews.length > 0
    ...
    if (video.avgWatchTime && video.duration && video.duration > 0) {
        ...
        const engagementRateVal = (video.views || 0) > 0
    ...
    const score = Math.round(viewsScore * 0.35 + retentionScore * 0.3 + savesScore * 0.2 + sharesScore * 0.15);
    let tier;
    if (score >= 70)
        tier = "hot";
    else if (score >= 40)
...
function scoreVideos(videos) {
    ...
    const positiveViews = viewCounts.filter((v) => v > 0);
    const avgViewsLog = positiveViews.length > 0
        ? positiveViews.reduce((sum, v) => sum + v, 0) / positiveViews.length
        : 0;
    const p95Views = getRobustMax(viewCounts);
    const blendMax = Math.max((p95Views + avgViewsLog) / 2, 1);
    const topViews = [...viewCounts].sort((a, b) => b - a).slice(0, 10);
    console.log("[Scoring] maxViews (p95):", p95Views, "avgViews:", Math.round(avgViewsLog), "blendMax:", Math.round(blendMax), "top views:", topViews);
    ...
    const avgEngagementRate = rates.length > 0 ? rates.reduce((sum, r) => sum + r, 0) / rates.length : 0;
...
function detectContentCategory(caption) {
    ...
    if (/\b(price|cost|how much|pricing|afford|payment|mortgage)\b/.test(t))
        return "pricing";
    if (/\b(tour|walkthrough|inside|walk through|showing|see the)\b/.test(t))
        return "tour";
    if (/\b(tip|mistake|secret|hack|advice|learn|did you know)\b/.test(t))
        return "educational";
    if (/\b(canyon lake|stone oak|new braunfels|san antonio|boerne|schertz|alamo ranch)\b/.test(t)) {
        return "location";
    }
    return "general";
}
...
function extractContentPatterns(videos) {
    const withViews = videos.filter((v) => v.views > 0);
    const hashtagBuckets = {};
    for (const v of withViews) {
        for (const tag of extractHashtags(v.caption)) {
            if (!hashtagBuckets[tag])
                hashtagBuckets[tag] = { totalViews: 0, count: 0 };
    ...
    for (const v of withViews) {
        ...
        if (!dayBuckets[day])
            dayBuckets[day] = { totalViews: 0, count: 0 };
        dayBuckets[day].totalViews += v.views;
        dayBuckets[day].count += 1;
        if (!hourBuckets[hour])
            hourBuckets[hour] = { totalViews: 0, count: 0 };
        hourBuckets[hour].totalViews += v.views;
        hourBuckets[hour].count += 1;
    }
    let bestPostingDay = null;
    let bestDayAvg = 0;
    ...
    const contentTypeBreakdown = Object.entries(categoryBuckets)
        .filter(([, b]) => b.count > 0)
        .map(([category, b]) => ({
        category,
        videoCount: b.count,
