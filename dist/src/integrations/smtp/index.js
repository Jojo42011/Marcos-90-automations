...
exports.smtpSentToday = smtpSentToday;
exports.getSmtpStatus = getSmtpStatus;
exports.sendViaSmtp = sendViaSmtp;
exports.explainSmtpError = explainSmtpError;
...
const smtpClient_js_1 = require("../../core/smtpClient.js");
...
function smtpSentToday() {
    try {
        if ((0, emailStore_js_1.kvGet)(KV_SENT_DATE) !== today())
            return 0;
        return Number((0, emailStore_js_1.kvGet)(KV_SENT_COUNT) || 0) || 0;
    }
    catch {
        return 0;
    }
}
function recordSend(recipients) {
    try {
        const count = smtpSentToday() + recipients;
        (0, emailStore_js_1.kvSet)(KV_SENT_DATE, today());
        (0, emailStore_js_1.kvSet)(KV_SENT_COUNT, String(count));
        (0, emailStore_js_1.kvSet)(KV_LAST_SENT_AT, new Date().toISOString());
        (0, emailStore_js_1.kvSet)(KV_LAST_ERROR, "");
    }
    catch {
...
function recordError(message) {
    try {
        (0, emailStore_js_1.kvSet)(KV_LAST_ERROR, message.slice(0, 500));
        (0, emailStore_js_1.kvSet)(KV_LAST_ERROR_AT, new Date().toISOString());
    }
    catch {
        /* bookkeeping only */
    }
...
function getSmtpStatus() {
    ...
    return {
        ...
        lastSentAt: (0, emailStore_js_1.kvGet)(KV_LAST_SENT_AT) || null,
        lastError: (0, emailStore_js_1.kvGet)(KV_LAST_ERROR) || null,
        lastErrorAt: (0, emailStore_js_1.kvGet)(KV_LAST_ERROR_AT) || null,
    };
}
/**
...
async function sendViaSmtp(opts) {
    const cfg = getSmtpConfig();
    if (!cfg)
        throw new Error("SMTP is not configured — set GMAIL_SMTP_USER and GMAIL_SMTP_APP_PASSWORD");
    ...
    if (!opts.bypassCap) {
        ...
        if (used + recipientCount > cfg.dailyCap) {
            ...
            throw new Error(`Daily email cap reached (${used}/${cfg.dailyCap} recipients today). ` +
    ...
    try {
        const result = await (0, smtpClient_js_1.smtpSend)({ host: cfg.host, port: cfg.port, secure: cfg.secure }, { user: cfg.user, pass: cfg.pass }, msg);
...
async function verifySmtpConnection() {
    ...
    try {
        const { smtpLoginCheck } = await Promise.resolve().then(() => __importStar(require("../../core/smtpClient.js")));
        await smtpLoginCheck({ host: cfg.host, port: cfg.port, secure: cfg.secure }, { user: cfg.user, pass: cfg.pass });
