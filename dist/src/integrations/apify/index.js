...
Object.defineProperty(exports, "extractContentPatterns", { enumerable: true, get: function () { return scoring_js_1.extractContentPatterns; } });
function num(v) {
...
function isLikelyMismappedMetric(n) {
    if (n <= 0)
        return true;
    if (n >= 1_000_000_000 && n <= 2_100_000_000_000)
        return true;
    return n >= 500_000_000;
}
function pickNum(obj, paths, maxSanity = 500_000_000) {
    for (const p of paths) {
        const v = dig(obj, p);
        const n = num(v);
        if (n > 0 && n <= maxSanity && !isLikelyMismappedMetric(n))
            return n;
    }
    return 0;
}
...
function normalizeVideo(item) {
    ...
    const coverUrl = str(item.coverUrl) ||
        str(item.cover) ||
        str(item.thumbnail) ||
...
function extractProfile(items) {
    for (const item of items) {
        const author = (item.authorMeta && typeof item.authorMeta === "object"
            ? item.authorMeta
            : null) ||
...
function isProfileOnlyRow(item) {
    const id = str(item.id) ||
        str(item.videoId) ||
        str(item.aweme_id) ||
        str(dig(item, "videoMeta.id"));
...
function getApifyClient() {
    ...
    if (!apifyClient) {
        apifyClient = new apify_client_1.ApifyClient({ token });
    }
    return apifyClient;
}
function commentCreatedMs(item) {
    const iso = str(item.createTimeISO) || str(item.createTimeIso);
    if (iso) {
...
function normalizeComment(item) {
    ...
    const author = (item.user && typeof item.user === "object"
        ...
        (item.author && typeof item.author === "object"
            ? item.author
            : null);
    const authorUsername = str(item.uniqueId) ||
        str(item.username) ||
        (author ? str(author.uniqueId) || str(author.username) || str(author.nickName) : "") ||
        "unknown";
    const postId = str(item.videoId) ||
        str(item.postId) ||
        str(item.awemeId) ||
        str(dig(item, "videoMeta.id")) ||
        "";
 ...
 */
async function fetchTikTokComments(postURLs, sinceMs) {
    ...
    for (const raw of items ?? []) {
        ...
        const item = raw;
        const createdMs = commentCreatedMs(item);
        if (createdMs > 0 && createdMs < since)
            continue;
