...
exports.renderReportEmail = renderReportEmail;
...
function shell(inner, sendId, footerNote) {
...
function listingCard(l, sendId, badge) {
...
function renderAlertEmail(input) {
    ...
    const inner = header(n === 1 ? "A new home matching your search" : `${n} new homes matching your search`, `${input.alertName} · ${input.criteriaLine}`) +
    ...
    const footer = `You're getting this because a ${esc(input.frequencyLabel)} listing alert (“${esc(input.alertName)}”) is set up for you.<br/>` +
...
function renderReportEmail(input) {
    ...
    const hi = input.contactName ? `Hi ${input.contactName.split(/\s+/)[0]},` : "Hi,";
    const valueBlock = input.includeHomeValue && r.displayValue != null
    ...
    const stats = `<tr><td style="padding:6px 16px 0"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>` +
    ...
    const inner = header(`What's happening around ${input.address}`, `${r.area.label} · ${r.stats.count} listing${r.stats.count === 1 ? "" : "s"} in this snapshot`) +
        (input.customMessage
    ...
    const footer = `${esc(r.notes.join(" "))}<br/><br/>` +
