 ...
 * Fail-open ledger.
 *
 * The intent gate deliberately fails OPEN — a real buyer must not be dropped
 * because the API hiccuped. The cost of that choice is that an API problem is
 * indistinguishable, from the outside, from the agent deciding to reply: every
 * message gets answered and nothing says why. This ring buffer is the missing
 * evidence. It is in-memory and bounded, so it costs nothing and disappears on
 * restart, which is the right lifetime for "what is happening right now".
 */
export interface FailOpenEvent {
  at: string;
  status: number | undefined;
  kind: "auth" | "credit" | "rate_limit" | "overloaded" | "bad_request" | "network" | "no_api_key" | "other";
  message: string;
}
...
export function classifyAnthropicFailure(e: unknown): { kind: FailOpenEvent["kind"]; status: number | undefined; message: string } {
  ...
  let kind: FailOpenEvent["kind"] = "other";
  if (status === 401 || status === 403) kind = "auth";
  else if (lower.includes("credit balance") || lower.includes("billing") || lower.includes("quota")) kind = "credit";
  else if (status === 429) kind = "rate_limit";
  else if (status === 529 || status === 503) kind = "overloaded";
  else if (status === 400) kind = "bad_request";
  else if (!status) kind = "network";
 ...
 */
export async function anthropicLiveCheck(): Promise<{
  ok: boolean; model: string; latencyMs: number;
  kind?: FailOpenEvent["kind"]; status?: number; error?: string;
}> {
...
function getDynamicRuleReinforcement(userMessage: string): string {
  ...
  if (
    ...
    rules.push(
      "ACTIVE RULE, SHOWING REQUEST DETECTED: Do NOT confirm a same-day showing. Check schedule and guide toward tomorrow. Use: \"Let me check my schedule. I am typically more available in the afternoons. Would tomorrow work for you?\"",
    );
  }
  ...
  if (
    ...
    rules.push(
      "ACTIVE RULE, URGENT PRICE REQUEST DETECTED: Do NOT guess or state a price. Use: \"Let me get that over to you as soon as possible. I'm currently on the move and not at my desk, but I will send all the details as soon as I'm able.\"",
    );
  }
  ...
  if (
    ...
    rules.push(
      "ACTIVE RULE, EMAIL/OPTIONS REQUEST: Only offer email AFTER phone number is captured. If phone is not yet captured, get the phone number first before offering to send anything via email.",
    );
  }
  ...
  if (
    ...
    rules.push(
      "ACTIVE RULE, AMBIGUOUS PROPERTY REFERENCE: Ask which property or video they are referring to. Use screenshot language from GLOBAL rules if property is unknown. Do NOT assume which property they mean.",
    );
  }
...
function openingContextAppendix(
  ...
  if (signalsTourOrScheduleIntent(lastUserText)) {
    lines.push(
      "TOUR_OR_SCHEDULE_SIGNAL: The lead is asking to tour, see the home, or schedule. Do NOT confirm same-day. Guide toward tomorrow with schedule-check language. Pricing is secondary in this turn.",
    );
  }
  ...
  if (threadContainsFirstTimeBuyingQuestion(conversation) || leadThreadSignalsExperiencedBuyer(conversation)) {
    ...
    if (isTikTokPlatform(platform)) {
      lines.push(
        "TIKTOK_AFTER_FIRST_TIME_ANSWER: Acknowledge their answer in a few words, then offer the breakdown AND ask for the best number to text it to, in this same reply. Marco's natural texting voice (warm, not robotic). Do not wait a turn for them to agree to the packet before asking.",
      );
    }
    if (igDmNeutral) {
      lines.push(
        "INSTAGRAM_DM_AFTER_FIRST_TIME_ANSWER: Acknowledge their answer in a few words, then offer the breakdown AND ask for the best number to text it to, in this same reply. Marco's natural texting voice (warm, not robotic). Do not wait a turn for them to agree to the packet before asking.",
      );
    }
  }
 ...
 */
function knownListingBlock(lead: Lead, neutralDm: boolean): string {
  ...
  return (
    "KNOWN_LISTING: You know exactly which home this lead is asking about. " +
    `It is ${address || key}` +
    (specs ? ` (${specs})` : "") +
...
export async function generateMarcoPipelineReply(input: {
  ...
  if (signalsTourOrScheduleIntent(lastUserText)) {
    postOpeningHints.push(
      "TOUR_OR_SCHEDULE_SIGNAL: Lead is focused on seeing the home or scheduling. Do NOT confirm same-day. Guide toward tomorrow. Use Marco's schedule-check language from GLOBAL rules.",
    );
  }
  if (messageAsksListingLocation(lastUserText)) {
    postOpeningHints.push(
