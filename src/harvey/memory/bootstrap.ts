...
export function bootstrapMemory(): void {
  ...
  const count = (db.prepare("SELECT COUNT(*) as c FROM harvey_semantic").get() as { c: number }).c;
  if (count > 0) {
  ...
  const insertAll = db.transaction(() => {
    for (const s of semanticFacts) {
      insertSemantic.run(randomUUID(), s.fact, s.category, s.tags, 1.0, now, now, s.weight);
    }
    for (const [a, rel, b] of relations) {
      insertRelational.run(randomUUID(), a, rel, b, 1.0, now, now);
    }
    for (const p of procedures) {
      insertProcedural.run(randomUUID(), p.trigger, p.action, p.category, now);
    }
  });

  insertAll();
  console.log(
    "[Harvey Memory] Bootstrap complete —",
    semanticFacts.length,
    "facts,",
    relations.length,
    "relations,",
    procedures.length,
