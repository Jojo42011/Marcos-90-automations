...
import { getMarcoClipParam, autoLoadMarcoClip } from "./marco-bridge";
...
function App() {
  ...
  useEffect(() => {
    ...
    // Marco Content Manager deep-link: /editor/#/editor?clip=<id>&token=<t>.
    // Create a vertical (TikTok) project, jump to the editor, and load the clip
    // from the Marco backend onto the timeline. Best-effort — see marco-bridge.
    const marcoClip = getMarcoClipParam();
    if (marcoClip) {
      hasHandledInitialRoute.current = true;
      createNewProject("Marco clip", { width: 1080, height: 1920, frameRate: 30 });
      navigate("editor");
      void autoLoadMarcoClip(marcoClip.clipId, marcoClip.token);
      return;
    }
