...
import { run as runBrowserCommand, status as browserStatus } from "./browserControl.js";
...
export interface SearchResponse {
  ...
  error?: string;
  note?: string;
  /** Present when the browser is the reason this failed, so the caller can say what to fix. */
...
/* ───────────────────────── engines ─────────────────────────
   Two, tried in order. One engine is a single point of failure against markup
   that changes without notice; the fallback is what keeps a redesign at DDG
   from silently ending Harvey's ability to search. */

interface Engine {
  name: string;
  url: (q: string) => string;
  /** Extraction schema in the extension's own syntax; `@href` pulls the attribute. */
  schema: Record<string, string>;
  /** Search engines wrap outbound links in a redirect; undo it. */
  unwrap?: (href: string) => string;
  /** A selector whose presence means results have rendered. */
  ready: string;
}
...
function bingUnwrap(href: string): string {
  try {
    const u = new URL(href, "https://www.bing.com");
    if (!/\/ck\/a/.test(u.pathname)) return href;
    const raw = u.searchParams.get("u");
    if (!raw || !raw.startsWith("a1")) return href;
 ...
 */
function browserProblem(): string | null {
 ...
 */
async function navigateWarm(
  url: string,
  device: string | undefined,
): Promise<{ ok: boolean; url?: string; title?: string; error?: string }> {
  const go = () =>
    runBrowserCommand(
      { action: "navigate", url, focus: false, timeoutMs: NAV_TIMEOUT_MS },
      { device, timeoutMs: NAV_TIMEOUT_MS + 8000 },
    );
...
export async function searchWeb(
  query: string,
  opts: { limit?: number; device?: string } = {},
): Promise<SearchResponse> {
  const q = String(query || "").trim();
  const limit = Math.min(Math.max(Number(opts.limit) || DEFAULT_RESULTS, 1), MAX_RESULTS);
  if (!q) return { ok: false, query: q, results: [], error: "A search query is required." };
  ...
  for (const engine of ENGINES) {
    ...
    if (!nav.ok) continue;   // engine unreachable — try the next one

    // Results are server-rendered, but a slow network still beats us to the read.
    await runBrowserCommand(
      { action: "waitFor", selector: engine.ready, timeoutMs: 8000 },
      { device: opts.device, timeoutMs: 16_000 },
    );

    const ex = await runBrowserCommand(
      { action: "extract", schema: engine.schema },
      { device: opts.device, timeoutMs: 25_000 },
    );
    if (!ex.ok || !ex.data || typeof ex.data !== "object") continue;
  ...
  return {
    ...
    results: [],
    error:
      `Could not read search results from ${tried.join(" or ")}. This is a SCRAPE FAILURE, not an empty result set — ` +
...
export async function readWebPage(
  url: string,
  opts: { device?: string; selector?: string } = {},
  ...
  const problem = browserProblem();
  if (problem) return { ok: false, error: problem, browserProblem: problem };
  ...
  const read = await runBrowserCommand(
    { action: "read", selector: opts.selector || undefined },
    { device: opts.device, timeoutMs: 30_000 },
  );
  if (!read.ok) {
  ...
  const needsLogin = Boolean((read.meta as Record<string, unknown> | undefined)?.needsLogin);
...
export function webResearchAvailable(): boolean {
  return browserStatus().configured;
}
