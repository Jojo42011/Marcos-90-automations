...
export async function enhanceClip(input: {
  ...
  try {
    const captionResp = await brain.chat(
  ...
  try {
    const reasonResp = await brain.chat(
      `Why does this clip align or not align with current trends? In one sentence. Hook: '${hookPrimary}'. Trend score: ${trendScore}. Trend brief: ${trendRecord.trendBrief.slice(0, 200)}`,
    );
    if (reasonResp?.trim()) trendAlignmentReason = reasonResp.trim().split("\n")[0];
  } catch {
    /* use default */
  }
