    ...
    case "daily_plan":
      return {
        ...
        data: {
          ...
          videos: (r.targets as Record<string, unknown> | undefined)?.videos ?? 7,
          calls: (r.targets as Record<string, unknown> | undefined)?.calls ?? 725,
          description: (r.targets as Record<string, unknown> | undefined)?.description ?? "",
          banked: (r.goalStatus as Record<string, unknown> | undefined)?.banked ?? 5956520,
          gap: (r.goalStatus as Record<string, unknown> | undefined)?.gap ?? 14043480,
          percentComplete: (r.goalStatus as Record<string, unknown> | undefined)?.percentComplete ?? 29.8,
          deadline: (r.goalStatus as Record<string, unknown> | undefined)?.deadline ?? "Nov 30, 2026",
          daysRemaining: (r.goalStatus as Record<string, unknown> | undefined)?.daysRemaining ?? 0,
...
export function buildDailyPlanData(): Record<string, unknown> {
  const now = new Date();
...
export function inferPanelFromMessage(message: string): PanelResult {
  const t = message.toLowerCase().trim();
  if (!t) return null;

  if (
    t.includes("daily") || t.includes("game plan") || t.includes("today") ||
  ...
  if (
    t.includes("world") || t.includes("political") || t.includes("legislation") ||
  ...
  if (
    t.includes("market") || t.includes("mortgage") || t.includes("interest rate") ||
  ...
  if (
    t.includes("tiktok") || t.includes("tik tok") || t.includes("video") ||
  ...
  if (
    t.includes("mojo") || t.includes("cold call") || t.includes("dialer") ||
  ...
  if (
    t.includes("20m") || t.includes("$20") || t.includes("goal") ||
  ...
  if (
    t.includes("number") || t.includes("metric") || t.includes("stat") ||
...
export function panelResultToUi(result: PanelResult): {
  panel: string;
  action: string;
