...
function fmtCriteria(lead) {
    ...
    if (c.timeline)
        bits.push(`Timeline: ${c.timeline}`);
    return bits.join(" | ");
}
/** Everything known about this person, as plain text for the model. */
function buildLeadContext(lead, messages) {
    const lines = [];
    const name = lead.name || lead.username || "Unknown";
    lines.push(`CONTACT: ${name}`);
    if (lead.platform)
        lines.push(`Channel: ${lead.platform}`);
    if (lead.source)
        lines.push(`Source: ${lead.source}`);
    if (lead.crmIntent)
        lines.push(`Intent: ${lead.crmIntent}`);
    if (lead.crmStage)
        lines.push(`Pipeline stage: ${lead.crmStage}`);
    if (lead.crmStatus)
        lines.push(`Status: ${lead.crmStatus}`);
    if (lead.state)
        lines.push(`Funnel state: ${lead.state}`);
    if (lead.propertyInquired)
        lines.push(`Property they asked about: ${lead.propertyInquired}`);
    if (lead.adCampaign)
        lines.push(`Came from campaign: ${lead.adCampaign}`);
    const crit = fmtCriteria(lead);
    if (crit)
        lines.push(`What they're looking for: ${crit}`);
    if (Array.isArray(lead.tags) && lead.tags.length)
        lines.push(`Tags: ${lead.tags.join(", ")}`);
    if (lead.crmNotes)
        lines.push(`Notes on file: ${String(lead.crmNotes).slice(0, 800)}`);
    ...
    if (lead.lastActivity)
        lines.push(`Last activity: ${lead.lastActivity}`);
    const convo = messages.slice(-MAX_MESSAGES);
    if (convo.length) {
        lines.push(`\nFULL CONVERSATION (${messages.length} message${messages.length === 1 ? "" : "s"}${messages.length > MAX_MESSAGES ? `, showing the last ${MAX_MESSAGES}` : ""}, oldest first):`);
...
{"reply": "<the message>", "why": "<one short line on why this is the right next move>"}`;
    const raw = await (0, index_js_1.complete)(prompt, context);
    let suggestion = "";
    let rationale;
    try {
...
return an empty string. Return plain text only, no quotes, no JSON.`;
    const raw = (await (0, index_js_1.complete)(prompt, context)).trim();
    ...
    if (out.toLowerCase().startsWith(typed.toLowerCase()))
